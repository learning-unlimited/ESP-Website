...
class Project(models.Model):
    ...
    sortorder = models.AutoField(primary_key=True, core=True) # Numerical sort order, if multiple projects exist; lowest number goes first
...
class ProjectVideoThunk(models.Model):
    ...
    project = models.ForeignKey(Project, edit_inline=models.STACKED, num_in_admin=1)
    ...
    sortorder = models.AutoField(primary_key=True, core=True)
...
class ProjectAuthor(models.Model):
    ...
    project = models.ForeignKey(Project, edit_inline=models.STACKED, num_in_admin=6)
    sortorder = models.AutoField(primary_key=True, core=True) # If multiple people are associated with a project, they are listed as ordered by sortorder, lowest values first
    def __str__(self):
