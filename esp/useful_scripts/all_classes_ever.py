...
all_subjects = (
    ClassSubject.objects.filter(
        status__gt=0,
        sections__status__gt=0,
        sections__meeting_times__isnull=False,
        sections__resourceassignment__isnull=False,
    )
    .exclude(category__is_lunch=True)
    .distinct()
)  # Returns a QuerySet of all ClassSubjects that are approved (and have an approved section), that have a scheduled section, and that aren't in the lunch category
...
for program in programs:
    ...
    if titles:
        titles = list(titles)
        titles = list(map(to_ascii, titles))
        titles = class_title_delimiter.join(titles)
        f.write("%s\n\n\t%s\n\n\n\n" % (program, titles))
