...
parser.add_argument("sched_filename", help="Full path to CSV file from schedules")
parser.add_argument("furnish_filename", help="Full path to CSV file with furnishings")
...
def do_match(items_to_match, possible_options, description, force_unique=False):
    ...
    while True:
        for item in items_to_match:
            ...
            if match_attempt is None or force_manual:
                ...
                print(
                    (
                        "Unable to automatically match {desc} '{}' to possible {desc}s."
                ...
                while not valid:
                    ...
                    if input_idx == "None":
                        ...
                        try:
                            ...
                            print("Invalid choice: {}; try again.".format(input_idx))
            matching_idx[item] = match_attempt
        print("We have the following matchings:")
        print(matching_idx)
        for item, match_idx in matching_idx.items():
            match_name = None if match_idx is None else possible_options[match_idx]
...
resource_matching = do_match(
    RESOURCE_NAMES, furnish_headers, "resource type", force_unique=True
)
...
for row in furnish_classrooms:
    ...
    try:
        ...
        others = [
            False
            if resource_matching[name] is None
            else row[resource_matching[name]] == "Yes"
            if resource_value_matching[name] is None
            else resource_value_matching[name][row[resource_matching[name]]]
