...
for s in sl:
    ...
    if p.contact_guardian:
        ...
        le, created = PlainRedirect.objects.get_or_create(
            original=list_name, destination=pe
        )
    else:
        pe = ""
    outfile.write(
        ...
        % (
            id,
            ln.encode("ascii", "replace"),
            fn.encode("ascii", "replace"),
            ea.encode("ascii", "replace"),
            pe.encode("ascii", "replace"),
        )
    )
...
print(
    "Created %d list entries" % PlainRedirect.objects.filter(original=list_name).count()
)
