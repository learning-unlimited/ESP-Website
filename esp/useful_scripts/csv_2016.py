...
sched_filename = os.path.expanduser(input("Full path to CSV file from schedules: "))
sched_csvfile = open(sched_filename, "r")
sched_reader = csv.reader(sched_csvfile)

furnish_filename = os.path.expanduser(input("Full path to CSV file with furnishings: "))
...
for row in furnish_reader:
    ...
    if row[1]:
        ...
        # find reference from previous program
        results = Resource.objects.filter(res_type=RTYPE_CLASSROOM, name=room_number).order_by("-id")
        if not results:
            print("WARNING: couldn't find reference information for %s; skipping" % room_number)
            continue
        reference = results[0]
        capacity = int(reference.num_students)

    moveable = (row[2] == "Yes") or (row[3] == "Yes")
    tables = (row[2] == "Yes")
    boards = (row[4] == "Yes") or (row[5] == "Yes")
    projector = (row[6] == "Yes")
    speakers = (row[7] == "Yes")
...
for row in sched_reader:
    ...
    if room_number not in rooms_dict:
        print('skip scheduled not furnished %s' % room_number)
        continue
