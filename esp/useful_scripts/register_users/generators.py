...
RandomPerson = collections.namedtuple(
    "RandomPerson",
    (
        "first",
        "last",
...
RandomAddress = collections.namedtuple(
    "RandomAddress", ("street", "city", "state", "zip")
)
...
def random_email(first, last):
    return "%s%s@%s" % (first.lower()[0], last.lower(), random.choice(email_domains))


RandomClass = collections.namedtuple(
    "RandomClass",
    (
        "title",
        "description",
...
def get_random_dob(grade):
    ...
    return datetime.date(current_year - grade - 6, 1, 1) + datetime.timedelta(
        days=random.randint(-182, 182)
    )
