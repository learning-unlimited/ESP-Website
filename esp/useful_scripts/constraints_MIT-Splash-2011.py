...
def setup_lunch_constraint(program_id, morning_ids, lunch_ids, afternoon_ids, failure_code):
    ...
    constraint.on_failure = failure_code
    constraint.save()
...
def generate_lunch_and_constraints(program_id, morning_ids, lunch_ids, afternoon_ids):
...
""" % lunch_ids
    ...
    #   Ensure that there is at least one "Lunch" class in the program on the desired day
    lunch_subjects = ClassSubject.objects.filter(parent_program__id=program_id, category__category='Lunch', sections__meeting_times__in=lunch_ids)
    lunch_subject = None
    ...
    for ts_id in lunch_ids:
        ...
        d = (event.start - event.end).seconds / 3600.0
        if lunch_sections.count() == 0:
            new_section = lunch_subject.add_section(status=10, duration=d)
    ...
    print 'Generated constraint: %s' % setup_lunch_constraint(program_id, morning_ids, lunch_ids, afternoon_ids, ON_FAILURE_CODE)


#   Stuff specific to MIT Splash Fall 2011 (Saturday)
program_id = 74
morning_ids = [549, 550, 551]
lunch_ids = [552, 553]
afternoon_ids = [554, 555, 556, 557]
evening_ids = [559, 560, 561]
generate_lunch_and_constraints(program_id, morning_ids, lunch_ids, afternoon_ids)
#   End of stuff specific to MIT Splash Fall 2011 (Saturday)

#   Stuff specific to Stanford Splash Fall 2011 (Sunday)
program_id = 74
morning_ids = [563, 564, 565]
lunch_ids = [566, 567]
afternoon_ids = [568, 569, 570, 571, 572]
generate_lunch_and_constraints(program_id, morning_ids, lunch_ids, afternoon_ids)
#   End of stuff specific to Splash Fall 2011 (Sunday)
