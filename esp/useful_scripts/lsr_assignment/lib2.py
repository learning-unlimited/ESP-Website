...
program = Program.objects.filter(id=75)[0]
print "Initialized program to", program

# Lunch hours, for checking whether a student has lunch free.
#satlunch = tuple([int(x.id) for x in Event.objects.filter(id__in=[522,518])]) # for Spark 2012, lunch is 12-1, 1-2 on Saturday, March 10
satlunch = (582, 583)
