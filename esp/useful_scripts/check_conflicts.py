...
sections = ClassSection.objects.filter(
    parent_class__parent_program__anchor__name=program_name
)
...
for s in sections:
    ...
    if (
        expected_duration != actual_duration and actual_duration > 0
    ):  # >0 to avoid unscheduled classes
        print(
            str(s.emailcode())
...
print(
    "Checking: The Events in the Resources for the section match the Events of the meeting_times for the section."
)
for s in sections:
...
for t in teachers:
    sections_taught = list(
        t.getTaughtSectionsFromProgram(program)
    )  # I don't trust the QuerySet to iterate in the same order for both loops.
            ...
            if len(c2_times) > 0:
                if set(c1_times).intersection(c2_times) != set():
                    print(
                        ...
                        + str(t)
                        + " at "
                        + str(list(set(c1_times).intersection(c2_times)))
                    )

# No two class sections are using the same resources (i.e. same room at the same time).
print(
    "Checking: No two class sections are using the same resources (i.e. same room at the same time)."
)
sectionlist = list(
    sections
)  # I don't trust the QuerySet to iterate in the same order for both loops.
            ...
            if s1_used_resources.intersection(s2_used_resources) != set():
                print(
                    ...
                    + str(
                        [
                            [x.name, x.event]
                            for x in set(s1_used_resources).intersection(
                                s2_used_resources
                            )
                        ]
                    )
