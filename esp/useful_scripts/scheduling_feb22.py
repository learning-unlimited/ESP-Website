...
for resource in floating_resources:
    file.write(
        '<resource id=%d fixed=0 name="%s" avail=1 />\n'
...
for cls in classes:
    ...
    title_sanitized = cgi.escape(
        [x for x in cls.title() if x in string.printable]
    ).replace('"', "&quot;")
    file.write(
        '<class id=%d regID="%s" category="%s" name="%s" cap=%d sections=%d duration=%d>\n'
        % (
            cls.id,
            cls.emailcode(),
            cls.category.category,
            title_sanitized,
            sec.max_class_capacity or cls.class_size_max,
            cls.sections.count(),
            int(float(sec.duration) + 0.5),
        )
    )
