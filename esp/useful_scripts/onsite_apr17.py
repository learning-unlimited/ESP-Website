...
bits = (
    UserBit.valid_objects()
    .filter(verb=filter_verb, qsc=splash.anchor)
    .order_by("startdate")
)

outfile.write(
    '"Checkin date","Checkin time","Student first name","Student last name","Student email","Teacher","Class code","Subject","Parent first name","Parent last name","Parent email","Street","City","State","Zip code"\n'
)
    ...
    for sec in student.getEnrolledSections(splash):
        if sec in appropriate_sections:
            ...
            outfile.write(
                ...
                % (
                    dateonly(bit.startdate),
                    timeonly(bit.startdate),
                    cu.first_name,
                    cu.last_name,
                    cu.e_mail,
                    tl,
                    sec.emailcode(),
                    ascii_sanitize(sec.title()),
                    cg.first_name,
                    cg.last_name,
                    cg.e_mail,
                    cu.address_street,
