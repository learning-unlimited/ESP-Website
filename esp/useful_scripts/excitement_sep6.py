...
def get_excitement(question_id, threshold):
    ...
    print(
        " -> Total number of responses: %d (%d answered this question)"
        % (num_responses, num_answers)
    )
    if num_answers > 0:
        print(
            " -> Number of responses above threshold for one or more classes: %d/%d (%.2f%%)"
            % (num_excited_one, num_answers, float(num_excited_one) / num_answers * 100)
        )
        print(
            " -> Number of responses above threshold for all classes rated: %d/%d (%.2f%%); avg %.2f classes/student"
            % (
                num_excited_all,
                num_answers,
                float(num_excited_all) / num_answers * 100,
                float(num_classes_taken) / num_excited_all,
            )
        )
