...
def setup_lunch_constraint(program_id, morning_ids, lunch_ids, afternoon_ids):
    ...
    exp_requirement.label = 'Choose a lunch period at either 12pm or 1pm' + constraint_suffix
...
if __name__ == '__main__':
    ...
    #   Find day of specified times
    lunch_day = Event.objects.get(id=lunch_ids[0]).start.day
    
    #   Ensure that there is at least one "Lunch" class in the program
    lunch_subjects = ClassSubject.objects.filter(parent_program__id=program_id, category__category='Lunch', sections__meeting_times__start__day=lunch_day)
    lunch_subject = None
    if lunch_subjects.count() == 0:
        ...
        new_subject.class_info = 'Join us on the main quad for lunch - either bring your own, or pick up a burrito or pizza from us.'
        ...
        new_anchor.friendly_name = title_prefix + 'Lunch Period'
        new_anchor.save()
