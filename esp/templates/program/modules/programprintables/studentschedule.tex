...
{% if "multiweek_schedule"|getBooleanProgramTag:program %}
{% classes_by_day student.classes as day_entries %}
{% for entry in day_entries %}\hline
{% if not entry.day_time %}N/A{% endif %}{{ entry.day_time }} \newline
\footnotesize{ {% if entry.cls.initial_rooms|length_is:0 %}N/A{% else %}{{ entry.cls.prettyrooms.0|texescape }}{% endif %} } &
{% if entry.cls.event_type.description == "Compulsory" %}{{ entry.cls.description }}{% elif entry.cls.title|length_is:"0" %}N/A{% else %}{{ entry.cls.title|truncatewords_char:45|texescape }}{% endif %} \newline
