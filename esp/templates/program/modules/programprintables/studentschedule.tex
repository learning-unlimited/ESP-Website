...
{% else %}
  ...
  {% else %}
  \multicolumn{1}{|c|}{ {{ cls.time_blocks.0.start|date:"f" }} -- {{ cls.time_blocks.0.end|date:"f A" }} } & {% if not cls.locations.exists %}N/A{% else %}{{ cls.prettyrooms|join:", "|texescape }}{% endif %} & {{ cls.emailcode }}: {{ cls.title|truncatewords:5|texescape }}
  {% endifequal %}
{% endifequal %}
\\ 
{% endfor %}\hline
