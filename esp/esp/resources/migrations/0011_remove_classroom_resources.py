...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        if orm.Resource.objects.filter(res_type__name='Classroom',
                                       res_group__location__isnull=True
                                       ).exists():
            raise Exception('This migration should be run immediately after '
                            '0010 to avoid losing data.  Unfortunately, at '
                            "this point you'll probably have to manually "
                            'migrate any resources that changed in the '
                            'meantime; take a look at the source of migration '
                            '0010 for hints as to how to do so.  Or reverse '
                            'migration 0010 to throw away the migrated data '
                            'and start from scratch.')
        orm.Resource.objects.filter(res_type__name='Classroom').delete()


    def backwards(self, orm):
        "Write your backwards methods here."
        # I'd really like to implement this, but there are enough weird edge
        # cases that I don't think I could get it close enough to right to be
        # useful.  Load your old resources from a DB dump, or do it manually,
        # and good luck.  If you intend to be moving back and forth a lot, just
        # skip this migration for the moment, having the old resources around
    ...
    models = {
        ...
        'program.registrationprofile': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'last_ts': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2015, 3, 28, 0, 0)'}),
        ...
        'resources.resourcegroup': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'location': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['resources.Location']", 'null': 'True', 'blank': 'True'})
        },
    ...
    complete_apps = ['program', 'cal', 'resources']
    symmetrical = True
