...
class ResourceRequestForm(forms.Form):
    resource_type = IDBasedModelChoiceField(
        queryset=ResourceType.objects.all(), widget=forms.HiddenInput
    )
    desired_value = forms.ChoiceField(
    ...
    def __init__(self, data=None, **kwargs):
        ...
        if hasattr(self, "resource_type"):
            ...
            if self.resource_type.only_one:
                ...
                self.fields["desired_value"] = forms.MultipleChoiceField(
            ...
            self.fields["desired_value"].choices = list(
                zip(self.resource_type.choices, self.resource_type.choices)
            )
