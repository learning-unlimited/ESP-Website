...
def create_question_types(apps, schema_editor):
    ...
    QuestionType.objects.using(db_alias).get_or_create(
        name="Instruction Text", _param_names="Text"
    )
    QuestionType.objects.using(db_alias).get_or_create(
        name="Long Answer", _param_names="Rows"
    )
    QuestionType.objects.using(db_alias).get_or_create(
        name="Short Answer", _param_names="Input length"
    )
    QuestionType.objects.using(db_alias).get_or_create(
    ...
    QuestionType.objects.using(db_alias).get_or_create(
        name="Checkboxes", is_countable=True
    )
    QuestionType.objects.using(db_alias).get_or_create(
        name="Multiple Choice", is_countable=True
    )
    QuestionType.objects.using(db_alias).get_or_create(
        name="Yes-No Response", is_countable=True
    )
