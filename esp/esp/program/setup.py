...
def prepare_program(program, data):
    ...
    perms += [
        (
            "Student/All",
            None,
            data.get("student_reg_start"),
            data.get("student_reg_end"),
        )
    ]  # it is recursive
    ...
    perms += [
        (
            "Teacher/All",
            None,
            data.get("teacher_reg_start"),
            data.get("teacher_reg_end"),
        )
    ]
    ...
    for pm in data.get("program_modules", []):
        ...
        if handler_cls:
            ...
            for perm_type in perm_types:
                ...
                if (
                    perm_type.startswith("Student")
                    or getattr(pm, "module_type", "") == "learn"
                ):
                    start_date = data.get("student_reg_start")
                    end_date = data.get("student_reg_end")
                elif (
                    perm_type.startswith("Teacher")
                    or getattr(pm, "module_type", "") == "teach"
                ):
                    start_date = data.get("teacher_reg_start")
                    end_date = data.get("teacher_reg_end")
                elif (
                    perm_type.startswith("Volunteer")
                    or getattr(pm, "module_type", "") == "volunteer"
                ):
                    start_date = data.get("teacher_reg_start") or data.get(
                        "student_reg_start"
                    )
                    end_date = data.get("teacher_reg_end") or data.get(
                        "student_reg_end"
                    )
                else:
                    start_date = data.get("student_reg_start")
...
def commit_program(prog, perms, cost=0, sibling_discount=None):
    ...
    prog.sibling_discount = (
        sibling_discount  # property saves Tag and LineItemType, no explicit save needed
    )
