...
class ArchiveClassAdmin(admin.ModelAdmin):
    list_display = ('id', 'title', 'year', 'date',
                    'category', 'program', 'teacher')
    search_fields = ['id', 'description',
                     'title', 'program', 'teacher', 'category']
    pass
...
class ProgramAdmin(admin.ModelAdmin):
    ...
    list_display = ('id', 'name', 'url', 'director_email',
                    'grade_min', 'grade_max',)
...
class FinancialAidRequestAdmin(admin.ModelAdmin):
    list_display = ('user', 'approved', 'reduced_lunch',
                    'program', 'household_income', 'extra_explaination')
    ...
    def approve(self, request, queryset):
        ...
        for req in queryset:
            req.approve()
        self.message_user(
            request, "%s request(s) successfully approved." % num_approved)
...
class ScheduleTestOccupiedAdmin(admin.ModelAdmin):
    ...
    list_display = ('timeblock', program, 'expr', 'seq',
                    subclass_instance_type, 'text')
...
class ScheduleTestCategoryAdmin(admin.ModelAdmin):
    def program(obj):
        return obj.timeblock.program
    list_display = ('timeblock', program, 'category', 'expr',
                    'seq', subclass_instance_type, 'text')
    list_filter = ('timeblock__program',)
...
class ScheduleTestSectionListAdmin(admin.ModelAdmin):
    def program(obj):
        return obj.timeblock.program
    list_display = ('timeblock', program, 'section_ids',
                    'expr', 'seq', subclass_instance_type, 'text')
    list_filter = ('timeblock__program',)
...
class VolunteerOfferAdmin(admin.ModelAdmin):
    ...
    list_display = ('id', 'user', 'email', 'name', description,
                    time, date, program, 'confirmed')
...
def expire_student_registrations(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request, "%s registration(s) successfully expired" % len(queryset))
...
def renew_student_registrations(modeladmin, request, queryset):
    for reg in queryset:
        reg.unexpire()
    modeladmin.message_user(
        request, "%s registration(s) successfully renewed" % len(queryset))


class StudentRegistrationAdmin(admin.ModelAdmin):
    list_display = ('id', 'section', 'user', 'relationship',
                    'start_date', 'end_date',)
    ...
    list_filter = ['section__parent_class__parent_program',
                   'relationship', ExpiredListFilter]
...
class SectionAdmin(admin.ModelAdmin):
    list_display = ('emailcode', 'title', 'friendly_times', 'status', 'duration',
                    'max_class_capacity', sec_teacher_optimal_capacity, sec_classrooms)
    list_display_links = ('title',)
    list_filter = ['status', 'parent_class__parent_program']
    search_fields = ['=id', '=parent_class__id', 'parent_class__title',
                     'parent_class__class_info', 'resourceassignment__resource__name']
...
class ClassFlagAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['modified_by',
                       'modified_time', 'created_by', 'created_time']
    search_fields = default_user_search('modified_by') + default_user_search('created_by') + [
        'flag_type__name', 'flag_type__id', 'subject__id', 'subject__title', 'subject__parent_program__url', 'comment']
