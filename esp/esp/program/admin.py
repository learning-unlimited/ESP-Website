...
class RegistrationProfileAdmin(admin.ModelAdmin):
    ...
    search_fields = default_user_search() + [
        ...
        "contact_guardian__first_name",
        "contact_guardian__last_name",
        "contact_emergency__first_name",
        "contact_emergency__last_name",
    ]
    list_filter = ("program",)
...
class FinancialAidGrantInline(admin.TabularInline):
    ...
    max_num = 1
    verbose_name_plural = (
        'Financial aid grant - enter 100 in "Percent" field to waive entire cost'
    )
...
class Admin_SplashInfo(admin.ModelAdmin):
    list_display = ("student", "program", "siblingdiscount", "siblingname", "submitted")
...
class StudentRegistrationAdmin(admin.ModelAdmin):
    ...
    search_fields = default_user_search(prefix="^") + [
        "=id",
        "=section__id",
        "=section__parent_class__id",
        "section__parent_class__title",
    ]
    list_select_related = (
        "user",
        "relationship",
        ...
        "section__parent_class__category",
    )
    list_filter = [
...
def sec_classrooms(obj):
    return "; ".join(
        list(
            {
                x.name + ": " + str(x.num_students) + " students"
                for x in obj.classrooms()
            }
        )
    )
...
def sec_teacher_optimal_capacity(obj):
    return (
        obj.parent_class.class_size_max
        if obj.parent_class.class_size_max
        else obj.parent_class.class_size_optimal
    )
...
class ClassFlagAdmin(admin.ModelAdmin):
    ...
    search_fields = (
        default_user_search("modified_by")
        + default_user_search("created_by")
        + [
            "flag_type__name",
            "flag_type__id",
            "subject__id",
            "subject__title",
            "subject__parent_program__url",
            "comment",
        ]
    )
    ...
    def clean_program(self):
        ...
        if program is None:
            raise forms.ValidationError(
                _("Program is required. Please select a program.")
            )
        return program
