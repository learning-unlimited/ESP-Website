"""Tests for batched catalog attribute lookups.

media_count, _index_qsd and _studentapps_count used to come from one
correlated SQL subquery per catalog row; they now come from three batched
queries keyed by class id.
"""
...
class CatalogBatchedAttributesTest(ProgramFrameworkTest):
    ...
    def setUp(self):
        ...
        #   LocMemCache survives the per-test transaction rollback.
        cache.clear()
    ...
    def _nav_category(self):
        return NavBarCategory.objects.get_or_create(
            name='learn', long_explanation='', include_auto_links=False)[0]
    ...
    def _make_index_qsd(self, cls):
        ...
        return QuasiStaticData.objects.create(
            ...
            nav_category=self._nav_category(),
        )
    ...
    def test_index_qsd_detection(self):
        ...
        #   Must agree with the uncached fallback in ClassSubject.got_index_qsd().
    ...
    def test_index_qsd_lowercase_category_symbol(self):
        """Lower-case category symbols are permitted and must still be detected.

        The old SQL pattern only matched '[A-Z]' before the class id, so these
        classes were always reported as having no index QSD.
        """
        target = self.classes[0]
    ...
    def test_program_index_qsd_is_not_a_class_index_qsd(self):
        ...
        for cls in self.classes:
            self.assertFalse(catalog[cls.id].got_index_qsd(),
                             "Program index QSD should not count for %s" % cls.emailcode())
...
class CatalogSortFieldTagTest(ProgramFrameworkTest):
    ...
    def test_descending_legacy_sort_field(self):
        """Descending legacy sort fields should translate too."""
        Tag.setTag('catalog_sort_fields', target=self.program,
