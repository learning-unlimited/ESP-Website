...
class TwoProgramCacheTest(ProgramFrameworkTest):
    def setUp(self, **kwargs):
        ...
        self.program_b = make_program(
            instance_name="2224_Summer",
            instance_label="Summer 2224",
            categories=self.categories,
            admins=self.admins,
            modules=self.settings["modules"],
        )
        self.class_b = make_class(
...
class GetFullClassesPrettyScopeTest(TwoProgramCacheTest):
    ...
    def test_editing_one_program_spares_the_other(self):
        ...
        self.assertIsNone(
            self.cached(self.program),
            "the edited program's entry should be invalidated",
        )
        self.assertIsNotNone(
            self.cached(self.program_b), "an unrelated program's entry should survive"
        )
    ...
    def test_adding_a_teacher_still_invalidates(self):
        ...
        self.assertIsNone(
            self.cached(self.program), "a teacher change must invalidate the entry"
        )
...
class TimeslotsJsonScopeTest(TwoProgramCacheTest):
    ...
    def test_event_change_spares_the_other_program(self):
        ...
        self.assertIsNone(
            self.cached(self.program), "program A's timeslots should be invalidated"
        )
        self.assertIsNotNone(
            self.cached(self.program_b), "program B's timeslots should survive"
        )
...
class TokenBackedScopingTest(TwoProgramCacheTest):
    ...
    def test_getFirstClassTime_is_scoped_by_program(self):
        ...
        self.assertIsNone(
            student.getFirstClassTime(self.program, cache_only=True),
            "program A's entry should be invalidated",
        )
        self.assertIsNotNone(
            student.getFirstClassTime(self.program_b, cache_only=True),
...
class FullClassesEnrolmentDepTest(TwoProgramCacheTest):
    ...
    def test_enrolment_invalidates(self):
        ...
        self.assertIsNone(
            self.cached(self.program), "enrolling a student must invalidate the entry"
        )
    ...
    def test_capacity_change_invalidates(self):
        ...
        self.assertIsNone(
            self.cached(self.program), "a capacity change must invalidate the entry"
        )
...
class TagCacheScopeTest(ProgramFrameworkTest):
    ...
    def test_writing_one_tag_spares_other_tags(self):
        ...
        self.assertIsNone(
            self.cached("cache_scope_a"), "the written tag should be invalidated"
        )
        self.assertIsNotNone(
            self.cached("cache_scope_b"), "an unrelated tag's lookup should survive"
        )
...
class NumStudentsCacheScopeTest(ProgramFrameworkTest):
    ...
    def setUp(self):
        super().setUp(
            num_students=1, num_teachers=2, classes_per_teacher=1, sections_per_class=1
        )
        secs = [
            c.sections.first()
            for c in ClassSubject.objects.filter(parent_program=self.program)
        ]
    ...
    def test_enrolling_in_one_section_spares_the_other(self):
        ...
        self.assertIsNone(
            self.sec_a.num_students(cache_only=True),
            "the enrolled section's count should be invalidated",
        )
        self.assertIsNotNone(
            self.sec_b.num_students(cache_only=True),
...
class WildcardScopingTest(TwoProgramCacheTest):
    ...
    def test_getTimeSlotList_scoped_by_program(self):
        ...
        Event.objects.create(
            program=self.program,
            event_type=self.event_type,
            start=datetime(2222, 7, 12, 9, 0),
            end=datetime(2222, 7, 12, 10, 0),
            name="A slot",
            short_description="a",
            description="slot in A",
        )

        self.assertIsNone(self.program.getTimeSlotList(cache_only=True))
        self.assertIsNotNone(
            self.program_b.getTimeSlotList(cache_only=True),
    ...
    def test_getTimeSlotList_global_event_still_dumps_everything(self):
        ...
        Event.objects.create(
            program=None,
            event_type=self.event_type,
            start=datetime(2222, 7, 13, 9, 0),
            end=datetime(2222, 7, 13, 10, 0),
            name="global",
            short_description="g",
    ...
    def test_getResourceTypes_scoped_by_program(self):
        ...
        self.assertIsNotNone(
            self.program_b.getResourceTypes(cache_only=True),
    ...
    def test_classes_timeslot_scoped_by_program(self):
        ...
        cf(ts_a, self.program)
        cf(ts_b, self.program_b)
        ...
        Event.objects.create(
            program=self.program,
            event_type=self.event_type,
            start=datetime(2222, 7, 14, 9, 0),
            end=datetime(2222, 7, 14, 10, 0),
            name="A slot 2",
            short_description="a2",
            description="another slot in A",
        )

        self.assertIsNone(cf(ts_a, self.program, cache_only=True))
        self.assertIsNotNone(
            cf(ts_b, self.program_b, cache_only=True),
