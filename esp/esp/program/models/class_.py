...
class ClassManager(Manager):
    ...
    def catalog_cached(
        ...
        for c in classes:
            ...
            c.parent_program = (
                p  # So that if we set attributes on one instance of the program,
            )
...
class ClassSection(models.Model):
    ...
    def ajax_autocomplete(cls, data):
        ...
        for value in values:
            value["ajax_str"] = "%ss%s: %s" % (
                value["parent_class__emailcode"],
                value["secnum"],
                value["parent_class__title"],
            )
        return values
                ...
                ans = min(
                    max(
                        self.parent_class.allowable_class_size_ranges.order_by(
                            "-range_max"
                        ).values_list("range_max", flat=True)[0],
                        self.parent_class.class_size_optimal,
                    ),
                    self._get_room_capacity(rooms),
                )
            elif self.parent_class.class_size_optimal and len(rooms) != 0:
                ans = min(
                    self.parent_class.class_size_optimal, self._get_room_capacity(rooms)
                )
    ...
    def emailcode(self):
        return self.parent_class.emailcode() + "s" + unicode(self.index())
    ...
    def assign_room(self, base_room, clear_others=False, allow_partial=False, lock=0):
        ...
        if rooms_to_assign.count() != self.meeting_times.count():
            status = False
            errors.append(
                "Room %s does not exist at the times requested by %s: %s != %s"
                % (
                    base_room.name,
                    self.emailcode(),
                    rooms_to_assign,
        ...
        for i, r in enumerate(rooms_to_assign):
            ...
            if not result:
                ...
                if (
                    occupiers_set.count() > 0
                ):  # We really shouldn't have to test for this, but I guess it's safer not to assume... -ageng 2008-11-02
                    occupiers_str = " by %s during %s" % (
                        (
                            occupiers_set[0].target or occupiers_set[0].target_subj
                        ).emailcode(),
                        r.event.pretty_time(),
                    )
                errors.append(
                    "Room %s is occupied%s." % (base_room.name, occupiers_str)
                )
    ...
    def viable_rooms(self):
        ...
        viable_list = filter(
            lambda x: room_satisfies_times(x, ordered_times), possible_rooms
        )
        ...
        if relevantConstraints:
            ...
            for exp in relevantConstraints:
                if not exp.evaluate(sm, recursive=False):
                    return (
                        "You can't remove this class from your schedule because it would violate the requirement that you %s.  You can go back and correct this."
                        % exp.requirement.label
                    )
        return False
    ...
    def cannotAdd(
        ...
        if relevantConstraints:
            ...
            for exp in relevantConstraints:
                if not exp.evaluate(sm, recursive=autocorrect_constraints):
                    return (
                        "Adding <i>%s</i> to your schedule requires that you %s.  You can go back and correct this."
                        % (self.title(), exp.requirement.label)
                    )
        ...
        if scrmi.use_priority:
            ...
            if priority > scrmi.priority_limit:
                return "You are only allowed to select up to %s top classes" % (
                    scrmi.priority_limit
                )
    ...
    def cannotSchedule(self, meeting_times, ignore_classes=True):
        ...
        for t in self.teachers:
            available = t.getAvailableTimes(
                self.parent_program, ignore_classes=ignore_classes
            )
            for e in meeting_times:
                if e not in available:
                    return (
                        "The teacher %s has not indicated availability during %s."
                        % (t.name(), e.pretty_time())
                    )
            conflicts = self.conflicts(t, meeting_times)
            if conflicts:
                return "The teacher %s is teaching %s during %s." % (
                    t.name(),
                    conflicts[0].emailcode(),
                    conflicts[1].pretty_time(),
                )
    ...
    def cancel(
        ...
        context["full_group_name"] = Tag.getTag("full_group_name") or "%s %s" % (
        ...
        email_title = "Class Cancellation at %s - Section %s" % (
            self.parent_program.niceName(),
            self.emailcode(),
        )
        ssi_email_title = "Class Cancellation at %s - Class %s" % (
            self.parent_program.niceName(),
            self.parent_class.emailcode(),
        )
        ...
        if email_students:
            ...
            for student in students_to_email:
                ...
                from_email = ESPUser.email_sendto_address(
                    ...
                    "%s at %s"
                    % (self.parent_program.program_type, settings.INSTITUTION_NAME),
                )
    ...
    def unpreregister_student(self, user, prereg_verb=None):
        ...
        if prereg_verb:
            qs = StudentRegistration.valid_objects(now).filter(
                relationship__name=prereg_verb, section=self, user=user
            )
            qs.update(end_date=now)
        else:
        ...
        list_names = [
            "%s-%s" % (self.emailcode(), "students"),
            "%s-%s" % (self.parent_class.emailcode(), "students"),
        ]
...
class ClassSubject(models.Model, CustomFormsLinkModel):
    ...
    def get_section(self, timeslot=None):
        ...
        if hasattr(self, "_sections"):
            ...
            if (
                did_search
            ):  # If we did successfully search all sections, but found none in this timeslot
                return None
    ...
    def pretty_teachers(self):
        ...
        return ", ".join(
            ["%s %s" % (u.first_name, u.last_name) for u in self.get_teachers()]
        )

    def pretty_moderators(self):
        """Return a prettified string listing of the class's moderators"""
        return ", ".join(
            ["%s %s" % (u.first_name, u.last_name) for u in self.moderators()]
        )
...
-esp.mit.edu Autogenerated Message""" % (
            self.title,
            self.parent_program.getUrlBase(),
            self.id,
        )
        if user is None:
            user = AnonymousUser()
    ...
    def getArchiveClass(self):
        ...
        teacher_strs = [
            "%s %s" % (t.first_name, t.last_name) for t in self.get_teachers()
        ]
    ...
    def save(self, *args, **kwargs):
        ...
        if self.status < UNREVIEWED:  # ie, all rejected or cancelled classes.
            ...
            for t in teachers:
                if (
                    t.getTaughtClasses(self.parent_program)
