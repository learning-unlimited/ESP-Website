...
class Program(models.Model, CustomFormsLinkModel):
    ...
    def _students_in_program_in_grades(self, grades):
        ...
        return len(
            [
                student
                for student in self._classreg_students()
    ...
    getTimeSlotList.depend_on_row(
        "cal.Event", lambda e: {"self": e.program} if e.program_id else {}
    )
    ...
    getResourceTypes.depend_on_row(
    ...
    getModules_cached.depend_on_m2m(
    ...
    getShirtInfo.depend_on_row(
        "program.ClassSection", lambda sec: {"self": sec.parent_class.parent_program}
    )
