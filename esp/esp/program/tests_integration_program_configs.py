...
class LotteryPipelineConfigMatrixTest(CacheFlushTestCase):
    ...
    def _add_priority_and_interest(
        self, pf, priority_rt, interested_rt, grade_by_index=None
    ):
        ...
        sections = list(
            pf.program.sections().filter(meeting_times__isnull=False).distinct()
        )
        ...
        for i, student in enumerate(pf.students):
            ...
            si = StudentInfo(
                user=student, graduation_year=ESPUser.YOGFromGrade(grade, schoolyear)
            )
            si.save()
            RegistrationProfile(
                user=student, student_info=si, most_recent_profile=True
            ).save()
    ...
    def _assert_no_overcapacity(self, pf):
        ...
        for section in pf.program.sections():
            enrolled = (
                ...
                .filter(section=section, relationship__name="Enrolled")
                .count()
            )
    ...
    def _assert_no_timeslot_conflicts(self, pf):
        ...
        for student in pf.students:
            enrolled_sections = list(
                StudentRegistration.valid_objects()
                .filter(user=student, relationship__name="Enrolled")
            ...
            for section_id in enrolled_sections:
                for ts_id in (
                    pf.program.sections()
                    .get(id=section_id)
                    .meeting_times.values_list("id", flat=True)
                ):
                    self.assertNotIn(
                        ts_id,
                        seen_timeslots,
...
class FCFSPipelineIntegrationTest(ProgramFrameworkTest):
    ...
    def _is_enrolled(self, student):
        return (
            StudentRegistration.valid_objects()
            .filter(user=student, section=self.section, relationship__name="Enrolled")
            .exists()
        )
    ...
    def test_registration_succeeds_at_grade_boundaries(self):
        ...
        for student in (low, high):
            ...
            self.assertTrue(
                self._is_enrolled(student),
                f"Grade-boundary student {student.id} was not enrolled",
            )
    ...
    def test_registration_blocked_outside_grade_range(self):
        ...
        for student in (below, above):
            ...
            self.assertEqual(
                payload.get("status"),
                200,
                f"Expected an error payload for {student.id}",
            )
    ...
    def test_full_section_rejects_then_frees_seat_for_next_student(self):
        ...
        self.assertEqual(
            payload.get("error"),
            self.program.studentclassregmoduleinfo.temporarily_full_text,
        )
...
class WaitlistToggleIntegrationTest(ProgramFrameworkTest):
    ...
    def setUp(self):
        ...
        super().setUp(
            ...
            room_capacity=30,
            modules=modules,
            program_instance_name="3331_WaitlistIntegration",
        )
        self.add_user_profiles()
        self.schedule_randomly()
        self.section = self.program.sections()[0]
    ...
    def test_full_program_waitlists_the_next_student(self):
        ...
        self.assertTrue(
            self.client.login(username=classreg_student.username, password="password")
        )
        ...
        self.assertTrue(
            self.client.login(username=blocked_student.username, password="password")
        )
        resp = self.client.post(
            "/learn/%s/waitlist_subscribe" % self.program.getUrlBase(), follow=True
        )
        self.assertEqual(resp.status_code, 200)
        ...
        self.assertTrue(
            Record.objects.filter(
                user=blocked_student, program=self.program, event__name="waitlist"
            ).exists()
        )
        self.assertIn(
            blocked_student,
            self._module().students(QObject=False)["waitlisted_students"],
        )
