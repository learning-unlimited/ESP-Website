...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        jobs = (
            PrintableJob.objects.filter(updated_at__lt=cutoff)
            .exclude(file=None)
            .exclude(file="")
        )
        count = 0
        for job in jobs:
            if job.file:
                self.stdout.write(
                    f"Deleting file for job {job.id} (updated at {job.updated_at})"
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully cleaned up files and cleared FileField for {count} jobs."
            )
        )
