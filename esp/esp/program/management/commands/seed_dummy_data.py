...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            for program in programs:
                ...
                teachers, sections = self._create_teachers(
                    program, timeslots, classrooms
                )
    ...
    def flush_seeded_data(self):
        ...
        qsd_filter |= (
            Q(url="learn/index") | Q(url="teach/index") | Q(url="volunteer/index")
        )
    ...
    def _seed_site(self):
        site, _ = Site.objects.get_or_create(
            id=1, defaults=dict(domain="devsite.learningu.org", name="LU Dev Site")
        )
    ...
    def _seed_lookup_data(self):
        ...
        for symbol, category, seq in [
            ...
            ClassCategories.objects.get_or_create(
                symbol=symbol, defaults=dict(category=category, seq=seq)
            )
        ...
        for handler, admin_title, module_type, seq, required in [
            ...
            ProgramModule.objects.get_or_create(
                ...
                defaults=dict(
                    admin_title=admin_title, seq=seq, required=required, choosable=1
                ),
            )
        ...
        for _, mod_name, _ in pkgutil.iter_modules(handlers_pkg.__path__):
            ...
            for _name, cls in inspect.getmembers(mod, inspect.isclass):
                ...
                try:
                    is_handler = (
                        issubclass(cls, ProgramModuleObj)
                        and cls is not ProgramModuleObj
                    )
                except TypeError:
                    continue
                if not is_handler:
    ...
    def _seed_navbar(self):
        cat, _ = NavBarCategory.objects.get_or_create(
            name="Dev Navbar",
            defaults=dict(
        ...
        for rank, text, link in [
            ...
            NavBarEntry.objects.get_or_create(
                category=cat,
                text=text,
    ...
    def _create_school(self, admin):
        school, created = K12School.objects.get_or_create(
            name="Dev High School", defaults=dict(school_type="Public", grades="7-12")
        )
        if created:
            contact, _ = ContactInfo.objects.get_or_create(
                ...
                last_name="Office",
                defaults=dict(
                    e_mail="office@devhighschool.edu",
                    phone_day="555-0100",
                    address_country="US",
            ...
            if created:
                ...
                ClassRegModuleInfo.objects.get_or_create(
                    ...
                    defaults=dict(
                        allow_coteach=True,
                        set_prereqs=True,
                        class_max_duration=120,
                        class_min_cap=5,
                        class_max_size=40,
                        class_size_step=5,
                        num_teacher_questions=1,
    ...
    def _create_teachers(self, program, timeslots, classrooms):
        ...
        for i in range(1, 6):
            ...
            ti, _ = TeacherInfo.objects.get_or_create(
                user=user,
                defaults=dict(
                    ...
                    major=random.choice(
                        ["CS", "Math", "Physics", "Biology", "English"]
                    ),
            ...
            TeacherBio.objects.get_or_create(
                user=user,
                program=program,
                defaults=dict(
            ...
            for _ in range(random.randint(1, 2)):
                ...
                subj, sc = ClassSubject.objects.get_or_create(
                    ...
                    defaults=dict(
                        category=cat,
                        class_info=self.fake.text(max_nb_chars=500),
    ...
    def _create_students(self, program, sections, school):
        enrolled_type = RegistrationType.objects.get(
        ...
        for i in range(1, 16):
            ...
            dob = datetime(
                current_year - grade - 6, random.randint(1, 12), random.randint(1, 28)
            )

            si, _ = StudentInfo.objects.get_or_create(
                user=user,
                defaults=dict(
            ...
            gi, _ = GuardianInfo.objects.get_or_create(
                user=user,
                defaults=dict(
                    num_kids=random.randint(1, 3),
                    year_finished=random.randint(1980, 2000),
                ),
            )

            self._make_reg_profile(
                user,
                program,
                contact_user=cu,
                contact_guardian=cg,
                contact_emergency=ce,
                student_info=si,
                guardian_info=gi,
            )

            for sec in random.sample(
                sections, random.randint(2, min(3, len(sections)))
            ):
            ...
            Record.objects.get_or_create(
                user=user, program=program, event=confirmed_rtype
            )
    ...
    def _create_survey(self, program, teachers):
        survey, survey_created = Survey.objects.get_or_create(
            name=f"Dev Survey - {program.name}",
            program=program,
        ...
        q1, _ = Question.objects.get_or_create(
            survey=survey, name="Overall rating", defaults=dict(seq=1, question_type=rt)
        )
        q2, _ = Question.objects.get_or_create(
            survey=survey, name="Feedback", defaults=dict(seq=2, question_type=ot)
        )
    ...
    def _create_global_qsd(self, admin, first_program_name):
        ...
        for url, name, title, content in self._global_qsd_pages(first_program_name):
            QuasiStaticData.objects.get_or_create(
                url=url,
                name=name,
                defaults=dict(
                    title=title,
    ...
    def _create_qsd(self, program, admin):
        nav_cat = self._get_nav_cat()
        for url, name, title, content in self._program_qsd_pages(
    ...
    def _program_qsd_pages(self, pn, pu):
        ...
        return [
            ...
            (
                f"teach/{pu}/teacher_class_import_description",
                "teacher_class_import_description",
                f"{pn} \u2014 Import a Class",
            ...
            # Volunteer flow
            (
                f"volunteer/{pu}/volunteer_signup",
                "volunteer_signup",
                f"Volunteer at {pn}",
                f"## Volunteer at {pn}\n\n"
            ...
            (
                f"volunteer/{pu}/volunteer_complete",
                "volunteer_complete",
            ...
            (
                f"volunteer/{pu}/volunteer_cancelled",
                "volunteer_cancelled",
    ...
    def _make_reg_profile(
        self,
        user,
        program,
        contact_user=None,
        contact_guardian=None,
