...
class MeetingTimesCacheDepTest(ProgramFrameworkTest):
    ...
    def setUp(self):
        super().setUp(
            num_students=1, num_teachers=1, classes_per_teacher=1, sections_per_class=1
        )
    ...
    def test_getFirstClassTime_follows_rescheduling(self):
        ...
        self.assertEqual(
            self.student.getFirstClassTime(self.program),
            self.timeslots[0],
    ...
    def test_getFirstClassTime_follows_added_time(self):
        ...
        self.enroll()
        self.assertEqual(
            self.student.getFirstClassTime(self.program), self.timeslots[2]
        )
...
class ProgramModulesCacheDepTest(ProgramFrameworkTest):
    ...
    def test_getModules_cached_follows_module_changes(self):
        ...
        self.assertEqual(
            len(self.program.getModules_cached()),
            before - 1,
...
class AllowableClassSizeRangesCacheDepTest(ProgramFrameworkTest):
    ...
    def setUp(self):
        super().setUp(
            num_students=0, num_teachers=1, classes_per_teacher=1, sections_per_class=1
        )
    ...
    def test_capacity_cache_is_dropped_when_size_ranges_change(self):
        ...
        self.assertIsNotNone(
            self.section._get_capacity(cache_only=True),
            "capacity should be cached after first access",
        )

        rng = ClassSizeRange.objects.create(
            range_min=1, range_max=99, program=self.program
        )
...
class ShirtInfoAndModNumsCacheDepTest(ProgramFrameworkTest):
    ...
    def test_getShirtInfo_follows_teacher_changes(self):
        ...
        self.assertIsNone(
            self.program.getShirtInfo(cache_only=True),
    ...
    def test_mod_nums_follows_rescheduling(self):
        ...
        self.assertIsNone(
            JSONDataModule.mod_nums(self.program, cache_only=True),
...
class ModeratorCacheDepTest(ProgramFrameworkTest):
    ...
    def test_moderating_sections_follows_rescheduling(self):
        ...
        self.assertIsNotNone(
            self.moderator.getModeratingSectionsFromProgram(
                self.program, cache_only=True
            )
        )
    ...
    def test_moderators_json_follows_category_changes(self):
        ...
        rec = ModeratorRecord.objects.create(
            user=self.moderator, program=self.program, will_moderate=True, num_slots=1
        )
        JSONDataModule.moderators.method.cached_function(self.program)
        self.assertIsNotNone(
            JSONDataModule.moderators.method.cached_function(
                self.program, cache_only=True
            )
        )
...
class ModeratorsAvailabilityScopeTest(ProgramFrameworkTest):
    ...
    def setUp(self):
        ...
        self.program_b = make_program(
            instance_name="2225_Summer",
            instance_label="Summer 2225",
    ...
    def test_availability_change_spares_the_other_program(self):
        ...
        self.assertIsNone(
            cf(self.program, cache_only=True),
            "program A's moderators payload should be invalidated",
        )
        self.assertIsNotNone(
            cf(self.program_b, cache_only=True),
