...
class StanfordTeacherQuestionsForm(FormWithRequiredCss):
    schedule_acknowledgement = forms.BooleanField(label='Acknowledgement', help_text='Splash will be on Nov. 8-9!  Please acknowledge that you are aware of this and commit to teaching at one of the times you have specified for your availability.')
    # walk_in = forms.ChoiceField(label="walk_in", help_text='Walk-in Seminars are courses geared towards students with gaps in their schedules or who are faced with last-minute class cancellations. While similar in spirit to a normal Splash class, students do not sign up in advance for Walk-in Seminars -- rather, they are free to choose to come and go as they would like. See http://www.stanfordesp.org/teach/walkins.html for more information on walk-ins.', choices=[('no', 'No'), ('yes', 'Yes')])
