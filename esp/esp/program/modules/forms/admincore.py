...
class VisibleRegistrationTypeForm(forms.Form):
    display_names = forms.MultipleChoiceField(
        choices=[],
        required=False,
        label="",
        help_text=mark_safe(
            "<br />Select the Registration Types that should be displayed on a student's schedule on the studentreg page. To select an entry, hold Ctrl (on Windows or Linux) or Meta (on Mac), and then press it with your mouse."
        ),
        widget=forms.SelectMultiple(attrs={"style": "height:150px; background:white;"}),
    )
...
class LunchConstraintsForm(forms.Form):
    def __init__(self, program, *args, **kwargs):
        ...
        self.fields["timeslots"].choices = [
            (ts.id, ts.short_description) for ts in self.program.getTimeSlots()
        ]
        self.load_data()
    ...
    def load_data(self):
        ...
        self.fields["timeslots"].initial = list(
            lunch_timeslots.values_list("id", flat=True)
        )
        ...
        if sched_constraints.exists():
            ...
            if (
                BooleanToken.objects.filter(exp__condition_constraint__program=2)
                .exclude(text="1")
                .exists()
            ):
    ...
    def save_data(self):
        timeslots = Event.objects.filter(
            id__in=self.cleaned_data["timeslots"]
        ).order_by("start")
        cg = LunchConstraintGenerator(
            self.program,
            timeslots,
            generate_constraints=(self.cleaned_data["generate_constraints"] is True),
            autocorrect=(self.cleaned_data["autocorrect"] is True),
            include_conditions=(self.cleaned_data["include_conditions"] is True),
        )
    ...
    generate_constraints = forms.BooleanField(
        initial=False,
        required=False,
        help_text="Check this box to generate lunch scheduling constraints. If unchecked, only lunch sections will be generated, and the other two check boxes will have no effect.",
    )
    autocorrect = forms.BooleanField(
        initial=False,
        required=False,
        help_text="Check this box to attempt automatically adding lunch to a student's schedule so that they are less likely to violate the schedule constraint.",
    )
    include_conditions = forms.BooleanField(
        initial=False,
        required=False,
        help_text="Check this box to allow students to schedule classes through lunch if they do not have morning or afternoon classes.",
    )
...
ProgramSettingsForm.base_fields["director_email"].widget = forms.EmailInput(
    attrs={"pattern": rf"(^.+@{_ESCAPED_SITE_DOMAIN}$)|(^.+@(\w+\.)*learningu\.org$)"}
)
...
class TeacherRegSettingsForm(BetterModelForm):
    ...
    progress_mode = forms.IntegerField(
        min_value=0,
        max_value=2,
        help_text=ClassRegModuleInfo._meta.get_field("progress_mode").help_text,
    )
    ...
    class Meta:
        fieldsets = [
            (
                "Teacher Settings",
                {"fields": ["allow_coteach", "set_prereqs", "num_teacher_questions"]},
            ),
            (
                "Class Duration and Size Options",
                {
                    "fields": [
                        "class_max_duration",
                        "class_min_cap",
            ...
            (
                "Other Options",
                {
                    "fields": [
                        ...
                        "use_optimal_class_size_range",
                        "use_allowable_class_size_ranges",
                        "open_class_registration",
                    ]
                },
            ),
...
class StudentRegSettingsForm(BetterModelForm):
    ...
    class_cap_multiplier = forms.DecimalField(
        min_value=0,
        max_digits=3,
        decimal_places=2,
        help_text=StudentClassRegModuleInfo._meta.get_field(
            "class_cap_multiplier"
        ).help_text,
    )
    class_cap_offset = forms.IntegerField(
        min_value=0,
        help_text=StudentClassRegModuleInfo._meta.get_field(
            "class_cap_offset"
        ).help_text,
    )
    priority_limit = forms.IntegerField(
        min_value=0,
        help_text=StudentClassRegModuleInfo._meta.get_field("priority_limit").help_text,
    )
    progress_mode = forms.IntegerField(
        min_value=0,
        max_value=2,
        help_text=StudentClassRegModuleInfo._meta.get_field("progress_mode").help_text,
    )
    ...
    class Meta:
        fieldsets = [
            (
                "Capacity Settings",
                {
                    "fields": [
                        ...
                        "class_cap_multiplier",
                        "class_cap_offset",
                        "apply_multiplier_to_room_cap",
                    ]
                },
            ),
            (
                "Priority Registration Settings",
                {"fields": ["priority_limit"]},
            ),  # use_priority is not included here to prevent confusion; to my knowledge, only HSSP uses this setting - WG
            (
                "Enrollment Settings",
                {
                    "fields": [
            ...
            ),  # use_grade_range_exceptions is excluded until there is an interface for it - WG 5/25/23
            (
                "Button Settings",
                {
                    "fields": [
            ...
            (
                "Visual Options",
                {"fields": ["progress_mode", "force_show_required_modules"]},
            ),
        ]  # Here you can also add description for each fieldset.
...
def get_template_source(template_list):
    ...
    if (
        template.origin.name == "(template override)"
    ):  # source is from a template override
        return (
            TemplateOverride.objects.get(name=template.template.name)
            .content.replace("\r\n", "\n")
            .strip()
        )  # Use unix line endings and strip whitespace just in case
...
class ReceiptsForm(BetterForm):
    confirm = forms.CharField(
        widget=forms.Textarea(attrs={"class": "fullwidth"}),
        help_text=mark_safe(
            "This text is <b>shown on the website</b> when a student clicks the 'confirm registration' button (HTML is supported).\
    ...
    confirmemail = forms.CharField(
        widget=forms.Textarea(attrs={"class": "fullwidth"}),
        help_text=mark_safe(
            "This text is <b>sent via email</b> when a student clicks the 'confirm registration' button.\
                                                     If no text is supplied, the default text will be used. The text is then followed by the student's information,\
    ...
    def __init__(self, *args, **kwargs):
        ...
        for action in ["confirm", "confirmemail", "cancel"]:
            ...
            if receipts.count() > 0:
                ...
                receipt_text = get_template_source(
                    [
                ...
                receipt_text = get_template_source(
                    [
                        "program/confemails/%s_confemail_pretext.html"
                        % (self.program.id),
    ...
    def save(self):
        for action in ["confirm", "confirmemail", "cancel"]:
            receipts = DBReceipt.objects.filter(program=self.program, action=action)
            cleaned_text = (
                self.cleaned_data[action].replace("\r\n", "\n").strip()
            )  # Use unix line endings and strip whitespace just in case
            if cleaned_text == "":
                receipts.delete()
            else:
                if action == "confirm":
                    default_text = get_template_source(
                        [
                            "program/receipts/%s_custom_pretext.html"
                            % (self.program.id),
                            "program/receipts/default_pretext.html",
                        ]
                    )
                ...
                if cleaned_text == default_text:
                    ...
                    receipt, created = DBReceipt.objects.get_or_create(
                        program=self.program, action=action
                    )
    ...
    class Meta:
        fieldsets = [
            (
                ...
                {"fields": ["confirm", "confirmemail", "cancel"]},
            )
        ]
...
class ProgramTagSettingsForm(BetterForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        classreg_fields = [
            field.name
            for field in TeacherClassRegForm(
                self.program.classregmoduleinfo
            ).visible_fields()
        ]
            ...
            if tag_info.get("is_setting", False):
                ...
                if key == "teacherreg_hide_fields":
                    self.fields[key] = forms.MultipleChoiceField(
                        choices=[
                            (field[0], field[1].label if field[1].label else field[0])
                            for field in TeacherClassRegForm.declared_fields.items()
                ...
                if key in ["student_self_checkin"]:
                    ...
                    self.fields[key].help_text = template.render(
                ...
                set_val = (
                    Tag.getBooleanTag(key, program=self.program)
                    if tag_info.get("is_boolean", False)
                    else Tag.getProgramTag(key, program=self.program)
                )
                ...
                if (
                    key.startswith("teacherreg_label")
                    and key.partition("teacherreg_label_")[2] not in classreg_fields
                ):
                    self.fields[key].widget = forms.HiddenInput()
                elif (
                    key.startswith("teacherreg_help_text")
                    and key.partition("teacherreg_help_text_")[2] not in classreg_fields
                ):
                    self.fields[key].widget = forms.HiddenInput()
    ...
    def save(self):
        ...
        for key in all_program_tags:
            ...
            if tag_info.get("is_setting", False):
                ...
                global_val = (
                    Tag.getBooleanTag(key, default=tag_info.get("default"))
                    if tag_info.get("is_boolean", False)
                    else Tag.getProgramTag(key, default=tag_info.get("default"))
                )
        ...
        fieldsets = [
            (
                cat,
                {
                    "fields": [
                        key
                        for key in sorted(all_program_tags.keys())
                        if all_program_tags[key].get("category") == cat
                    ],
