...
class OnSiteRegForm(forms.Form):
    first_name = forms.CharField(
        max_length=30, widget=forms.TextInput({"size": 20, "class": "required"})
    )
    last_name = forms.CharField(
        max_length=30, widget=forms.TextInput({"size": 30, "class": "required"})
    )
    email = forms.EmailField(
        max_length=75, widget=forms.TextInput({"size": 20, "class": "required"})
    )
    ...
    k12school = AjaxForeignKeyNewformField(
        ...
        required=False,
        label="School",
        help_text="(Type the school's name and click on a match if it pops up.)",
    )

    grade = forms.ChoiceField(
        choices=list(zip(list(range(7, 13)), list(range(7, 13)))),
        widget=forms.Select({"class": "required"}),
    )
...
class OnsiteBarcodeCheckinForm(forms.Form):
    ...
    attended = forms.BooleanField(
        label="Attending? (i.e. here right now?)", required=False, initial=True
    )
...
class TeacherCheckinForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.base_fields["when"].initial = datetime.datetime(
