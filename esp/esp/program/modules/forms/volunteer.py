...
class VolunteerRequestForm(forms.Form):
    ...
    start_time = forms.DateTimeField(
        help_text="Enter a time in the form MM/DD/YYYY hh:mm.", widget=DateTimeWidget
    )
    ...
    description = forms.CharField(
        max_length=128,
        help_text="What would volunteers do during this timeslot?  (Examples: Registration, Security)",
    )
    ...
    def __init__(self, *args, **kwargs):
        if "program" in kwargs:
            ...
            raise KeyError(
                "Need to supply program as named argument to VolunteerRequestForm"
            )
...
class VolunteerOfferForm(forms.Form):
    ...
    requests = forms.MultipleChoiceField(
        choices=(),
        label="Timeslots",
        help_text="Sign up for one or more shifts; remember to avoid conflicts with your classes if you're teaching!",
        widget=forms.CheckboxSelectMultiple,
        required=False,
    )
    has_previous_requests = forms.BooleanField(
        widget=forms.HiddenInput, required=False, initial=False
    )
    clear_requests = forms.BooleanField(
        widget=forms.HiddenInput, required=False, initial=False
    )

    comments = forms.CharField(
        widget=forms.Textarea(attrs={"rows": 3, "cols": 60}),
        help_text="Any comments or special circumstances you would like us to know about?",
        required=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["requests"].choices = [
            (
                v.id,
                f"{v.timeslot.pretty_time()}: {v.timeslot.description} ({positive_or_no(v.num_volunteers - v.num_offers())} more needed)",
            )
            for v in vrs
        ]
        self.fields["shirt_size"].choices = [("", "")] + [
            (x.strip(), x.strip())
            for x in Tag.getTag("volunteer_shirt_sizes").split(",")
        ]
    ...
    def load(self, user):
        ...
        if previous_offers.exists():
            ...
            self.fields["requests"].initial = list(
                previous_offers.values_list("request", flat=True)
            )
...
class VolunteerImportForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        qs = Program.objects.annotate(vr_count=Count("volunteerrequest")).filter(
            vr_count__gt=0
        )
