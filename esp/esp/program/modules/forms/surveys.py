...
class QuestionForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.base_fields["survey"].choices = (
            (str(survey.id), survey.name + " (" + survey.category + ")")
...
class CSVQuestionImportForm(forms.Form):
    csv_file = forms.FileField(
        label="CSV File", help_text="Upload a CSV file with survey questions."
    )
...
def parse_csv(csv_file):
    ...
    try:
        ...
        if first_bytes.startswith(
            b"PK\x03\x04"
        ):  # ZIP-based formats (xlsx, numbers, etc.)
            errors.append(
                {
                    "row_number": 0,
    ...
    if reader.fieldnames is None:
        errors.append(
            {"row_number": 0, "message": "CSV file is empty or has no header row."}
        )
        return parsed_rows, errors
        ...
        errors.append(
            {
                "row_number": 0,
                "message": (
                    "Multiple QuestionType entries share the same name (case-insensitive): %s. "
                    "Please resolve these duplicates before importing."
                )
                % ", ".join(sorted(duplicate_qtype_names)),
            }
        )
        return parsed_rows, errors
    ...
    for row_number, row in enumerate(reader, start=2):
        ...
        if seq_str:
            try:
                ...
                row_errors.append(
                    'Invalid seq value "%s". Must be an integer.' % seq_str
                )
