...
exclude_line_items = [
    ...
    "Financial aid grant",
    "Student payment",
    "Donation to Learning Unlimited",
]
...
class LineItemForm(forms.ModelForm):
    text = forms.CharField(
        ...
        validators=[
            RegexValidator(
                regex="^(" + "|".join(exclude_line_items) + ")$",
                message="That line item name is reserved for internal operations. Please choose another name.",
                inverse_match=True,
            )
        ],
    )
...
class LineItemImportForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        progs = (
            LineItemType.objects.exclude(text__in=exclude_line_items)
