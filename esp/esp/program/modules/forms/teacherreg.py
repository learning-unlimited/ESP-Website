...
class TeacherClassRegForm(FormWithRequiredCss):
    ...
    class_size_max = forms.ChoiceField( label='Maximum Number of Students',
                                        ...
                                        help_text='The above class-size and grade-range values are absolute, not the "optimal" or "recommended" amounts. We will not allow any more students than you specify, nor allow any students in grades outside the range that you specify. If you would like a different size, please specify in \'Message for Directors.\'' )
    class_size_optimal = forms.IntegerField( label='Optimal Number of Students', help_text="This is the number of students you would have in your class in the most ideal situation.  This number is not a hard limit, but we'll do our best to honor this." )
    ...
    purchase_requests = forms.CharField( label='Planned Purchases', widget=forms.Textarea(), required=False,
                                         help_text='We give all teachers a $30 budget per class section or hour of walk-in class; we can reimburse you if you turn in an itemized receipt and reimbursement form before the end of the program.  If you would like to exceed this budget, please type a budget proposal here stating what you would like to buy, how much it will cost, and why you would like to purchase it.' )
