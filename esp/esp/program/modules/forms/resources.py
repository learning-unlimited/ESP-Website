...
class TimeslotForm(forms.Form):
    ...
    description = forms.CharField(
        ...
        help_text='Include the exact times here (e.g. "First class period: Sat 9:05 - 9:55 AM")',
    )
    start = forms.DateTimeField(
        label="Start Time",
        help_text=mark_safe(
            "Format: MM/DD/YYYY HH:MM:SS <br />Example: 10/14/2007 14:00:00"
        ),
    ...
    openclass = forms.BooleanField(
        required=False,
        label="Open Class Timeslot",
        help_text="Check this if the timeslot should be used for open classes only. If in doubt, don't check this.",
    )
    compulsory = forms.BooleanField(
        required=False,
        label="Compulsory Timeslot",
        help_text="Check this if the timeslot should be compulsory for all students.",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["group"].help_text = mark_safe(
            """All timeslots with this value will always be included in the same timeslot group.
                                                    ...
                                                    <a href = '%stags/teach' target='_blank'>'Availability group tolerance' tag</a>."""
            % program.get_manage_url()
        )
    ...
    def load_timeslot(self, slot):
        ...
        self.fields["minutes"].initial = int(
            length // 60 - 60 * self.fields["hours"].initial
        )
        self.fields["group"].initial = slot.group
        self.fields["openclass"].initial = slot.event_type == EventType.get_from_desc(
            "Open Class Time Block"
        )
        self.fields["compulsory"].initial = slot.event_type == EventType.get_from_desc(
            "Compulsory"
        )
    ...
    def clean(self):
        ...
        if cleaned_data.get("openclass") and cleaned_data.get("compulsory"):
            raise forms.ValidationError(
                "A timeslot cannot be both 'Open Class' and 'Compulsory' at the same time."
            )
        return cleaned_data
    ...
    def save_timeslot(self, program, slot):
        ...
        slot.end = slot.start + timedelta(
            hours=self.cleaned_data["hours"], minutes=self.cleaned_data["minutes"]
        )
...
class ResourceTypeForm(forms.Form):
    ...
    priority = forms.IntegerField(
        required=False,
        help_text="Assign this a unique number in relation to the priority of other resource types",
    )
    only_one = forms.BooleanField(
        label="Only one?",
        required=False,
        help_text="Limit teachers to selecting only one of the options?",
    )
    is_global = forms.BooleanField(
        label="Global?",
        required=False,
        help_text="Should this resource be associated with all programs?",
    )
    hidden = forms.BooleanField(
        label="Hidden?",
        required=False,
        help_text="Should this resource type be hidden during teacher registration?",
    )
    ...
    def save_restype(self, program, res_type, choices=None):
        ...
        if choices and [_f for _f in choices if _f]:
            ...
            """ This is already set by default when making a new resource type,
                ...
                setting choices. """
            res_type.attributes_dumped = ResourceType._meta.get_field(
                "attributes_dumped"
            ).default
...
def setup_furnishings(restype_list):
    ...
    return (
        (str(r.id), r.name + (" (Hidden)" if r.hidden else "")) for r in restype_list
    )
...
class EquipmentForm(forms.Form):
    ...
    num_items = forms.IntegerField(
        label="Number of unique items", validators=[MinValueValidator(1)]
    )
    ...
    def __init__(self, *args, **kwargs):
        if isinstance(args[0], Program):
            self.base_fields["resource_type"].choices = tuple(
                [("", "(type)")] + list(setup_furnishings(args[0].getResourceTypes()))
            )
    ...
    def load_equipment(self, program, resource):
        ...
        self.fields["times_available"].initial = [
            mt.id for mt in resource.matching_times()
        ]
    ...
    def save_equipment(self, program):
        initial_resources = list(
            Resource.objects.filter(
                name=self.cleaned_data["name"], event__program=program
            )
        )
        new_timeslots = [
            Event.objects.get(id=int(id_str))
            for id_str in self.cleaned_data["times_available"]
        ]
        new_restype = ResourceType.objects.get(
            id=int(self.cleaned_data["resource_type"])
        )
...
class ClassroomForm(forms.Form):
    ...
    def save_classroom(self, program, furnishings):
        ...
        timeslots = Event.objects.filter(
            id__in=[int(id_str) for id_str in self.cleaned_data["times_available"]]
        )
        ...
        for t in new_timeslots:
            ...
            for f in furnishings:
                ...
                new_resource.name = (
                    res_type.name + " for " + self.cleaned_data["room_number"]
                )
        ...
        for rm in rooms_to_delete:
            ...
            for ra in ra_room:
                if ra.resource.event in new_timeslots:
                    ra.resource = timeslots[
                        new_timeslots.index(ra.resource.event)
                    ].new_room
                    ra.save()
        ...
        for room in rooms_to_keep:
            ...
            for f in initial_furnishings[room]:
                if {
                    "furnishing": str(f.res_type.id),
                    "choice": f.attribute_value,
                } not in furnishings:
                    f.delete()
    ...
    class FurnishingForm(forms.Form):
        ...
        choice = forms.CharField(
            ...
            widget=forms.TextInput(
                attrs={"placeholder": "(option)", "style": "margin-left: 8px"}
            ),
        )

        def __init__(self, *args, **kwargs):
            furnishings = setup_furnishings(prog.getResourceTypes())
            self.base_fields["furnishing"].choices = tuple(
                [("", "(furnishing)")] + list(furnishings)
            )
            super().__init__(*args, **kwargs)
...
class ClassroomImportForm(forms.Form):
    ...
    complete_availability = forms.BooleanField(
        required=False,
        help_text="Check this box if you would like the new classrooms to be available at all times during the program, rather than attempting to replicate their availability from the previous program.",
    )
    import_furnishings = forms.BooleanField(
        required=False,
        help_text="Check this box if you would like the new classrooms to have the same furnishings as they did for the previous program.",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        progs = (
            Resource.objects.filter(res_type=ResourceType.get_or_create("Classroom"))
            .values_list("event__program", flat=True)
            .distinct()
        )
...
class TimeslotImportForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        qs = Program.objects.annotate(
            vr_count=Count(
                Case(
                    When(event__event_type__description="Class Time Block", then=1),
                    default=None,
...
class ResTypeImportForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        cur_prog = kwargs.pop("cur_prog", None)
        super().__init__(*args, **kwargs)
        qs = Program.objects.annotate(rt_count=Count("resourcetype")).filter(
            rt_count__gt=0
        )
        if cur_prog is not None:
            qs = qs.exclude(id=cur_prog.id)
...
class EquipmentImportForm(forms.Form):
    ...
    complete_availability = forms.BooleanField(
        required=False,
        help_text="Check this box if you would like the new floating resources to be available at all times during the program, rather than attempting to replicate their availability from the previous program.",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        progs = (
            Resource.objects.filter(is_unique=True)
            .exclude(res_type=ResourceType.get_or_create("Classroom"))
            .values_list("event__program", flat=True)
            .distinct()
        )
        qs = Program.objects.filter(id__in=progs)
