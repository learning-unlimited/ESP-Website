...
class ProgramManagerTestHelper:
    def __init__(self, client, program, teachers, rooms, timeslots):
        ...
        self.schedule_class_url = (
            "/manage/%s/" % self.program.getUrlBase() + "ajax_schedule_class"
        )

    def getClassToSchedule(
        self, section=None, teacher=None, timeslots=None, rooms=None
    ):
        if section is None:
            if teacher is None:
    ...
    def scheduleClass(self, section=None, teacher=None, timeslots=None, rooms=None):
        ...
        (section, timeslots, rooms) = self.getClassToSchedule(
            section=section, teacher=teacher, timeslots=timeslots, rooms=rooms
        )
    ...
    def unschedule_class(self, section_id):
        resp = self.client.post(
            self.schedule_class_url, {"action": "deletereg", "cls": section_id}
        )
