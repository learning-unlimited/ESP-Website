...
class UnenrollModuleTest(ProgramFrameworkTest):
    def setUp(self, *args, **kwargs):
        ...
        for student in random.sample(self.students, 10):
            ...
            Record.objects.create(event=rt, program=self.program, user=student)
    ...
    def test_json_view(self):
        ...
        for student_id, ts_id in data["student_timeslots"].items():
            ...
            self.assertTrue(
                ...
                .filter(user=student_id, section__meeting_times=ts_id)
                .exists()
            )
            self.assertFalse(
                Record.objects.filter(
                    event__name="attended", program=self.program, user=student_id
                ).exists()
            )
        ...
        for section_id, ts_id in data["section_timeslots"].items():
            ...
            self.assertTrue(
                StudentRegistration.valid_objects().filter(section=section_id).exists()
            )
    ...
    def test_submit(self):
        ...
        self.assertFalse(
            enrollments.filter(StudentRegistration.is_valid_qobject()).exists()
        )
