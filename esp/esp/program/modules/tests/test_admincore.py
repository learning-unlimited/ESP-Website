...
class AdminCoreSettingsTest(ProgramFrameworkTest):
    ...
    def test_settings_view(self):
        ...
        prog_dict["program_modules"] = [
            m.id for m in self.program.program_modules.all()
        ]
        prog_dict["class_categories"] = [
            c.id for c in self.program.class_categories.all()
        ]
        ...
        if not prog_dict.get("term_friendly"):
            prog_dict["term_friendly"] = self.program.name.replace(
                self.program.program_type, "", 1
            ).strip()
        prog_dict["program_type"] = self.program.program_type

        response = self.client.post(
            f"/manage/{self.program.url}/settings/program", prog_dict
        )
...
class AdminCoreDeadlineManagementTest(ProgramFrameworkTest):
    ...
    def test_deadline_management_open_creates_new(self):
        ...
        perm = Permission.objects.get(
            role=group, permission_type=perm_type, program=self.program
        )
...
class AdminCoreWipeTestDataTest(ProgramFrameworkTest):
    ...
    def test_wipe_test_data_preview(self):
        ...
        with patch(
            "esp.program.modules.handlers.admincore.DataCleanupController.execute"
        ) as mock_execute:
    ...
    def test_wipe_test_data_execute(self):
        ...
        with patch(
            ...
            response = self.client.post(
                url, {"username": "test_user_wipe", "confirmed": "1"}
            )
            self.assertEqual(response.status_code, 200)
