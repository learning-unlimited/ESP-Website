...
class StudentExtraCostsTest(ProgramFrameworkTest):
    ...
    def test_extracosts_page_loads(self):
        ...
        response = self.client.get(
            "/learn/%s/extracosts" % self.program.getUrlBase(), follow=True
        )
    ...
    def test_lineitemtypes_excludes_admission(self):
        ...
        admission_items = list(
            LineItemType.objects.filter(
                program=self.program, text__icontains="admission"
            )
        )
        self.assertTrue(
            admission_items,
    ...
    def test_multi_cost_item_form(self):
        form = MultiCostItem(
            data={"count": 2}, cost=25, required=False, max_quantity=5, for_finaid=True
        )
        self.assertTrue(form.is_valid())
        form = MultiCostItem(
            data={"count": 10}, cost=25, required=False, max_quantity=5, for_finaid=True
        )
        self.assertFalse(form.is_valid())
...
class StudentRegCoreTest(ProgramFrameworkTest):
    def setUp(self):
        super().setUp(
            num_students=5, num_teachers=3, classes_per_teacher=2, base_cost=100
        )
        self.add_student_profiles()
    ...
    def test_student_reg_page_loads(self):
        ...
        self.assertTrue(logged_in)
        response = self.client.get(
            "/learn/%s/studentreg" % self.program.getUrlBase(), follow=True
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_confirmreg_page(self):
        ...
        self.assertTrue(logged_in)
        response = self.client.get(
            "/learn/%s/confirmreg" % self.program.getUrlBase(), follow=True
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_cancelreg(self):
        ...
        self.assertTrue(logged_in)
        response = self.client.get(
            "/learn/%s/cancelreg" % self.program.getUrlBase(), follow=True
        )
        self.assertEqual(response.status_code, 200)
...
class StudentRegPhaseZeroTest(ProgramFrameworkTest):
    ...
    def test_phase_zero_page_loads(self):
        ...
        self.assertTrue(logged_in)
        response = self.client.get(
            "/learn/%s/studentregphasezero" % self.program.getUrlBase(), follow=True
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_phase_zero_record(self):
        ...
        self.assertTrue(
            PhaseZeroRecord.objects.filter(
                user=self.student, program=self.program
            ).exists()
        )
    ...
    def test_studentlookup(self):
        ...
        record.user.add(self.student)
        logged_in = self.client.login(
            username=self.students[1].username, password="password"
        )
        self.assertTrue(logged_in)
        response = self.client.get(
            "/learn/%s/studentlookup?username=%s"
    ...
    def test_joingroup_errors(self):
        ...
        self.assertTrue(logged_in)
        response = self.client.post(
            "/learn/%s/joingroup" % self.program.getUrlBase(), {}, follow=True
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.post(
            "/learn/%s/joingroup" % self.program.getUrlBase(),
            {"student_selected": "999999"},
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
...
class StudentRegConfirmTest(ProgramFrameworkTest):
    ...
    def test_do_confirmreg_redirects(self):
        ...
        self.assertTrue(logged_in)
        response = self.client.get(
            "/learn/%s/do_confirmreg" % self.program.getUrlBase()
        )
...
class StudentAcknowledgementTest(ProgramFrameworkTest):
    ...
    def test_students_query(self):
        module = self.program.getModule("StudentAcknowledgementModule")
        self.assertEqual(
            module.students(QObject=False)["studentacknowledgement"].count(), 0
        )
        rt = RecordType.objects.get(name="studentacknowledgement")
        Record.objects.create(user=self.student, program=self.program, event=rt)
        self.assertIn(
            self.student, module.students(QObject=False)["studentacknowledgement"]
        )
...
class StudentOnsiteTest(ProgramFrameworkTest):
    ...
    def test_studentonsite_page(self):
        ...
        self.assertTrue(logged_in)
        response = self.client.get(
            "/learn/%s/studentonsite" % self.program.getUrlBase(), follow=True
        )
        self.assertEqual(response.status_code, 200)
...
class StudentClassRegModuleTest(ProgramFrameworkTest):
    ...
    def test_catalog_json(self):
        response = self.client.get(
            "/learn/%s/catalog_json" % self.program.getUrlBase(), follow=True
        )
        self.assertEqual(response.status_code, 200)
