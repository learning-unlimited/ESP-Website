...
class AvailabilityModuleTest(ProgramFrameworkTest):
    def setUp(self, *args, **kwargs):
        ...
        am = ProgramModule.objects.get(
            handler="AvailabilityModule", module_type="teach"
        )
    ...
    def runTest(self):
        ...
        self.assertTrue(
            ...
            "Couldn't log in as teacher %s" % self.teachers[0].username,
        )

        # Submit availability, checking results each time
        # Available for one timeslot
        response = self.client.post(
            self.moduleobj.get_full_path(), data={"timeslots": timeslots[:1]}
        )
        self.assertTrue(response.status_code == 302)
        self.assertTrue(not self.moduleobj.isCompleted(self.teachers[0]))
        # Two timeslots
        response = self.client.post(
            self.moduleobj.get_full_path(), data={"timeslots": timeslots[:2]}
        )
        self.assertTrue(response.status_code == 302)
        self.assertTrue(not self.moduleobj.isCompleted(self.teachers[0]))
        # Three timeslots
        response = self.client.post(
            self.moduleobj.get_full_path(), data={"timeslots": timeslots}
        )
        self.assertTrue(response.status_code == 302)
