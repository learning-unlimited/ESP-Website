...
class GetHoursQueryCountTest(CacheFlushTestCase):
    ...
    def setUp(self):
        ...
        self.category = ClassCategories.objects.create(symbol="T", category="Test")
        ...
        for i in range(10):
            ...
            for j in range(2):
                ClassSection.objects.create(
                    parent_class=cls, status=10, duration=Decimal("1.00")
                )
...
class GetHoursApprovedFilterTest(CacheFlushTestCase):
    ...
    def setUp(self):
        ...
        self.category = ClassCategories.objects.create(symbol="A", category="ApprTest")
        self.program.class_categories.add(self.category)
        ...
        self.sec_approved = ClassSection.objects.create(
            parent_class=self.cls, status=10, duration=Decimal("1.00")
        )

        # Section 2: NOT approved (status=0)
        self.sec_unapproved = ClassSection.objects.create(
            parent_class=self.cls, status=0, duration=Decimal("2.00")
        )
...
class GetHoursScheduledFilterTest(CacheFlushTestCase):
    ...
    def setUp(self):
        ...
        self.category = ClassCategories.objects.create(symbol="S", category="SchedTest")
        self.program.class_categories.add(self.category)
        ...
        self.sec_unscheduled = ClassSection.objects.create(
            parent_class=self.cls, status=10, duration=Decimal("2.00")
        )
...
class StaticHoursTest(CacheFlushTestCase):
    ...
    def setUp(self):
        ...
        self.category = ClassCategories.objects.create(symbol="H", category="HoursTest")
        self.program.class_categories.add(self.category)
        ...
        for i, (dur, cap) in enumerate([(1, 20), (2, 30), (1.5, 10)]):
            ...
            cls.teachers.add(self.teacher)
            ClassSection.objects.create(
                parent_class=cls, status=10, duration=Decimal(str(dur))
            )
