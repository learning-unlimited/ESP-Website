...
class StudentRegTwoPhaseTest(ProgramFrameworkTest):
    ...
    def test_confirm_success(self):
        ...
        self.assertTrue(
            Record.objects.filter(
                user=student, event=rt, program=self.program
            ).exists(),
    ...
    def test_reconfirm_no_duplicate(self):
        ...
        # Confirm twice
        self.client.post("/learn/%s/confirm_registration" % self.program.getUrlBase())
        self.client.post("/learn/%s/confirm_registration" % self.program.getUrlBase())
    ...
    def test_min_classes_rejects(self):
        ...
        self.assertEqual(
            response.status_code, 302, "Expected redirect when below min_classes"
        )
    ...
    def test_no_min_classes_no_restriction(self):
        ...
        rt = RecordType.objects.get(name="twophase_reg_done")
        self.assertTrue(
            Record.objects.filter(user=student, event=rt, program=self.program).exists()
        )
    ...
    def test_mark_classes_page_loads(self):
        ...
        response = self.client.get("/learn/%s/mark_classes" % self.program.getUrlBase())
        self.assertEqual(response.status_code, 200)
    ...
    def test_mark_classes_interested_saves(self):
        ...
        classes = list(
            self.program.classes().filter(
                status__gte=0, grade_min__lte=grade, grade_max__gte=grade
            )
        )[:2]
        ...
        response = self.client.post(
            "/learn/%s/mark_classes_interested" % self.program.getUrlBase(),
            {
                "json_data": json_module.dumps(
                    {"interested": class_ids, "not_interested": []}
                )
            },
    ...
    def test_mark_classes_interested_removes(self):
        ...
        response = self.client.post(
            ...
            {
                "json_data": json_module.dumps(
                    {"interested": [], "not_interested": class_ids}
                )
            },
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
        ...
        for cls in starred:
            ...
            self.assertIsNotNone(ssi.end_date, "SSI should have end_date set")
    ...
    def test_view_classes_page_loads(self):
        ...
        response = self.client.get("/learn/%s/view_classes" % self.program.getUrlBase())
        self.assertEqual(response.status_code, 200)
