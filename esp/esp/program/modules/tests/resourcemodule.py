...
class ResourceModuleTest(ProgramFrameworkTest):
    ...
    def checkDisplayedClassroomList(self):
        ...
        program_classroom_names = {
            str(x.name) for x in self.program.groupedClassrooms()
        }

        #   Compare to those shown on the resources page
        self.assertEqual(
            program_classroom_names,
            self.getDisplayedList(r'<div id="classroom-([0-9]+)">(.*?)</div>', 1),
        )
    ...
    def checkDisplayedResourceTypeList(self):
        ...
        #   Compare to those shown on the resources page
        self.assertEqual(
            program_restype_names,
            self.getDisplayedList(r'<div id="restype-([0-9]+)">(.*?)</div>', 1),
        )
    ...
    def testClassrooms(self):
        ...
        response = self.client.post(
            "/manage/%s/resources/classroom" % self.program.getUrlBase(),
            add_classroom_data,
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "New Room", self.checkDisplayedClassroomList()
        )  #   checks consistency and presence of new room

        #   Check that we can edit a classroom and have it show up
        matching_rooms = [
            x for x in self.program.groupedClassrooms() if x.name == "New Room"
        ]
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "Edited Room", self.checkDisplayedClassroomList()
        )  #   checks consistency and presence of edited room
        ...
        self.assertNotIn(
            "Edited Room", self.checkDisplayedClassroomList()
        )  #   checks consistency and presence of room
    ...
    def testResourceTypes(self):
        ...
        response = self.client.post(
            "/manage/%s/resources/restype" % self.program.getUrlBase(), add_restype_data
        )
        self.assertEqual(response.status_code, 200)
        ...
        matching_restypes = [
            x for x in self.program.getResourceTypes() if x.name == "New Resource Type"
        ]
        ...
        response = self.client.post(
            "/manage/%s/resources/restype" % self.program.getUrlBase(),
            edit_restype_data,
        )
        self.assertEqual(response.status_code, 200)
