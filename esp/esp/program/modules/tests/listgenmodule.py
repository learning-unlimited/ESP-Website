...
from esp.program.modules.handlers.listgenmodule import (
    ListGenModule,
    ListGenForm,
    UserAttributeGetter,
)
...
class ListGenModuleTest(ModuleHandlerTestMixin, ProgramFrameworkTest):
    ...
    def test_generate_list_html_output(self):
        ...
        url = (
            self.get_module_url("manage", "generateList")
            + "?filterid=%s" % filter_obj.id
        )
    ...
    def test_select_list_old_invalid_filterid_errors_gracefully(self):
        ...
        self.assertEqual(response.status_code, 500)
        self.assertTemplateUsed(response, "error.html")
        self.assertIn("invalid or expired", response.content.decode("utf-8").lower())
