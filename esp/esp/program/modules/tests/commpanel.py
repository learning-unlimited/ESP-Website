...
class CommunicationsPanelTest(ProgramFrameworkTest):
    ...
    def test_basic_comm_flow(self):
        ...
        response = self.client.post(
            f"/manage/{self.program.getUrlBase()}/commpanel_old", post_data
        )
        ...
        response = self.client.post(
            f"/manage/{self.program.getUrlBase()}/commfinal", post_data
        )
        self.assertEqual(response.status_code, 200)

        #   Check that a MessageRequest has been created
        m = MessageRequest.objects.filter(
            recipients__id=filterid, subject="Test Subject 123"
        )
        ...
        self.assertEqual(
            len(html_alternatives), 1, "Expected exactly one text/html alternative."
        )
    ...
    def test_program_date_variables_in_comms(self):
        ...
        context_dict = {
            "user": ActionHandler(self.students[0], self.students[0]),
        ...
        self.assertIn(
            expected_first_day,
            rendered,
            "program.date should render as first program day",
        )
        self.assertIn(
            expected_date_range,
            rendered,
            "program.date_range should render as program date range",
        )
        self.assertIn(
            expected_teacher_reg_deadline,
            rendered,
            "program.teacher_reg_deadline should render as Teacher/Classes/Create end_date",
        )
...
class MakeImageUrlsAbsoluteTest(SimpleTestCase):
    ...
    def test_rewrites_relative_img_src(self):
        result = _make_image_urls_absolute(
            '<img src="/media/uploaded/foo.png">',
            self._make_image_urls_absolute_request(),
        )
        self.assertEqual(
            result, '<img src="https://example.com/media/uploaded/foo.png">'
        )
    ...
    def test_leaves_protocol_relative_url_unchanged(self):
        ...
        self.assertEqual(
            _make_image_urls_absolute(body, self._make_image_urls_absolute_request()),
            body,
        )
    ...
    def test_rewrites_multiple_images(self):
        ...
        result = _make_image_urls_absolute(
            body, self._make_image_urls_absolute_request()
        )
        self.assertEqual(
            result,
            '<img src="https://example.com/a.png"> text <img src="https://example.com/b.png">',
        )
    ...
    def test_empty_body(self):
        self.assertEqual(
            _make_image_urls_absolute("", self._make_image_urls_absolute_request()), ""
        )
