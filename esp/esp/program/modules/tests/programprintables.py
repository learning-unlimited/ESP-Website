...
class TestOnsiteUseridValidation(ProgramFrameworkTest):
    """Test onsite userid validation in ProgramPrintables"""
    ...
    def setUp(self, *args, **kwargs):
        ...
        m = ProgramModule.objects.get(handler='ProgramPrintables', module_type='manage')
        self.moduleobj = ProgramModuleObj.getFromProgModule(self.program, m)
        self.base_url = '/manage/%s/%s' % (self.program.getUrlBase(), '')
    ...
    def test_student_financial_spreadsheet_missing_userid(self):
        ...
        self._login_admin()
        response = self.client.get('/manage/%s/%s' % (self.program.getUrlBase(), 'student_financial_spreadsheet'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_student_financial_spreadsheet_invalid_userid(self):
        ...
        self._login_admin()
        response = self.client.get('/manage/%s/%s?userid=invalid' % (self.program.getUrlBase(), 'student_financial_spreadsheet'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_student_financial_spreadsheet_nonexistent_userid(self):
        ...
        self._login_admin()
        response = self.client.get('/manage/%s/%s?userid=99999' % (self.program.getUrlBase(), 'student_financial_spreadsheet'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_studentschedules_missing_userid(self):
        ...
        self._login_admin()
        response = self.client.get('/manage/%s/%s' % (self.program.getUrlBase(), 'studentschedules'))
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Missing userid parameter', response.content)
    ...
    def test_studentschedules_invalid_userid(self):
        ...
        self._login_admin()
        response = self.client.get('/manage/%s/%s?userid=invalid' % (self.program.getUrlBase(), 'studentschedules'))
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Invalid userid format', response.content)
    ...
    def test_studentschedules_nonexistent_userid(self):
        ...
        self._login_admin()
        response = self.client.get('/manage/%s/%s?userid=99999' % (self.program.getUrlBase(), 'studentschedules'))
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'User not found with the provided userid', response.content)
