...
class StudentRegPhaseZeroTestCase(ProgramFrameworkTest):
    ...
    def test_get_before_opening_requires_student_phasezero_permission(self):
        ...
        self.assertFalse(
            Permission.user_has_perm(
                self.student, "Student/PhaseZero", program=self.program
            ),
    ...
    def test_get_after_lottery_closed_before_run_for_entered_student_shows_closed_message(
        self,
    ):
        ...
        self.assertContains(
            response, "This page will be updated once the lottery has been run"
        )

    @patch("esp.program.modules.handlers.studentregphasezero.send_mail")
    def test_post_creates_phasezero_record_and_sends_confirmation_email(
        self, mock_send_mail
    ):
        ...
        self.assertEqual(
            PhaseZeroRecord.objects.filter(program=self.program).count(), 0
        )

        response = self.client.post(self.url, data={})

        self.assertEqual(
            PhaseZeroRecord.objects.filter(program=self.program).count(), 1
        )
