...
class TeacherPreviewModuleTest(ProgramFrameworkTest):
    ...
    def setUp(self, *args, **kwargs):
        ...
        pm = ProgramModule.objects.get(
            handler="TeacherPreviewModule", module_type="teach"
        )
        ...
        ProgramModuleObj.objects.filter(
            program=self.program, module__handler__icontains="printables"
        ).delete()
        pm_printables = ProgramModule.objects.get(
            handler="ProgramPrintables", module_type="manage"
        )
    ...
    def _get_schedule_response(self, user, endpoint, query_params=None):
        ...
        response = self.client.get(
            "/teach/%s/%s" % (self.program.getUrlBase(), endpoint), query_params or {}
        )
        self.client.logout()
        return response
    ...
    def test_teacher_schedule_generation_with_printables_module(self):
        ...
        pmos = ProgramModuleObj.objects.filter(
            program=self.program, module__handler="ProgramPrintables"
        )
        self.assertEqual(
            pmos.count(), 1, "Expected one ProgramPrintables module for this program"
        )

        self._assign_resources_to_classes(self.teacher_classes)
        scheduled_sections = [
            sec
            for sec in self.teacher_sections
        ...
        self.assertEqual(
            ...
            "Expected %d schedule items, got %d"
            % (len(scheduled_sections), len(scheditems)),
        )
    ...
    def test_moderator_inclusive_schedule_generation(self):
        ...
        moderated_class = (
            self.teacher_classes[1] if len(self.teacher_classes) > 1 else taught_class
        )
    ...
    def test_admin_impersonation_via_user_query_param(self):
        ...
        impersonate_classes = impersonate_teacher.getTaughtClassesFromProgram(
            self.program
        )
    ...
    def test_non_admin_cannot_impersonate_other_teacher(self):
        ...
        self._assign_resources_to_classes(
            teacher1.getTaughtClassesFromProgram(self.program)
        )
        self._assign_resources_to_classes(
            teacher2.getTaughtClassesFromProgram(self.program)
        )

        response = self._get_schedule_response(
            teacher1, "teacherschedule", query_params={"user": str(teacher2.id)}
        )
    ...
    def test_error_when_no_printables_module_resolved(self):
        ...
        request = SimpleNamespace(
            ...
            build_absolute_uri=lambda: (
                "/teach/%s/teacherschedule" % self.program.getUrlBase()
            ),
        )
    ...
    def test_schedule_items_sorted_by_class(self):
        ...
        if len(scheditems) > 1:
            ...
            valid_sections = sorted(
                [
                    sec
                    for sec in self.teacher.getTaughtSectionsFromProgram(self.program)
                    if sec.meeting_times.all().exists()
                    and sec.resourceassignment_set.all().exists()
                    and sec.status > 0
                ],
    ...
    def test_moderator_schedule_filters_correctly(self):
        ...
        for item in response.context["scheditems"]:
            ...
            self.assertTrue(
                cls.resourceassignment_set.all().exists(), "Class has resources"
            )
