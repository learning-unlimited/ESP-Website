...
class ClassFlagModuleTest(ProgramFrameworkTest):
    def setUp(self, *args, **kwargs):
        ...
        self.flag_type = ClassFlagType.objects.create(name="Test Flag", color="#ff0000")
    ...
    def test_create_flag(self):
        ...
        flag_type2 = ClassFlagType.objects.create(name="Test Flag 2", color="#00ff00")
        self.program.flag_types.add(flag_type2)

        response = self.client.post(
            self.base_url + "/newflag/",
            {
        ...
        self.assertTrue(
            ClassFlag.objects.filter(
                subject=self.subject, flag_type=flag_type2
            ).exists()
        )
    ...
    def test_editflag_returns_json(self):
        ...
        response = self.client.post(
            self.base_url + "/editflag/%d/" % self.flag.id, {"comment": "json test"}
        )
        self.assertEqual(response["Content-Type"], "application/json")
        data = json.loads(response.content)
        self.assertIn("flag_detail", data)

    def test_resolveflag_rejects_get(self):
        """GET to resolveflag returns 400."""
        response = self.client.get(self.base_url + "/resolveflag/%d/" % self.flag.id)
    ...
    def test_resolve_flag(self):
        ...
        response = self.client.post(
            self.base_url + "/resolveflag/%d/" % self.flag.id, {"action": "resolve"}
        )
        self.assertEqual(response.status_code, 200)
        self.flag.refresh_from_db()
    ...
    def test_unresolve_flag(self):
        ...
        # Then unresolve
        response = self.client.post(
            self.base_url + "/resolveflag/%d/" % self.flag.id, {"action": "unresolve"}
        )
        self.assertEqual(response.status_code, 200)
        self.flag.refresh_from_db()
        self.assertFalse(self.flag.resolved)
    ...
    def test_delete_flag(self):
        ...
        response = self.client.post(self.base_url + "/deleteflag/", {"id": flag_id})
        self.assertEqual(response.status_code, 200)
    ...
    def test_cross_program_idor_blocked(self):
        ...
        response = self.client.post(
            self.base_url + "/resolveflag/%d/" % other_flag.id, {"action": "resolve"}
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_newflag_cross_program_subject_idor_blocked(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertFalse(ClassFlag.objects.filter(subject=other_subject).exists())
    ...
    def test_newflag_cross_program_flagtype_idor_blocked(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertFalse(ClassFlag.objects.filter(flag_type=other_flag_type).exists())
    ...
    def test_search_flag_with_resolved_status(self):
        ...
        self.assertGreaterEqual(
            len(all_classes), 2, "Need at least 2 classes to test exclusion"
        )
        ...
        response = self.client.get(self.base_url + "/classsearch/", {"query": query})
        self.assertEqual(response.status_code, 200)
