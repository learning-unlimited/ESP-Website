...
class TeacherScheduleDeadlineTest(ProgramFrameworkTest):
    ...
    That deadline gates the room/time assignments that teachers (and
    moderators) see on the teacher class registration page, so that chapters
    can hide them while scheduling is still in progress.
    """

    #   Permissions that grant Teacher/Classes/Schedule, directly or by
    #   implication.  Deleting these (and nothing else) closes the schedule
    #   deadline while leaving the rest of teacher reg reachable.
    ...
    def setUp(self, *args, **kwargs):
        ...
        #   Disable force_show_required_modules so that requesting the teacher
        #   reg page renders it, instead of CoreModule routing serving the
        #   first incomplete required module (e.g. the profile editor).
        ...
        #   Give the classes real room/time assignments so that the templates
    ...
    def test_can_view_schedule_with_specific_permission(self):
        """Teacher/Classes/Schedule alone is enough."""
    ...
    def test_teacherreg_shows_schedule_when_open(self):
        ...
        self.assertNotContains(response, 'Room and time assignments are not available yet.')
    ...
    def test_teacherreg_hides_schedule_when_closed(self):
        ...
        self.assertContains(response, 'Room and time assignments are not available yet.')
    ...
    def test_class_status_hides_time_blocks_when_closed(self):
        ...
        response = self.client.get(url)
        self.assertContains(response, 'Time Blocks', status_code=200)

        self.close_schedule_deadline()
        response = self.client.get(url)
        self.assertNotContains(response, 'Time Blocks', status_code=200)
