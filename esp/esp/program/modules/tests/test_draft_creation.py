...
class DraftCreationTestMixin(object):
    ...
    def setUp(self):
        ...
        # Set up resource types for testing
        from esp.resources.models import ResourceType
        self.resource_types = ResourceType.objects.all()[:2]  # Get first 2 resource types
...
class MakeAClassDraftTest(DraftCreationTestMixin, ProgramFrameworkTest):
    ...
    def test_javascript_draft_button_initially_disabled(self):
        ...
        self.assertContains(response, 'initialFormValues')
        
        # The button should be disabled initially (form hasn't changed yet)
        self.assertContains(response, 'draftButton.disabled = !hasChanged')
    ...
    def test_draft_with_resource_requests(self):
        ...
        if sections:
            resource_requests = ResourceRequest.objects.filter(target=sections[0])
            self.assertGreaterEqual(resource_requests.count(), 0,
    ...
    def test_draft_with_all_field_types(self):
        ...
        draft_data.update({
            ...
            'class_size_optimal': '15',
            'session_count': '3',
            'allow_lateness': 'on',  # Use 'on' instead of 'True' to match form expectations
        })
    ...
    def test_draft_with_custom_fields(self):
        """Draft should handle custom form fields properly."""
        teacher = self.teachers[0]
        self.assertTrue(
            self.client.login(username=teacher.username, password='password'),
            "Couldn't log in as teacher %s" % teacher.username
        ...
        # Create draft with custom field data
        draft_data = self._make_draft_form_data(teacher)
        draft_data['custom_field_1'] = 'Custom value 1'
        draft_data['custom_field_2'] = 'Custom value 2'
        ...
        # Check custom_form_data contains the custom fields
        if draft.custom_form_data:
            self.assertIn('custom_field_1', draft.custom_form_data)
            self.assertIn('custom_field_2', draft.custom_form_data)
            self.assertEqual(draft.custom_form_data['custom_field_1'], 'Custom value 1')
            self.assertEqual(draft.custom_form_data['custom_field_2'], 'Custom value 2')
