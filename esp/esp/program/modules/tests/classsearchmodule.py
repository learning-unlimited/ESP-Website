...
class ClassSearchModuleTest(ProgramFrameworkTest):
    ...
    def test_results_page(self):
        ...
        r = self.client.get(
            "/manage/" + self.program.url + "/classsearch/", {"query": query}
        )
    ...
    def test_simple_search_query(self):
        ...
        self.assertEqual(
            query,
            {
                ...
                "negated": False,
                "values": [
                    {"filter": "title", "negated": False, "values": ["Test class 3"]},
        ...
        self.assertEqual(
            [cls.title for cls in self.qb.as_queryset(query)], ["Test class 3"]
        )
    ...
    def test_simple_search_ignores_bad_values(self):
        ...
        query = self.simple_search_query(
            s_title="Test class 3", s_category="not a number"
        )
        self.assertEqual(
            query["values"],
            [{"filter": "title", "negated": False, "values": ["Test class 3"]}],
    ...
    def test_simple_search_bad_values_page(self):
        self.client.login(username="admin", password="password")
        r = self.client.get(
            "/manage/" + self.program.url + "/classsearch/",
            {"s_category": "not a number"},
        )
    ...
    def test_simple_search_page(self):
        self.client.login(username="admin", password="password")
        r = self.client.get(
            "/manage/" + self.program.url + "/classsearch/", {"s_title": "Test class 3"}
        )
        self.assertContains(r, "Test class 3")
        self.assertNotContains(r, "Test class 4")
        # The query builder should be preloaded with the equivalent query.
    ...
    def test_simple_search_by_teacher(self):
        ...
        titles = sorted(
            cls.title for cls in self.program.classes().filter(teachers=teacher)
        )
    ...
    def test_advanced_search_stays_available(self):
        ...
        for params in ({}, {"s_title": "Test class 3"}):
            r = self.client.get("/manage/" + self.program.url + "/classsearch/", params)
