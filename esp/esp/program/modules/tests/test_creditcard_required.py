...
class CreditCardSelfBlockingTest(TestCase):
    ...
    def test_payonline_still_checks_other_modules(self):
        ...
        self.assertFalse(
            completedAll, "Other incomplete required modules should block payment"
        )
...
class CreditCardStripeApplySettingsTest(TestCase):
    ...
    def test_apply_settings_tag_valid_json(self):
        ...
        Tag.setTag(
            "stripe_settings", target=self.program, value='{"offer_donation": false}'
        )
        result = self.cc_module.apply_settings()
