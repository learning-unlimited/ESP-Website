...
class TeacherCheckinModuleTest(ProgramFrameworkTest):
    ...
    def isCheckedIn(self, when=None, teacher=None):
        ...
        return Record.user_completed(
            teacher, self.event, self.program, when, only_today=True
        )
    ...
    def missingTeachersPage(self, **params):
        ...
        response = self.client.get(
            "%smissingteachers" % self.program.get_onsite_url(), params
        )
    ...
    def test_checkIn(self):
        ...
        self.assertIn("is not a teacher", self.checkIn(teacher=self.teacher2))
        self.assertIn("was not checked in for", self.undoCheckIn(teacher=self.teacher2))
    ...
    def test_phone_numbers_on_checkin_page(self):
        ...
        phone = phonenumbers.format_number(
            self.teacher.getLastProfile().contact_user.phone_cell,
    ...
    def test_missingteachers_when_not_defaulted_for_today(self):
        ...
        self.addTimeslot(
            datetime.datetime.combine(today, datetime.time(9, 0)), "Today Slot"
        )
    ...
    def test_missingteachers_explicit_when_is_used_and_preserved(self):
        ...
        for offset, arrived in (
            ...
            with self.subTest(when=when):
                ...
                self.assertEqual(
                    response.context["url_when"], when.strftime("%m/%d/%Y %H:%M")
                )
                self.assertEqual(
                    self.teacher.id in response.context["arrived"], arrived
                )
