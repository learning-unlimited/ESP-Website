...
class OnSiteRegisterTest(ModuleHandlerTestMixin, ProgramFrameworkTest):
    ...
    def test_successful_registration(self, mock_paid, mock_recover):
        ...
        self.assertTemplateUsed(
            response, "program/modules/onsiteregister/reg_success.html"
        )
        ...
        self.assertTrue(
            RegistrationProfile.objects.filter(
                user=new_user, program=self.program
            ).exists()
        )
        self.assertTrue(
            Record.objects.filter(
                user=new_user, program=self.program, event__name="onsite"
            ).exists()
        )
        self.assertTrue(
            Record.objects.filter(
                user=new_user, program=self.program, event__name="attended"
            ).exists()
        )
        self.assertTrue(
            Record.objects.filter(
                user=new_user, program=self.program, event__name="med"
            ).exists()
        )
        self.assertTrue(
            Record.objects.filter(
                user=new_user, program=self.program, event__name="liab"
            ).exists()
        )
    ...
    def test_invalid_form_stays_on_page(self, mock_paid, mock_recover):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "program/modules/onsiteregister/reg_info.html"
        )
