...
class AjaxStudentRegTest(ProgramFrameworkTest):
    ...
    def expect_empty_schedule(self, response):
        ...
        search_str = (
            "Your schedule for %s is empty.  Please add classes below!"
            % self.program.niceName()
        )
        self.assertTrue(
            search_str in resp_data["student_schedule_html"],
            f'Could not find empty fragment "{search_str}" in response "{resp_data["student_schedule_html"]}"',
        )
    ...
    def expect_sections_in_schedule(self, response, sections=[]):
        ...
        for sec in sections:
            ...
            self.assertTrue(
                sec.friendly_times()[0] in resp_data["student_schedule_html"]
            )
    ...
    def expect_ajaxerror(self, request_url, post_data, error_str):
        ...
        response = self.client.post(
            request_url, post_data, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
        ...
        self.assertTrue(
            error_msg == error_str,
            f'Unexpected Ajax error: "{error_msg}", expected "{error_str}"',
        )
    ...
    def test_ajax_addclass(self):
        ...
        conflicting_sections = list(
            ClassSection.objects.filter(
                parent_class__parent_program=program, meeting_times=sec1.start_time()
            ).exclude(parent_class=sec1.parent_class)
        )

        if not conflicting_sections:
            self.skipTest(
                "Skipping rest of test_ajax_addclass: No conflicting sections found (random seed)."
            )

        sec2 = random.choice(conflicting_sections)
        self.expect_ajaxerror(
            "/learn/%s/ajax_addclass" % program.getUrlBase(),
            {"class_id": sec2.parent_class.id, "section_id": sec2.id},
            "This section conflicts with your schedule--check out the other sections!",
        )
        ...
        if not other_sections:
            self.skipTest(
                "Skipping rest of test_ajax_addclass: No other sections of the same class found."
            )

        sec3 = random.choice(other_sections)
        self.expect_ajaxerror(
            "/learn/%s/ajax_addclass" % program.getUrlBase(),
            {"class_id": sec3.parent_class.id, "section_id": sec3.id},
            "You are already signed up for a section of this class!",
        )
        ...
        valid_sections = list(
            program.sections()
            .exclude(parent_class=sec1.parent_class)
            .exclude(meeting_times__in=sec1.meeting_times.all())
        )

        if not valid_sections:
            self.skipTest(
                "Skipping rest of test_ajax_addclass: No valid non-conflicting sections found."
            )

        sec4 = random.choice(valid_sections)
        response = self.client.post(
            "/learn/%s/ajax_addclass" % program.getUrlBase(),
            {"class_id": sec4.parent_class.id, "section_id": sec4.id},
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
    ...
    def test_ajax_addclass_mismatched_ids(self):
        ...
        sec1 = random.choice(program.sections())
        other_class_sections = list(
            program.sections().exclude(parent_class=sec1.parent_class)
        )
        if not other_class_sections:
            self.skipTest(
                "Skipping test_ajax_addclass_mismatched_ids: no section from a different class found."
            )
        ...
        response = self.client.post(
            "/learn/%s/ajax_addclass" % program.getUrlBase(),
            {"class_id": sec1.parent_class.id, "section_id": 999999},
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
        ...
        response = self.client.post(
            "/learn/%s/ajax_addclass" % program.getUrlBase(),
            {"class_id": sec_other.parent_class.id, "section_id": sec1.id},
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_ajax_clearslot(self):
        ...
        if not valid_second_sections:
            self.skipTest(
                "Skipping remainder of test_ajax_clearslot due to bad random seed."
            )
        ...
        response = self.client.get(
            f"/learn/{program.getUrlBase()}/ajax_clearslot/{sec1.meeting_times.all()[0].id}",
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
        ...
        response = self.client.get(
            f"/learn/{program.getUrlBase()}/ajax_clearslot/{sec2.meeting_times.all()[0].id}",
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
        self.expect_empty_schedule(response)
