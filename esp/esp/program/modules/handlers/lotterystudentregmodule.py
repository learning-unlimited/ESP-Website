...
class LotteryStudentRegModule(ProgramModuleObj):
    ...
    def isCompleted(self, user=None):
        ...
        return bool(
            StudentRegistration.valid_objects().filter(
                section__parent_class__parent_program=self.program, user=user
            )
        )
    ...
    def lotterystudentreg(self, request, tl, one, two, module, extra, prog):
        ...
        context = {
            ...
            "open_class_registration": {False: 0, True: 1}[
                crmi.open_class_registration
            ],
            "open_class_category": open_class_category,
        }
        ...
        # Splark/Splash style lottery
        return render_to_response(
            "program/modules/lotterystudentregmodule/student_reg_splash.html",
            request,
            context,
        )
    ...
    def timeslots_json(self, request, tl, one, two, module, extra, prog, timeslot=None):
        ...
        ordered_timeslots = sorted(
            self.program.getTimeSlotList(), key=lambda event: event.start
        )
    ...
    def viewlotteryprefs(self, request, tl, one, two, module, extra, prog):
        ...
        uniquified_flags = []
        priority_flags = StudentRegistration.valid_objects().filter(
            user=request.user,
        ...
        if priority_flags.count() == 0:
            ...
            context["pempty"] = False

        interested_classids = set()
        uniquified_interested = []
        interested = StudentRegistration.valid_objects().filter(
            user=request.user,
            section__parent_class__parent_program=prog,
