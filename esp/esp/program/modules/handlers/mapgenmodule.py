...
class MapGenModule(ProgramModuleObj):
    ...
    def usermap(self, request, tl, one, two, module, extra, prog):
        ...
        if request.method == "POST":
            ...
            states_raw = (
                ContactInfo.objects.filter(user__in=users, as_user__isnull=False)
                .distinct("user")
            ...
            zipcodes_raw = (
                ContactInfo.objects.filter(user__in=users, as_user__isnull=False)
                .distinct("user")
                .values_list("address_zip", flat=True)
            )
            ...
            states_corr = {
                state: (
                    states.get(state, 0)
                    if states.get(state, 0) > zip_states[state]
                    else zip_states[state]
                )
                for state in zip_states.keys()
            }
