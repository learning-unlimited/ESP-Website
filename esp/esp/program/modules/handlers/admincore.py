...
class AdminCore(ProgramModuleObj, CoreModule):
    ...
    def deadline_management(self, request, tl, one, two, module, extra, prog):
       ...
       #  Handle 'open' / 'close' / 'delete' actions SECURELY via POST
        if request.method == 'POST':
            if extra == 'open':
            ...
            elif extra == 'delete' and 'perm_id' in request.POST:
                perms = Permission.objects.filter(id=request.POST['perm_id'])
                if perms.count() == 1:
                    perm = perms[0]
                    message_good = 'Action successful for %s.' % (perm.user if perm.user else perm.role)
                    perm.delete()
                else:
                    message_bad = 'Error while deleting ID %s.' % request.POST['perm_id']
