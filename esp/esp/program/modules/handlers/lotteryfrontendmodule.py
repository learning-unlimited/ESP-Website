...
class LotteryFrontendModule(ProgramModuleObj):
    ...
    def lottery(self, request, tl, one, two, module, extra, prog):
        ...
        if (
            "lotteried_students" not in students
            and "twophase_star_students" not in students
        ):
            return render_to_response(
                self.baseDir() + "not_configured.html", request, {"program": prog}
            )
        ...
        context = {
            "options": {
                k: v
                for k, v in LotteryAssignmentController.default_options.items()
                if v[1] is not False
            },
            "has_old_schedules": StudentRegistration.objects.filter(
    ...
    def lottery_save(self, request, tl, one, two, module, extra, prog):
        if "lottery_data" not in request.POST:
            return {
                "response": [
                    {"success": "no", "error": "missing lottery_data POST field"}
                ]
            }
