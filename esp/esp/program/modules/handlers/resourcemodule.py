...
from esp.program.modules.forms.resources import (
    ClassroomForm,
    TimeslotForm,
    ResourceTypeForm,
    ResourceChoiceForm,
    EquipmentForm,
    FurnishingFormForProgram,
    ClassroomImportForm,
    TimeslotImportForm,
    ResTypeImportForm,
    EquipmentImportForm,
...
class ResourceModule(ProgramModuleObj):
        ...
        if request.method == "POST":
            ...
            if data["command"] == "reallyremove":
                try:
                    ...
                    return (
                        HttpResponseBadRequest("Invalid or missing timeslot ID."),
                        {},
                    )
    ...
    def resources_restype(self, request, tl, one, two, module, extra, prog):
        ...
        if request.GET.get("op") == "edit":
            #   pre-fill form
            try:
                current_slot = self.program.getResourceTypes(
                ...
                return (
                    HttpResponseBadRequest("Invalid or missing resource type ID."),
                    {},
                )
            ...
            ResourceChoiceSet = formset_factory(
                ResourceChoiceForm, max_num=10, extra=0 if len(choices) else 1
            )
            context["reschoice_formset"] = ResourceChoiceSet(
                initial=choices, prefix="resourcechoices"
            )

        if request.GET.get("op") == "delete":
            #   show delete confirmation page
            try:
                restype = self.program.getResourceTypes(
                    include_global=Tag.getBooleanTag("allow_global_restypes")
                ).get(id=request.GET["id"])
            except (KeyError, ValueError, ResourceType.DoesNotExist):
                return (
            ...
            response = render_to_response(
                self.baseDir() + "restype_delete.html", request, context
            )
        ...
        if request.method == "POST":
            ...
            if data["command"] == "reallyremove":
                    ...
                    try:
                        ...
                        return (
                            HttpResponseBadRequest(
                                "Invalid or missing resource type ID."
                            ),
                            {},
                        )
                ...
                ResourceChoiceSet = formset_factory(
                    ResourceChoiceForm, max_num=10, extra=0 if num_choices else 1
                )
    ...
    def resources_classroom(self, request, tl, one, two, module, extra, prog):
        ...
        if request.GET.get("op") == "edit":
            ...
            context["classroom_form"] = ClassroomForm(
                self.program, auto_id="classroom_%s"
            )
            context["classroom_form"].load_classroom(self.program, current_room)
            furnishings = [
                {
                    "furnishing": furnishing.res_type.id,
                    "choice": furnishing.attribute_value,
                }
                for furnishing in current_room.associated_resources()
            ]
            FurnishingFormSet = formset_factory(
                FurnishingFormForProgram(prog), max_num=1000, extra=0
            )
            context["furnishing_formset"] = FurnishingFormSet(
                initial=furnishings, prefix="furnishings"
            )
        ...
        if request.method == "POST":
            ...
            if data["command"] == "reallyremove":
                try:
                    ...
                    return (
                        HttpResponseBadRequest("Invalid or missing classroom ID."),
                        {},
                    )
                ...
                furnishings = list(
                    map(
                        dict, frozenset(frozenset(list(i.items())) for i in furnishings)
                    )
                )
                if form.is_valid():
    ...
    def resources_timeslot_import(self, request, tl, one, two, module, extra, prog):
        ...
        import_form = TimeslotImportForm(
            request.POST, cur_prog=prog, auto_id="timeslot_import_%s"
        )
        if not import_form.is_valid():
            ...
            for orig_timeslot in prev_timeslots:
                ...
                if (
                    ...
                    and str(orig_timeslot.id) in to_import
                ):
                    new_timeslot.save()
                else:
    ...
    def resources_restype_import(self, request, tl, one, two, module, extra, prog):
        ...
        import_form = ResTypeImportForm(
            request.POST, cur_prog=prog, auto_id="restype_import_%s"
        )
        if not import_form.is_valid():
            ...
            for res_type in res_types:
                ...
                if (
                    import_mode == "save"
                    and not ResourceType.objects.filter(
                        name=new_res_type.name,
                        description=new_res_type.description,
                        program=self.program,
                    ).exists()
                    and str(res_type.id) in to_import
                ):
                    new_res_type.save()
                else:
            ...
            if import_mode == "preview":
                context["prog"] = self.program
                context["new_restypes"] = sorted(
                    res_type_list,
                    key=lambda x: (not x.hidden, x.priority_default),
                    reverse=True,
                )
                response = render_to_response(
                    self.baseDir() + "restype_import.html", request, context
                )
            else:
                extra = "restype"
    ...
    def resources_classroom_import(self, request, tl, one, two, module, extra, prog):
        ...
        import_form = ClassroomImportForm(
            request.POST, cur_prog=prog, auto_id="classroom_import_%s"
        )
        if not import_form.is_valid():
                    ...
                    for timeslot in self.program.getTimeSlots():
                        ...
                        if (
                            ...
                            and not Resource.objects.filter(
                                name=new_res.name, event=new_res.event
                            ).exists()
                            and str(resource.id) in to_import
                        ):
                            new_res.save()
                        else:
                            new_res.old_id = resource.id
                        if import_furnishings:
                            new_furns = self.furnishings_import(
                                resource, new_res, self.program, import_mode, to_import
                            )
                            furnishing_dict[resource.name].update(
                                new_furn.res_type.name
                                + (" (Hidden)" if new_furn.res_type.hidden else "")
                                + (
                                    (": " + new_furn.attribute_value)
                                    if new_furn.attribute_value
                                    else ""
                                )
                                for new_furn in new_furns
                            )
                        resource_list.append(new_res)
            else:
                #   Attempt to match timeslots for the programs
                ts_old = (
                    past_program.getTimeSlots()
                    .filter(event_type__description__icontains="class")
                    .order_by("start")
                )
                ts_new = (
    ...
    def furnishings_import(old_res, new_res, prog, import_mode, to_import):
        ...
        for f in furnishings:
            ...
            if (
                import_mode == "save"
                and not Resource.objects.filter(
                    name=new_furnishing.name,
                    event=new_res.event,
                    attribute_value=new_furnishing.attribute_value,
                ).exists()
                and str(old_res.id) in to_import
            ):
                new_furnishing.save()
    ...
    def resources_equipment(self, request, tl, one, two, module, extra, prog):
        ...
        if request.GET.get("op") == "delete":
            #   show delete confirmation page
            try:
                equipment = self.program.getFloatingResources(queryset=True).get(
                    id=request.GET["id"]
                )
            except (KeyError, ValueError, Resource.DoesNotExist):
                return (HttpResponseBadRequest("Invalid or missing equipment ID."), {})
        ...
        if request.method == "POST":
            ...
            if data["command"] == "reallyremove":
                try:
                    self.program.getFloatingResources(queryset=True).get(
                        id=data.get("id")
                    )
                except (ValueError, Resource.DoesNotExist):
                    return (
                        HttpResponseBadRequest("Invalid or missing equipment ID."),
                        {},
                    )
    ...
    def resources_equipment_import(self, request, tl, one, two, module, extra, prog):
        ...
        import_form = EquipmentImportForm(
            request.POST, cur_prog=prog, auto_id="equipment_import_%s"
        )
        if not import_form.is_valid():
            ...
            if complete_availability:
                ...
                for equipment in past_program.getFloatingResources():
                    ...
                    for timeslot in self.program.getTimeSlots():
                        res_types = ResourceType.objects.filter(
                            name=res_type.name, program=self.program
                        )
                        if res_types.exists():
                            new_res_type = res_types[0]
                        else:
                        ...
                        if (
                            import_mode == "save"
                            and not Resource.objects.filter(
                                name=new_equip.name, event=new_equip.event
                            ).exists()
                            and str(equipment.id) in to_import
                        ):
                            new_equip.save()
                        else:
            ...
            if import_mode == "preview":
                ...
                context["new_equipment"] = [
                    result[key] for key in sorted(result.keys())
                ]
                response = render_to_response(
                    self.baseDir() + "equipment_import.html", request, context
                )
            else:
                extra = "equipment"
    ...
    def resources(self, request, tl, one, two, module, extra, prog):
        ...
        if extra in handlers:
            (response, context) = handlers[extra](
        ...
        time_options = self.program.getTimeSlots(
            types=["Class Time Block", "Open Class Time Block", "Compulsory"]
        )
        time_groups = self.program.getTimeGroups(
            types=["Class Time Block", "Open Class Time Block", "Compulsory"]
        )
        ...
        res_types = self.program.getResourceTypes(
            include_global=Tag.getBooleanTag("allow_global_restypes")
        )
        context["resource_types"] = sorted(
            res_types, key=lambda x: (not x.hidden, x.priority_default), reverse=True
        )
        ...
        return render_to_response(
            self.baseDir() + "resource_main.html", request, context
        )
    ...
    def newassignment(self, request, tl, one, two, module, extra, prog):
        ...
        if (
            request.method != "POST"
            or "resource" not in request.POST
        ...
        for ts in section.meeting_times.all():
            avail_res = [
                res
                for res in prog.getAvailableResources(ts, queryset=True)
        ...
        response = json.dumps(
            {
                "assignment_name": render_to_string(
                    self.baseDir() + "assignment_name.html",
                    context=context,
                    request=request,
                ),
            }
    ...
    def editassignment(self, request, tl, one, two, module, extra, prog):
        ...
        if (
            ...
            or "returned" not in request.POST
        ):
            return HttpResponseBadRequest("")
        ...
        assignments = ResourceAssignment.objects.filter(
            assignment_group=results[0].assignment_group
        )
        ...
        context = {"assignment": assignment}
        return render_to_response(
            self.baseDir() + "assignment_name.html", request, context
        )
    ...
    def getavailableequipment(self, request, tl, one, two, module, extra, prog):
        ...
        for ts in section.meeting_times.all():
            ts_res_list.append(
                [res.name for res in prog.getAvailableResources(ts, queryset=True)]
            )
    ...
    def isCompleted(self, user=None):
        return (
            self.program.getTimeSlots().exists()
            and self.program.getResourceTypes().exists()
            and self.program.getClassrooms().exists()
        )
