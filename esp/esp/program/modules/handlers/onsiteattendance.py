        ...
        if extra:
            ...
            if len(timeslots) == 1:
                ...
                time_min = datetime.datetime.combine(
                    date, timeslot.start_w_buffer().time()
                )
                ...
                attended = ESPUser.objects.filter(
                    Q(
                        studentregistration__section__meeting_times=timeslot,
                        studentregistration__relationship__name="Attended",
                    )
                    & nest_Q(
                        StudentRegistration.is_valid_qobject(when),
                        "studentregistration",
                    )
                ).distinct()
                # Students that are enrolled in a class during this timeslot on the specified day
                enrolled = ESPUser.objects.filter(
                    Q(
                        studentregistration__section__meeting_times=timeslot,
                ...
                checked_in = (
                    ESPUser.objects.filter(
                        Q(
                            record__event__name="attended",
                            record__program=prog,
                            record__time__lt=time_max,
                        )
                    )
                    .exclude(id__in=checked_out)
                    .distinct()
                )
                # Students that have been checked in for the program during this timeslot on the specified day
                checked_in_during_ts = ESPUser.objects.filter(
                    Q(
                        record__event__name="attended",
                        record__program=prog,
                        record__time__range=(time_min, time_max),
                    )
                ).distinct()
                ...
                enrolled_srs = {
                    sr.user: sr.section
                    for sr in StudentRegistration.valid_objects(when)
                    .filter(
                        section__meeting_times=timeslot, relationship__name="Enrolled"
                    )
                    .select_related("user")
                }
                ...
                onsite_srs = {
                    ...
                    .filter(
                        ...
                        relationship__name="OnSite/AttendedClass",
                    )
                    .select_related("user")
                }
                ...
                attended_srs = {
                    ...
                    for sr in StudentRegistration.valid_objects(when)
                    .filter(
                        section__meeting_times=timeslot, relationship__name="Attended"
                    )
                    .select_related("user")
                }
                ...
                no_attendance = ClassSection.objects.filter(
                    meeting_times=timeslot, status__gt=0
                ).exclude(
                    id__in=StudentRegistration.objects.filter(
                        start_date__date=when.date(),
                        section__meeting_times=timeslot,
            ...
            timess = [
                (
                    "checked in to the program",
                    [(1, time) for time in self.times_checked_in(prog)],
                    True,
                ),  # cumulative
                (
                    "attended a class",
                    [(len(att_dict[time]), time) for time in att_keys],
                    False,
                ),  # not cumulative
            ]
            timess_data, start = BigBoardModule.make_graph_data(timess)
            context["left_axis_data"] = [
                {"axis_name": "# students", "series_data": timess_data}
            ]
    ...
    def times_attending_class(self, prog):
        ...
        for sr in srs:
            ...
            end_time = section_end_dt.replace(
                year=sr.start_date.year,
                month=sr.start_date.month,
                day=sr.start_date.day,
                minute=0,
                second=0,
                microsecond=0,
            )
        ...
        if len(timeslots) == 1:
            ...
            context["sched_sections"] = (
                ClassSection.objects.filter(
                    parent_class__parent_program=prog, meeting_times=timeslot
                )
                .distinct()
                .order_by("id")
            )
            ...
            if len(sections) == 1:
                section = sections[0]
                context["section"], context["not_found"] = (
                    TeacherClassRegModule.process_attendance(section, request, prog)
                )

        return render_to_response(
            "program/modules/teacherclassregmodule/section_attendance.html",
            request,
            context,
        )
