...
class FinAidApproveModule(ProgramModuleObj):
    ...
    def finaidapprove(self, request, tl, one, two, module, extra, prog):
        ...
        users_approved = []
        users_error = []
        context["amount_max_dec_help_text"] = FinancialAidGrant._meta.get_field(
            "amount_max_dec"
        ).help_text
        context["percent_help_text"] = FinancialAidGrant._meta.get_field(
            "percent"
        ).help_text
        ...
        if request.method == "POST":
            ...
            for req in reqs:
                ...
                if not approve_blanks:
                    if is_blank(req.household_income) or is_blank(
                        req.extra_explaination
                    ):
                        continue
