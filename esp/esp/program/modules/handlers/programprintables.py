...
class ProgramPrintables(ProgramModuleObj):
    ...
    def teachersbyFOO(
        ...
        if extra and "secondday" in extra:
            ...
            allclasses = prog.sections().filter(
                status=10, parent_class__status=10, meeting_times__isnull=False
            )
    ...
    def teachermoderatorlist(self, request, tl, one, two, module, extra, prog):
        """default list of teachers; function left in for compatibility"""
        return self.teachersbyFOO(
            request, tl, one, two, module, extra, prog, teaching=True, moderating=True
        )
    ...
    def moderatorlist(self, request, tl, one, two, module, extra, prog):
        """default list of teachers; function left in for compatibility"""
        return self.teachersbyFOO(
            request, tl, one, two, module, extra, prog, teaching=False, moderating=True
        )

    @staticmethod
    def cmpsorttime(one, other):
        if (
            one["cls"].meeting_times.count() > 0
            and other["cls"].meeting_times.count() > 0
        ):
            cmp0 = cmp(
                one["cls"].meeting_times.all()[0].start,
                other["cls"].meeting_times.all()[0].start,
            )
        else:
            cmp0 = cmp(
                one["cls"].meeting_times.count(), other["cls"].meeting_times.count()
            )
    ...
    def students_lineitem(self, request, tl, one, two, module, extra, prog):
        ...
        return self.studentsbyFOO(
            ...
            extra,
            prog,
            filt_exp=lambda x: x.id in student_ids,
        )
    ...
    def teachermoderatorschedules(self, request, tl, one, two, module, extra, prog):
        ...
        return render_to_response(
            self.baseDir() + "teachermoderatorschedule.html", request, context
        )
    ...
    def teacherschedules(self, request, tl, one, two, module, extra, prog):
        ...
        return render_to_response(
            self.baseDir() + "teacherschedule.html", request, context
        )
    ...
    def moderatorschedules(self, request, tl, one, two, module, extra, prog):
        ...
        return render_to_response(
            self.baseDir() + "moderatorschedule.html", request, context
        )
    ...
    def get_student_classlist(program, student):
        ...
        classes = [
            cls for cls in classes if cls.parent_program == program and cls.isAccepted()
        ]
        classes.sort()
        return classes
    ...
    def get_teacher_classlist(program, teacher, teaching=True, moderating=False):
        ...
        if moderating:
            classes += [
                cls for cls in teacher.getModeratingSectionsFromProgram(program)
            ]
        classes = [
            cls for cls in classes if cls.meeting_times.exists() and cls.status >= 0
        ]
        classes.sort()
    ...
    def getTranscript(program, student, format="text"):
        ...
        context = {
            "classlist": ProgramPrintables.get_student_classlist(program, student)
        }
    ...
    def studentschedules(
        ...
        if "hackuserids" in request.GET:
            students = list(
                ESPUser.objects.filter(id__in=request.GET["hackuserids"].split(","))
            )
        elif onsite:
            students = [ESPUser.objects.get(id=request.GET["userid"])]
        else:
    ...
    def studentchecklist(self, request, tl, one, two, module, extra, prog):
        ...
        for student in students:
            ...
            studentList.append(
                {
                    "user": student,
                    "paid": Record.user_completed(student, "paid", self.program)
                    or iac.has_paid(in_full=True),
    ...
    def adminbinder(self, request, tl, one, two, module, extra, prog):
        ...
        if extra == "teacher":
            ...
            for teacher in teachers:
                classes = [
                    cls
                    for cls in teacher.getTaughtClasses()
                    if cls.isAccepted() and cls.parent_program == self.program
                ]
                for cls in classes:
                    scheditems.append({"teacher": teacher, "class": cls})

            context["scheditems"] = scheditems
            return render_to_response(
                self.baseDir() + "adminteachers.html", request, context
            )

        if extra == "classid":
            classes = [cls for cls in self.program.classes() if cls.isAccepted()]
            ...
            context["scheditems"] = scheditems
            return render_to_response(
                self.baseDir() + "adminclassid.html", request, context
            )
        ...
        if extra == "timeblock":
            ...
            context["scheditems"] = scheditems
            return render_to_response(
                self.baseDir() + "adminclasstime.html", request, context
            )
    ...
    def all_classes_spreadsheet(self, request, tl, one, two, module, extra, prog):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                csv_headings = [
                    _encode_ascii(converter.field_dict[fieldname])
                ...
                for cls in ClassSubject.objects.filter(parent_program=prog):
                    write_cvs.writerow(
                        [
                            _encode_ascii(converter.fieldvalue(cls, f))
        ...
        for section, timeslist in sections_possible_times:
            ...
            write_csv.writerow(
                ...
                + [
                    ", ".join(
                        [
                            "%s: %s" % (r.res_type.name, r.desired_value)
                ...
                + [
                    "%d--%d"
                    % (section.parent_class.grade_min, section.parent_class.grade_max)
                ]
    ...
    def concise_oktimes_spr(self, request, tl, one, two, module, extra, prog):
        ...
        for section, timeslist in sections_possible_times:
            ...
            for teacher in teachers:
                conflicts.extend(
                    filter(
                        lambda x: x not in conflicts and x != section.parent_class,
                        teacher.getTaughtClassesFromProgram(prog),
                    )
                )
