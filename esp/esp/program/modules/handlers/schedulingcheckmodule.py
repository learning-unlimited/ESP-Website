...
class SchedulingCheckRunner:
    ...
    def _getLunchByDay(self):
        ...
        lunch_timeslots = (
            Event.objects.filter(
                meeting_times__parent_class__parent_program=self.p,
        ...
        for sec in self._all_class_sections():
            ...
            if wrong:
                output.append(
                    {
                        "Section": sec,
                        "Scheduled (hrs)": scheduled_hours,
                        "Expected (hrs)": expected_hours,
                    }
                )

        return self.formatter.format_table(
            output, {"headings": ["Section", "Scheduled (hrs)", "Expected (hrs)"]}
        )
    ...
    def room_capacity_mismatch(
        ...
        for s in self._all_class_sections(include_walkins=False):
            ...
            if len(r) > 0:
                ...
                if (
                    room.num_students < lower_reporting_ratio * size
                    or room.num_students > upper_reporting_ratio * size
                ):
                    l.append(
                        {
                            "Section": str(s),
                            "Class Max/Optimal": size,
                            "Room Max": room.num_students,
                        }
                    )
        return self.formatter.format_table(
            l, {"headings": ["Section", "Class Max/Optimal", "Room Max"]}
        )
    ...
    def _calculate_d_categories(self):
        ...
        lunch_cat = "Lunch"
        if lunch_cat in self.class_categories:
            self.class_categories.remove(lunch_cat)
