...
class TeacherSurveyModule(ProgramModuleObj):
    ...
    permission_types = ("Teacher/Survey",)
    ...
    def teachers(self, QObject=False):
        ...
        if QObject:
            return {
                "teacher_survey": Q(record__program=program)
                & Q(record__event__name=event)
            }
        return {
            "teacher_survey": ESPUser.objects.filter(
    ...
    def isStep(self):
        return (
            Tag.getBooleanTag("teacher_survey_isstep", program=self.program)
            and self.program.getTimeSlots()[0].start < datetime.datetime.now()
            and self.program.getSurveys()
            .filter(category__in=["learn", "teach"])
            .exists()
        )
