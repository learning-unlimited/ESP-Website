...
from esp.program.modules.forms.management import (
    ClassManageForm,
    SectionManageForm,
    ClassCancellationForm,
    SectionCancellationForm,
)
...
class AdminClass(ProgramModuleObj):
    ...
    def getClasses(self):
        return ClassSubject.objects.catalog(
            self.program, force_all=True, order_args_override=["id"]
        )
    ...
    def deletesection(self, request, tl, one, two, module, extra, prog):
        ...
        if request.method == "POST":
            if request.POST.get("sure") == "True":
                try:
                    ...
                    return HttpResponseRedirect(
                        f"/manage/{one}/{two}/manageclass/{extra}"
                    )
                except (ValueError, ClassSection.DoesNotExist):
                    raise ESPError(
                        f"Unable to delete a section.  The section requested was: {request.GET['sec_id']}",
                        log=False,
                    )
        else:
            ...
            return render_to_response(
                self.baseDir() + "delete_confirm.html", request, context
            )
    ...
    def manageclass(self, request, tl, one, two, module, extra, prog):
        ...
        sec_forms = [
            SectionManageForm(self, section=sec, prefix="sec" + str(sec.index()))
            for sec in cls.sections.all().order_by("id")
        ]
        ...
        if request.method == "POST":
            if action == "cancel_cls":
                ...
                if cls_cancel_form.is_valid():
                    ...
                    cls_cancel_form.cleaned_data["target"].cancel(
                        email_students=True,
                        include_lottery_students=cls_cancel_form.cleaned_data[
                            "email_lottery_students"
                        ],
                        text_students=cls_cancel_form.cleaned_data["text_students"],
                        email_teachers=cls_cancel_form.cleaned_data["email_teachers"],
                        explanation=cls_cancel_form.cleaned_data["explanation"],
                        unschedule=cls_cancel_form.cleaned_data["unschedule"],
                    )
                    return HttpResponseRedirect(
                        request.get_full_path()
                    )  # Other forms may need updating, so just reload this view
                ...
                if sec_cancel_forms.is_valid():
                    ...
                    target_ids = set(
                        cleaned_data["target"].values_list("id", flat=True)
                    )
                    for sec in sections:
                        if not sec.isCancelled() and sec.id in target_ids:
                            sec.cancel(
                                email_students=True,
                                include_lottery_students=cleaned_data[
                                    "email_lottery_students"
                                ],
                                text_students=cleaned_data["text_students"],
                                email_teachers=cleaned_data["email_teachers"],
                                explanation=cleaned_data["explanation"],
                                unschedule=cleaned_data["unschedule"],
                            )
                    return HttpResponseRedirect(
                        request.get_full_path()
                    )  # Other forms may need updating, so just reload this view
    ...
    def approveclass(self, request, tl, one, two, module, extra, prog):
        cls = self.getClass(request, extra)
        cls.accept()
        return self._safe_redirect(
            request, prog.get_manage_url() + "manageclass/" + str(cls.id)
        )
    ...
    def coteachers(self, request, tl, one, two, module, extra, prog):
        ...
        return TeacherClassRegModule.coteachers_logic(
            cls, request, prog, self.baseDir() + "coteachers.html", ajax, is_admin=True
        )
    ...
    def editclass(self, request, tl, one, two, module, extra, prog):
        ...
        for mod in module_list:
            if isinstance(mod, TeacherClassRegModule):
                return mod.makeaclass_logic(
                    request, tl, one, two, module, extra, prog, cls, action=action
                )
    ...
    def classavailability(self, request, tl, one, two, module, extra, prog):
        ...
        for time in time_options:
            ...
            if (
                len(unavail_teachers[time])
                + len(teaching_teachers[time])
                + len(moderating_teachers[time])
            ) == 0:
        ...
        context["is_overbooked"] = sum(
            [sec.duration for sec in cls.get_sections()]
        ) > sum(
            [
                Event.total_length(events).seconds / 3600.0
                for events in Event.group_contiguous(
                    viable_times,
                    int(
                        Tag.getProgramTag(
        ...
        return render_to_response(
            self.baseDir() + "classavailability.html", request, context
        )
