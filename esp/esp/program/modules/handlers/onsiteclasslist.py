...
class OnSiteClassList(ProgramModuleObj):
    ...
    def catalog_status(self, request, tl, one, two, module, extra, prog):
        ...
        # Fetch sections with prefetch to avoid N+1 queries when computing capacity
        sections_qs = (
            ClassSection.objects.filter(parent_class__parent_program=prog, status__gt=0)
            .select_related(
                "parent_class",
        ...
        for s in sections_qs:
            event_ids = [e.id for e in s.meeting_times.all()]
            sections.append(
                {
                    "id": s.id,
                    "parent_class__id": s.parent_class_id,
                    "enrolled_students": s.enrolled_students,
                    "event_ids": event_ids,
                    "registration_status": s.registration_status,
                    "capacity": s._get_capacity_prefetched(),
                }
            )
        ...
        data = {
            ...
            "classes": list(
                ClassSubject.objects.filter(parent_program=prog, status__gt=0)
                .extra(
                    {
                        "teacher_names": """array_to_string(ARRAY(SELECT auth_user.first_name || ' ' || auth_user.last_name FROM auth_user,program_class_teachers WHERE program_class_teachers.classsubject_id=program_class.id AND auth_user.id=program_class_teachers.espuser_id), ', ')""",
                        "class_size_max_optimal": """SELECT program_classsizerange.range_max FROM program_classsizerange WHERE program_classsizerange.id = optimal_class_size_range_id""",
                    }
                )
                .values(
                    "id",
                    "class_size_max",
                    "class_size_max_optimal",
                    "class_info",
                    "prereqs",
                    "hardness_rating",
                    "grade_min",
            ...
            "timeslots": list(
                ...
                .extra(
                    {
                        "start_millis": """EXTRACT(EPOCH FROM start) * 1000""",
                        "label": """to_char("start", 'Dy HH:MI -- ') || to_char("end", 'HH:MI AM')""",
                    }
                )
                .values_list("id", "label", "start_millis")
                .order_by("start")
            ),
            "categories": list(
                prog.class_categories.all()
                .order_by("-symbol")
                .values("id", "symbol", "category")
            ),
        }
    ...
    def students_status(self, request, tl, one, two, module, extra, prog):
        ...
        if search_query:
            ...
            for token in search_tokens:
                search_qset = search_qset & (
                    Q(last_name__icontains=token) | Q(first_name__icontains=token)
                )
        ...
        students = (
            students.values_list("id", "last_name", "first_name")
            .distinct()
        ...
        if success:
            with transaction.atomic():
                ...
                IndividualAccountingController.updatePaid(
                    self.program, student, paid=True
                )
    ...
    def counts_status(self, request, tl, one, two, module, extra, prog):
        resp = HttpResponse(content_type="application/json")
        data = ClassSection.objects.filter(
            status__gt=0, parent_class__status__gt=0, parent_class__parent_program=prog
        ).values_list("id", "enrolled_students", "attending_students")
        json.dump(list(data), resp)
        return resp
    ...
    def full_status(self, request, tl, one, two, module, extra, prog):
        resp = HttpResponse(content_type="application/json")
        data = [
            [section.id, section.isFull(webapp=True)]
            for section in ClassSection.objects.filter(
                status__gt=0,
    ...
    def rooms_status(self, request, tl, one, two, module, extra, prog):
        ...
        data = (
            ClassSection.objects.filter(
                ...
                parent_class__status__gt=0,
                parent_class__parent_program=prog,
                resourceassignment__resource__res_type__name="Classroom",
            )
            .select_related("resourceassignment__resource__name")
            .values_list(
                "id",
                "resourceassignment__resource__name",
                "resourceassignment__resource__num_students",
            )
        )
        json.dump(list(data), resp)
        return resp
    ...
    def get_schedule_json(self, request, tl, one, two, module, extra, prog):
        ...
        if result["user"]:
            result["user_grade"] = ESPUser.objects.get(id=result["user"]).getGrade(
                program=prog
            )
    ...
    def update_schedule_json(self, request, tl, one, two, module, extra, prog):
        ...
        try:
            ...
            result["messages"].append(
                f"Error: could not parse requested sections {request.GET.get('sections', None)}"
            )
            desired_sections = None

        #   Check in student if not currently checked in, since if they're using this view they must be onsite
        if (
            request.GET.get("check_in") == "true"
            and user
            and not prog.isCheckedIn(user)
        ):
        ...
        if user and desired_sections is not None:
            ...
            for sec in sections_to_add:
                if sec.isFull(webapp=True) and not override_full:
                    result["messages"].append(
                        f"Failed to add {user.name()} ({user.id}) to {sec.emailcode()}: {sec.title()} ({sec.id}).  Error was: Class is currently full."
                    )
            ...
            if len(failed_add_sections) == 0:
                ...
                for sec in sections_to_remove:
                    ...
                    result["messages"].append(
                        f"Removed {user.name()} ({user.id}) from {sec.emailcode()}: {sec.title()} ({sec.id})"
                    )

                #   Remove sections that conflict with those the student wants into
                sec_times = (
                    sections_to_add.select_related("meeting_times__id")
                    ...
                    if ts and ts in sm.map and len(sm.map[ts]) > 0:
                        ...
                        for sm_sec in sm.map[ts]:
                            if sm_sec.id not in sections_to_add_ids:
                                ...
                                result["messages"].append(
                                    f"Removed {user.name()} ({user.id}) from {sm_sec.emailcode()}: {sm_sec.title()} ({sm_sec.id})"
                                )
                            else:
                ...
                for sec in sections_to_add:
                    if (
                        sec not in existing_sections
                        and sec.id not in failed_add_sections
                    ):
                        ...
                        if reg_result:
                            result["messages"].append(
                                f"Added {user.name()} ({user.id}) to {sec.emailcode()}: {sec.title()} ({sec.id})"
                            )
                        else:
                            result["messages"].append(
                                f"Failed to add {user.name()} ({user.id}) to {sec.emailcode()}: {sec.title()} ({sec.id}).  Error was: {error}"
                            )
    ...
    def printschedule_status(self, request, tl, one, two, module, extra, prog):
        ...
        result["message"] = (
            f"Submitted {user_obj.name()}'s schedule for printing (print request #{req.id})."
        )
    ...
    def schedule_pdf(self, request, tl, one, two, module, extra, prog):
        ...
        try:
            ...
            return HttpResponse(
                "Error: Could not find user.", status=400, content_type="text/plain"
            )

        return ProgramPrintables.get_student_schedules(
            request, [user_obj], prog, onsite=False
        )
    ...
    def classchange_grid(self, request, tl, one, two, module, extra, prog):
        ...
        open_class_category = dict(
            [(k, getattr(open_class_category, k)) for k in ["id", "symbol", "category"]]
        )
    ...
    def classList(self, request, tl, one, two, module, extra, prog):
        return self.classList_base(
            request, tl, one, two, module, extra, prog, template_name="classlist.html"
        )
    ...
    def classlist_public(self, request, tl, one, two, module, extra, prog):
        return self.classList_base(
            ...
            prog,
            options={},
            template_name="allclass_fragment.html",
        )
        ...
        if options is None:
            ...
            if len(list(options.keys())) == 0:
                return render_to_response(
                    self.baseDir() + "classlist_options.html", request, {"prog": prog}
                )

        context = {}
        defaults = {"refresh": 120, "scrollspeed": 1}
        for key_option in defaults:
        ...
        if curtime:
            ...
            if sort_spec == "unsorted":
                ...
                classes = classes.order_by(
                    "begin_time", "parent_class__category", "id"
                ).distinct()
            else:
                classes = classes.order_by(
                    "parent_class__category", "begin_time", "id"
                ).distinct()
    ...
    def allClassList(self, request, tl, one, two, module, extra, prog):
        ...
        classes = sorted(
            [
                (i.num_students() // (i.class_size_max + 1), i)
        ...
        return render_to_response(
            self.baseDir() + "allclasslist.html",
            request,
            {
                "classes": classes,
                "prog": self.program,
    ...
    def makeLink(self):
        calls = [
            ("classchange_grid", "Grid-based Class Changes Interface"),
            ("classList", "Scrolling Class List"),
            (self.get_main_view(), self.module.link_title),
        ]
        strings = [
            f'<a href="/{self.module.module_type}/{self.program.url}/{call[0]}" title="{call[1]}" class="vModuleLink" >{call[1]}</a>'
            for call in calls
        ]
