...
class StudentRegPhaseZeroManage(ProgramModuleObj):
    doc = (
        """Track registration for the student lottery and/or run the student lottery."""
    )
        ...
        if post.get("mode") == "default":
            ...
            if grade_caps:
                ...
                for i in records:
                    ...
                    for j in sibs:
                        ...
                        # Find which grade range this student belongs to
                        grade_keys = [key for key in grade_caps if grade in key]
                        if len(grade_keys) == 1:
                            newcounts[grade_keys[0]] += 1
                        elif len(grade_keys) == 0:
                            messages["error"].append(
                                f"The <i>program_size_by_grade</i> <a href='/manage/{prog.getUrlBase()}/tags/learn'>tag</a> does not include grade {grade}. Lottery not run."
                            )
                            break
                        else:
                            messages["error"].append(
                                f"The <i>program_size_by_grade</i> <a href='/manage/{prog.getUrlBase()}/tags/learn'>tag</a> includes grade {grade} multiple times. Lottery not run."
                            )
                            break
                ...
                messages["error"].append(
                    f"<i>program_size_by_grade</i> <a href='/manage/{prog.getUrlBase()}/tags'>tag</a> is not set. Lottery not run."
                )

        elif post.get("mode") == "manual":
            usernames = [
                _f for _f in re.split(r"[;,\s]\s*", post.get("usernames")) if _f
            ]
            ...
            for username in usernames:
                ...
                if student.isStudent():
                    records = PhaseZeroRecord.objects.filter(
                        program=prog, user=student
                    ).order_by("time")
                    if records.count() > 0:
                        if post.get("groups"):
            ...
            messages["error"].append(
                "Lottery mode " + post.get("mode") + " is not supported"
            )
    ...
    def phasezero(self, request, tl, one, two, module, extra, prog):
        ...
        if request.POST:
            if request.POST.get("mode") == "addnew":
                ...
                in_lottery = PhaseZeroRecord.objects.filter(
                    user=add_user, program=prog
                ).exists()
                if in_lottery:
                    context["error"] = (
                        f"Error - {add_user.name()} is already in the lottery."
                    )
                else:
                    ...
                    context["success"] = (
                        f"{add_user.name()} has been added to the lottery."
                    )
                ...
                try:
                    ...
                    context["error"] = (
                        f"Error - {join_user.name()} is not in an existing lottery group."
                    )
                else:
                    in_lottery = PhaseZeroRecord.objects.filter(
                        user=add_user, program=prog
                    ...
                    if in_lottery:
                        old_rec = PhaseZeroRecord.objects.get(
                            user=add_user, program=prog
                        )
                    ...
                    if num_users < num_allowed_users:
                        ...
                        if in_lottery:
                            ...
                            if not old_rec.user.exists():
                                old_rec.delete()
                            context["success"] = (
                                f"{add_user.name()} has been moved to a different lottery group."
                            )
                        context["success"] = (
                            f"{add_user.name()} has been added to the lottery group."
                        )
                    else:
                        context["error"] = (
                            f"Error - This group already contains the maximum number of students ({num_allowed_users})."
                        )
                ...
                try:
                    ...
                    context["error"] = (
                        f"Error - {remove_user.name()} is not in the lottery."
                    )
                else:
                    ...
                    if not rec.user.exists():
                        rec.delete()
                    context["success"] = (
                        f"{remove_user.name()} has been removed from the lottery."
                    )
                ...
                if request.POST.get("mode") == "undo":
                    if "confirm" in request.POST:
                        ...
                        context["lottery_succ_msg"] = [
                            "The student lottery has been undone."
                        ]
                    else:
                        context["error"] = (
                            "You did not confirm that you would like to undo the lottery."
                        )
                else:
            ...
            # Run lottery if requested
                ...
                if "confirm" in request.POST:
                    ...
                    context["error"] = (
                        "You did not confirm that you would like to run the lottery"
                    )
        ...
        timess = [
            (
                "number of lottery students",
                [(rec.user.count(), rec.time) for rec in recs],
                True,
            )
        ]
        ...
        if Tag.getBooleanTag("student_lottery_run", prog):
            ...
            for grade in stats:
                if stats[grade]["in_lottery"] == 0:
                    ...
                    stats[grade]["per_accepted"] = (
                        round(stats[grade]["num_accepted"], 1)
                        // stats[grade]["in_lottery"]
                        * 100
                    )
        context["stats"] = stats
