...
class JSONDataModule(ProgramModuleObj, CoreModule):
    ...
    def _bulk_availability(prog, user_ids, subtract_moderation=False):
        ...
        if subtract_moderation:
            ...
            mod_times = ClassSection.objects.filter(
        ...
        return {
            "sections": list(
                ClassSection.objects.filter(
                    ...
                    parent_class__parent_program=prog,
                ).values("id", "enrolled_students")
            )
        }

    counts.method.cached_function.depend_on_row(
        ClassSection, lambda sec: {"prog": sec.parent_class.parent_program}
    )
    counts.method.cached_function.depend_on_row(
    ...
    def moderators(prog):
        ...
        moderators = ESPUser.objects.filter(
            Q(moderatorrecord__program=prog, moderatorrecord__will_moderate=True)
            | Q(moderating_sections__parent_class__parent_program=prog)
        )
        moderator_list = []
        for m in moderators:
            ...
            if rec.exists():
                mod_dict.update(
                    {
                        "will_moderate": rec[0].will_moderate
                        if rec.exists()
                        else False,
                        "num_slots": rec[0].num_slots if rec.exists() else 0,
                        "categories": [c.id for c in rec[0].class_categories.all()]
                        if rec.exists
                        else [],
    ...
    moderators.method.cached_function.depend_on_m2m(
        ClassSection,
        "moderators",
        lambda sec, moderator: {"prog": sec.parent_class.parent_program},
    )
    moderators.method.cached_function.depend_on_row(
        ModeratorRecord, lambda rec: {"prog": rec.program}
    )
    moderators.method.cached_function.depend_on_m2m(
        ModeratorRecord, "class_categories", lambda rec, cat: {"prog": rec.program}
    )
    moderators.method.cached_function.depend_on_row(
        UserAvailability,
    ...
    def categories(prog):
        ...
        if prog.open_class_registration:
            categories = categories.union(
                ClassCategories.objects.filter(pk=prog.open_class_category.pk)
            )
        if len(categories) == 0:
    ...
    categories.cached_function.depend_on_m2m(
        Program, "class_categories", lambda prog, cat: {"prog": prog}
    )
    ...
    def resource_types(prog):
        res_types = prog.getResourceTypes(
    ...
    schedule_assignments.method.cached_function.depend_on_row(
        ClassSection, lambda sec: {"prog": sec.parent_class.parent_program}
    )
    schedule_assignments.method.cached_function.depend_on_row(
        ResourceAssignment, lambda ra: {"prog": ra.target.parent_class.parent_program}
    )
    ...
    def timeslots(prog):
        timeslots = list(
            ...
            .extra(
                {
                    "label": """to_char("start", 'Dy MM/DD HH:MI -- ') || to_char("end", 'HH:MI AM')"""
                }
            )
            .values("id", "short_description", "label", "start", "end")
        )
        for i in range(len(timeslots)):
            ...
            timeslots[i]["starting_sections"] = list(
                ClassSection.objects.exclude(meeting_times__in=timeslots_before)
                .filter(meeting_times=timeslots[i]["id"])
                .order_by("id")
                .values_list("id", flat=True)
            )
            timeslots[i]["sections"] = list(
                ClassSection.objects.filter(meeting_times=timeslots[i]["id"])
                .order_by("id")
                .values_list("id", flat=True)
            )
    ...
    timeslots.cached_function.depend_on_row(
        Event, lambda e: {"prog": e.program} if e.program_id else {}
    )
    timeslots.cached_function.depend_on_m2m(
        ClassSection,
        "meeting_times",
    ...
    lunch_timeslots.cached_function.depend_on_m2m(
        ClassSection,
        "meeting_times",
        lambda sec, event: {"prog": sec.parent_class.parent_program},
    )
    ...
    sections.cached_function.depend_on_row(
        ClassSection, lambda sec: {"prog": sec.parent_class.parent_program}
    )
    sections.cached_function.depend_on_m2m(
        ClassSection,
        "moderators",
        lambda sec, moderator: {"prog": sec.parent_class.parent_program},
    )
    sections.cached_function.depend_on_row(
        ClassSubject, lambda subj: {"prog": subj.parent_program}
    )
    sections.cached_function.depend_on_row(
        UserAvailability,
        lambda ua: {"prog": ua.event.program} if ua.event.program_id else {},
    ...
    sections.cached_function.depend_on_cache(
        ClassSubject.get_teachers,
        lambda self=wildcard, **kwargs: {"prog": self.parent_program},
    )
    sections.cached_function.depend_on_cache(
        ClassSection.friendly_times,
        lambda self=wildcard, **kwargs: {
            "prog": self.parent_class.parent_program,
            "extra": "catalog",
        },
    )
    sections.cached_function.depend_on_cache(
        ClassSection._get_capacity,
        lambda self=wildcard, **kwargs: {
            "prog": self.parent_class.parent_program,
            "extra": "catalog",
        },
    ...
    sections_admin.method.cached_function.depend_on_cache(
        sections.cached_function,
        lambda extra=wildcard, prog=wildcard, **kwargs: {"prog": prog, "extra": extra},
    )
    sections_admin.method.cached_function.get_or_create_token(("prog",))
    sections_admin.method.cached_function.depend_on_row(
        ResourceRequest,
        lambda rr: (
            {"prog": rr.target.parent_class.parent_program}
            if rr.target_id
            else {"prog": rr.target_subj.parent_program}
            if rr.target_subj_id
            else {}
        ),
    )
    sections_admin.method.cached_function.depend_on_row(
        ClassFlag, lambda cf: {"prog": cf.subject.parent_program}
    )
    ...
    def classes_timeslot(extra, prog):
        ...
        for cls in subjects:
            ...
            for sec in cls.get_sections():
                ...
                if meeting_times.count() > 0 and meeting_times[0].id == timeslot_id:
                    section_ids.append({"id": sec.id})
                    added = True
            if added:
                subject_ids.append({"id": cls.id})

        return {"timeslot_sections": section_ids, "timeslot_subjects": subject_ids}

    classes_timeslot.cached_function.get_or_create_token(("prog",))
    classes_timeslot.cached_function.depend_on_row(
        Event, lambda e: {"prog": e.program} if e.program_id else {}
    )
    classes_timeslot.cached_function.depend_on_m2m(
        ClassSection,
        "meeting_times",
        lambda sec, event: {
    ...
    def class_subjects(extra, prog):
        ...
        moderators = []
        classes = []
        qs = prog.classes().prefetch_related("category", "sections", "teachers")
        ...
        for c in qs:
            ...
            if catalog:
                ...
                cls["difficulty_description"] = (
                    difficulty_map.get(str(c.hardness_rating))
                    if c.hardness_rating
                    else None
                )
    ...
    class_subjects.cached_function.depend_on_row(
        ClassSubject, lambda cls: {"prog": cls.parent_program}
    )
    class_subjects.cached_function.depend_on_cache(
        ClassSubject.get_teachers,
        lambda cls=wildcard, **kwargs: {"prog": cls.parent_program},
    )
    ...
    def interested_classes(self, request, tl, one, two, module, extra, prog):
        ...
        return {"interested_subjects": subject_ids, "interested_sections": section_ids}
    ...
    def class_info(self, request, tl, one, two, module, extra, prog):
        ...
        if "section_id" in request.GET:
            ...
            raise ESPError(
                "Need a section or subject ID to fetch catalog info", log=False
            )
        ...
        section_info = []
        priority_names = list(
            RegistrationType.objects.filter(name__startswith="Priority").values_list(
                "name", flat=True
            )
        )
        for sec in cls.get_sections():
    ...
    class_info.cached_function.depend_on_row(
        ClassSubject, lambda cls: {"prog": cls.parent_program}
    )
    class_info.cached_function.depend_on_row(
        ClassSection, lambda sec: {"prog": sec.parent_class.parent_program}
    ...
    def class_size_info(self, request, tl, one, two, module, extra, prog):
        ...
        if return_key == "sections":
            ...
            matching_classes = ClassSubject.objects.catalog_cached(
                prog, initial_queryset=target_qs
            )
        ...
        return_dict = {
            ...
            "optimal_class_size_ranges": cls.optimal_class_size_range.range_str()
            if cls.optimal_class_size_range
            else None,
            "allowable_class_size_ranges": [
                cr.range_str() for cr in cls.get_allowable_class_size_ranges()
            ],
        }
    ...
    def class_admin_info(self, request, tl, one, two, module, extra, prog):
        ...
        return_dict = {
            ...
            "moderator_title": Tag.getProgramTag("moderator_title", prog).capitalize()
            + "s",
            "resource_requests": rrequest_dict,
            "comments": cls.message_for_directors,
            "special_requests": cls.requested_special_resources,
    ...
    def class_nums(prog):
        ...
        class_num_list.append(
            (
                "Total # of Classes Scheduled",
                classes.filter(sections__meeting_times__isnull=False)
                .distinct()
                .count(),
            )
        )
        class_num_list.append(
            ("Total # of Class Sections", sections.distinct().count())
        )
        class_num_list.append(
            (
                "Total # of Class Sections Scheduled",
                sections.filter(meeting_times__isnull=False).distinct().count(),
            )
        )
        class_num_list.append(
            (
                "Total # of Lunch Classes",
                classes.filter(category__is_lunch=True, status=ClassStatus.ACCEPTED)
                .distinct()
                .count(),
            )
        )
        class_num_list.append(
            (
                ...
                classes.filter(status=ClassStatus.UNREVIEWED)
        ...
        class_num_list.append(
            (
                ...
                classes.filter(
                    status=ClassStatus.ACCEPTED, sections__status=ClassStatus.ACCEPTED
                )
                .exclude(category__is_lunch=True)
                .distinct()
                .count(),
        ...
        class_num_list.append(
            (
                ...
                classes.filter(status=ClassStatus.REJECTED)
                .exclude(category__is_lunch=True)
                .distinct()
                .count(),
            )
        ...
        class_num_list.append(
            (
                ...
                classes.filter(status=ClassStatus.CANCELLED)
                .exclude(category__is_lunch=True)
                .distinct()
                .count(),
            )
    ...
    class_nums.depend_on_row(
        ClassSection, lambda sec: {"prog": sec.parent_class.parent_program}
    )
    class_nums.depend_on_m2m(
        ClassSection,
        "meeting_times",
        lambda sec, event: {"prog": sec.parent_class.parent_program},
    )
    ...
    def flags_nums(prog):
        ...
        for ft in ClassFlagType.get_flag_types(prog):
            flags_num_list.append(
                (
                    'Total # of Classes with the <i><span style="color: %s;">%s</span></i> flag'
                    % (ft.color, ft.name),
                    classes.filter(flags__flag_type=ft, flags__resolved=False)
                    .distinct()
                    .count(),
                )
            )
    ...
    flags_nums.depend_on_row(
        ClassFlag, lambda flag: {"prog": flag.subject.parent_program}
    )
    ...
    def mod_nums(prog):
        ...
        if "will_moderate" in teachers:
            moderator_list.append(
                (
                    "Teachers who have offered to be a "
                    + Tag.getProgramTag("moderator_title", prog).lower(),
                    teachers["will_moderate"].count(),
                )
            )
        if "assigned_moderator" in teachers:
            moderator_list.append(
                (
                    Tag.getProgramTag("moderator_title", prog).capitalize()
                    + "s who have been assigned to sections",
                    teachers["assigned_moderator"].count(),
                )
            )
        moderator_list.append(
            (
                "Total number of time blocks offered by "
                + Tag.getProgramTag("moderator_title", prog).lower()
                + "s",
                ModeratorRecord.objects.filter(program=prog).aggregate(
                    Sum("num_slots")
                )["num_slots__sum"]
                or 0,
            )
        ...
        moderator_list.append(
            (
                "Total number of time blocks assigned "
                + Tag.getProgramTag("moderator_title", prog).lower()
                + "s",
                ClassSection.objects.filter(
                    parent_class__parent_program=prog, moderators__isnull=False
                )
                .distinct()
                .aggregate(Count("meeting_times"))["meeting_times__count"],
            )
        )
        moderator_list.append(
            (
                "Total number of sections assigned "
                + Tag.getProgramTag("moderator_title", prog).lower()
                + "s",
                ClassSection.objects.filter(
                    parent_class__parent_program=prog, moderators__isnull=False
    ...
    def cat_nums(prog):
        ...
        program_categories = (
            ClassCategories.objects.filter(Q_categories)
            .distinct()
            .values_list("id", flat=True)
        )
        annotated_categories = (
            ClassCategories.objects.filter(cls__parent_program=prog, cls__status__gte=0)
            .annotate(
                num_subjects=Count("cls", distinct=True),
                num_sections=Count("cls__sections"),
                num_class_hours=Sum("cls__sections__duration"),
            )
            .order_by("-num_subjects")
            .values("id", "num_sections", "num_subjects", "num_class_hours", "category")
            .distinct()
        )
    ...
    def grade_nums(prog):
        ...
        for g in grades:
            ...
            grade_classes = classes.filter(
                status__gte=0, grade_min__lte=g, grade_max__gte=g
            )
    ...
    grade_nums.depend_on_row(
        ClassSection, lambda sec: {"prog": sec.parent_class.parent_program}
    )
    grade_nums.depend_on_model(
        StudentInfo
    )  # I can't think of a more efficient way to depend on graduation year with depend_on_row -WG
        ...
        for key in teachers.keys():
            ...
            if key not in ["taught_before"]:
                if key in list_labels:
                    ...
                    teacher_num_list.append(
                        (key, teachers[key].filter(is_active=True).distinct().count())
                    )
                # Hack to insert this combined count in a logical position
                if key == "class_submitted":
                    teacher_num_list.append(
                        (
                            """Teachers who have submitted a class and have not taught for a previous program""",
                            ESPUser.objects.filter(
                                ~teachers_qobjects["taught_before"]
                                & teachers_qobjects["class_submitted"]
                                & Q(is_active=True)
                            )
                            .distinct()
                            .count(),
    ...
    teacher_nums.depend_on_row(
        ClassSection, lambda sec: {"prog": sec.parent_class.parent_program}
    )
    teacher_nums.depend_on_m2m(
        ClassSubject, "teachers", lambda cls, teacher: {"prog": cls.parent_program}
    )
    teacher_nums.depend_on_row(
        ResourceRequest, lambda rr: {"prog": rr.target.parent_class.parent_program}
    )
        ...
        for key in students.keys():
            ...
            if key not in ["attended_past", "enrolled_past"]:
                if key in list_labels:
                    ...
                    student_num_list.append(
                        (key, students[key].filter(is_active=True).distinct().count())
                    )
                # Hack to insert this combined count into a logical position
                if key == "enrolled":
                    student_num_list.append(
                        (
                            """Students who are enrolled and have not enrolled in the past""",
                            ESPUser.objects.filter(
                                ~students_qobjects["enrolled_past"]
                                & students_qobjects["enrolled"]
                                & Q(is_active=True)
                            )
                            .distinct()
                            .count(),
    ...
    student_nums.depend_on_row(
        StudentSubjectInterest, lambda ssi: {"prog": ssi.subject.parent_program}
    )
    student_nums.depend_on_row(
        StudentRegistration, lambda sr: {"prog": sr.section.parent_class.parent_program}
    )
    ...
    student_nums.depend_on_row(
        Transfer, lambda trans: {"prog": trans.line_item.program}
    )
    ...
    def volunteer_nums(prog):
        ...
        for key in volunteers.keys():
            if key in list_labels:
                volunteer_num_list.append(
                    (
                        list_labels[key],
                        volunteers[key].filter(is_active=True).distinct().count(),
                    )
                )
            else:
                volunteer_num_list.append(
                    (key, volunteers[key].filter(is_active=True).distinct().count())
                )
        return volunteer_num_list

    volunteer_nums.depend_on_row(
        VolunteerOffer, lambda vo: {"prog": vo.request.program}
    )
    ...
    def calc_hours(classes):
        hours = {
            "class-hours": 0,
            "class-student-hours": 0,
            "class-registered-hours": 0,
            "class-checked-in-hours": 0,
        }
        for cls in classes:
            if cls["subject_duration"]:
                hours["class-hours"] += float(cls["subject_duration"])
                hours["class-student-hours"] += float(cls["subject_duration"]) * (
                    float(cls["class_size_max"]) if cls["class_size_max"] else 0
                )
                hours["class-registered-hours"] += (
                    float(cls["subject_duration"])
                    * float(cls["subject_students"])
                    / float(cls["num_sections"])
                )
                hours["class-checked-in-hours"] += (
                    float(cls["subject_duration"])
                    * float(cls["subject_checked_in_students"])
                    / float(cls["num_sections"])
                )
        return hours
    ...
    def calc_section_hours(sections):
        ...
        for sec in sections:
            if sec["duration"]:
                ...
                hours["class-registered-hours"] += float(sec["duration"]) * float(
    ...
    def hour_nums(prog):
        ...
        reg_classes = (
            ...
            .annotate(
                ...
                subject_students=Sum("sections__enrolled_students"),
            )
        )
        for cls in reg_classes:
            cls.subject_checked_in_students = sum(
                [sec.count_ever_checked_in_students() for sec in cls.get_sections()]
            )
        reg_hours = JSONDataModule.calc_hours(
            [
                {
                    "num_sections": cls.num_sections,
                    "subject_duration": cls.subject_duration,
                    "subject_students": cls.subject_students,
                    "subject_checked_in_students": cls.subject_checked_in_students,
                    "class_size_max": cls.class_size_max,
                }
                for cls in reg_classes
            ]
        ...
        app_classes = (
            prog.classes()
            .filter(status__gt=0, sections__status__gt=0)
            .exclude(category__is_lunch=True)
            .annotate(
                num_sections=Count("sections"),
                subject_duration=Sum("sections__duration"),
        ...
        app_hours = JSONDataModule.calc_hours(
            [
                {
                    "num_sections": cls.num_sections,
                    "subject_duration": cls.subject_duration,
        ...
        sched_sections = (
            ...
            .values("duration", "enrolled_students", "id")
        )
        sched_hours = JSONDataModule.calc_section_hours(sched_sections)
        hour_num_list = []
        hour_num_list.append(
            ("Total # of Class-Hours (registered)", round(reg_hours["class-hours"], 2))
        )
        hour_num_list.append(
            ("Total # of Class-Hours (approved)", round(app_hours["class-hours"], 2))
        )
        hour_num_list.append(
            ("Total # of Class-Hours (scheduled)", round(sched_hours["class-hours"], 2))
        )
        hour_num_list.append(
            (
                "Total # of Class-Student-Hours (registered)",
                round(reg_hours["class-student-hours"], 2),
            )
        )
        hour_num_list.append(
            (
                "Total # of Class-Student-Hours (approved)",
                round(app_hours["class-student-hours"], 2),
            )
        )
        hour_num_list.append(
            (
                "Total # of Class-Student-Hours (scheduled)",
                round(sched_hours["class-student-hours"], 2),
            )
        )
        hour_num_list.append(
            (
                "Total # of Class-Student-Hours (enrolled)",
                round(reg_hours["class-registered-hours"], 2),
            )
        )
        hour_num_list.append(
            (
                "Total # of Class-Student-Hours (attended program)",
                round(reg_hours["class-checked-in-hours"], 2),
            )
        )
        if sched_hours["class-student-hours"]:
            hour_num_list.append(
                (
                    "Class-Student-Hours Utilization",
                    str(
                        round(
                            100
                            * reg_hours["class-registered-hours"]
                            / sched_hours["class-student-hours"],
                            2,
                        )
                    )
                    + "%",
    ...
    hour_nums.depend_on_row(
        ClassSection, lambda sec: {"prog": sec.parent_class.parent_program}
    )
    hour_nums.depend_on_m2m(
        ClassSection,
        "meeting_times",
        lambda sec, event: {"prog": sec.parent_class.parent_program},
    )
    ...
    def timeslots_nums(prog):
        ...
        for timeslot in timeslots:
            ...
            def student_count(clslist):
                lst = [0] + [
                    x.num_students() for x in clslist if not x.category.is_lunch
                ]
    ...
    timeslots_nums.depend_on_row(
        ClassSection, lambda sec: {"prog": sec.parent_class.parent_program}
    )
    timeslots_nums.depend_on_m2m(
        ClassSection,
        "meeting_times",
        lambda sec, event: {"prog": sec.parent_class.parent_program},
    )
    ...
    def stats(self, request, tl, one, two, module, extra, prog):
        ...
        shirt_data["moderator_title"] = Tag.getProgramTag(
            "moderator_title", prog
        ).capitalize()
    ...
    def set_donation_amount(self, request, tl, one, two, module, extra, prog):
        ...
        if amount_donation != Decimal("0"):
            ...
            iac.set_preference(
                "Donation to Learning Unlimited", 1, amount=amount_donation
            )
    ...
    def teachers_for_autoscheduler(self, request, tl, one, two, module, extra, prog):
        ...
        teacher_dicts = [
            {"uid": t.id, "text": t.name(), "availability": avail_lookup.get(t.id, [])}
            for t in teachers
        ]
