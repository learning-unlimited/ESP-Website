...
def teacheracknowledgementform_factory(prog):
    ...
    if date_range is None:
        label = (
            "I have read the above and commit to serving as a %s for my %s class(es)."
            % (teach_text, prog.program_type)
        )
    else:
        label = (
            "I have read the above and commit to serving as a %s for my %s class(es) on %s."
            % (teach_text, prog.program_type, date_range)
        )
