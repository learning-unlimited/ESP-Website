...
class StudentLunchSelectionForm(forms.Form):
    ...
    def __init__(self, program, user, day, *args, **kwargs):
        ...
        events_filtered = [x for x in events_all if x.start.date() == self.day]
    ...
    def load_data(self):
        ...
        lunch_registrations = [lunch_registration for lunch_registration in lunch_registrations if list(lunch_registration.section.meeting_times.all())[0].start.date() == self.day]
    ...
    def save_data(self):
        ...
        for section in self.user.getSections(self.program):
            if section.parent_class.category.category == 'Lunch':
                if section.get_meeting_times()[0].start.date() == self.day:
