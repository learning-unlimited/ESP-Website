...
from esp.program.modules.forms.lineitems import (
    OptionFormset,
    LineItemForm,
    LineItemImportForm,
    exclude_line_items,
...
class LineItemsModule(ProgramModuleObj, CoreModule):
    ...
    def lineitems(self, request, tl, one, two, module, extra, prog):
        ...
        if request.GET.get("op") == "edit":
            ...
            context["option_formset"] = OptionFormset(
                queryset=lineitem.lineitemoptions_set.all(), prefix="options"
            )
        elif request.GET.get("op") == "delete":
            # show delete confirmation page
            context["lineitem"] = LineItemType.objects.get(id=request.GET["id"])
            return render_to_response(
                self.baseDir() + "lineitem_delete.html", request, context
            )
            ...
            if not import_form.is_valid():
                ...
                context["lineitems"] = past_program.lineitemtype_set.exclude(
                    text__in=exclude_line_items
                )
                return render_to_response(
                    self.baseDir() + "lineitem_import.html", request, context
                )
            ...
            if request.POST.get("command") == "reallyremove":
                ...
                past_lineitems = past_program.lineitemtype_set.exclude(
                    text__in=exclude_line_items
                )
                ...
                if "id_lineitem" in request.POST:
                    lineitem_form = LineItemForm(
                        request.POST,
                        instance=LineItemType.objects.get(
                            id=request.POST["id_lineitem"]
                        ),
                    )
                else:
                ...
                if lineitem_form.is_valid() and options_formset.is_valid():
                    if (
                        LineItemType.objects.filter(
                            text=lineitem_form.cleaned_data["text"], program=prog
                        ).exists()
                        and "id_lineitem" not in request.POST
                    ):
                        lineitem_form.add_error(
                            "text",
                            "A line item with that name already exists. Please choose another name.",
                        )
        ...
        if "option_formset" not in context:
            context["option_formset"] = OptionFormset(
                queryset=LineItemOptions.objects.none(), prefix="options"
            )
        if "import_lineitem_form" not in context:
            context["import_lineitem_form"] = LineItemImportForm(cur_prog=prog)
        context["lineitems"] = prog.lineitemtype_set.exclude(
            text__in=exclude_line_items
        )  # exclude ones that shouldn't be edited here
    ...
    def isCompleted(self, user=None):
        return self.program.lineitemtype_set.exclude(
            text__in=exclude_line_items
        ).exists()
