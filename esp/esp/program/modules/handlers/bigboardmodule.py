...
class BigBoardModule(ProgramModuleObj):
    ...
    def bigboard(self, request, tl, one, two, module, extra, prog):
        ...
        numbers = [
            ...
            ("students checked in", self.num_checked_in_users(prog)),
            ("students with lottery preferences", self.num_users_with_lottery(prog)),
            ("students enrolled in a class", self.num_users_enrolled(prog)),
            ("students who completed the medical form", self.num_medical(prog)),
            ("priority selections entered", self.num_priority_selections(prog)),
            ("stars entered", self.num_ssis(prog)),
            ("lottery preferences entered", self.num_prefs(prog)),
        ]
        ...
        timess = [
            (
                "completed the medical form",
                [(1, time) for time in self.times_medical(prog)],
                True,
            ),
            (
                "set class lottery preferences",
                [(1, time) for time in self.times_lottery(prog)],
                True,
            ),
            (
                "enrolled in classes",
                [(1, time) for time in self.times_enrolled(prog)],
                True,
            ),
        ]
        ...
        return render_to_response(self.baseDir() + "bigboard.html", request, context)
    ...
    def num_active_users(self, prog, minutes=10):
        ...
        users_with_meds = set(
            Record.objects.filter(program=prog, event__name__in=["med", "med_bypass"])
    ...
    def num_medical(self, prog):
        return Record.objects.filter(
            program=prog, event__name__in=["med", "med_bypass"]
        ).count()

    @cache_function_for(105)
    def checked_in_users(prog):
        return (
            Record.objects.filter(program=prog, event__name="attended")
            .values_list("user", flat=True)
            .distinct()
        )
    ...
    def popular_classes(self, prog):
        ...
        fields = [
            ...
            (
                "number of priority selections",
                "studentregistration",
                sections.filter(
                    studentregistration__relationship__name__startswith="Priority"
                ),
            ),
            (
                ...
                sections.filter(
                    (
                        Q(studentregistration__start_date=None)
                        | Q(
                            studentregistration__start_date__lte=datetime.datetime.now()
                        )
                    )
                    & (
                        Q(studentregistration__end_date=None)
                        | Q(studentregistration__end_date__gte=datetime.datetime.now())
                    )
        ...
        for description, field, qs in fields:
            qs = (
                qs.annotate(points=Count(field))
                .values(
                    "id",
                    "points",
                )
                .exclude(points__lte=0)
                .order_by("-points")
            )
            ...
            # 'category__symbol'.
            qs.query.group_by = [
                column for column in qs.query.group_by if column in qs.query.select
            ]
            qs_list = list(qs)
            if len(qs_list) > 0:
                series = []
                ...
                for sec in qs_list:
                    ...
                    sec_dict["data"] = [
                        [ts.start, sec["points"]] for ts in timeslots if ts in mts
                    ]
    ...
    popular_classes.depend_on_row(
        StudentRegistration,
        lambda sr: {"prog": sr.section.parent_class.parent_program},
        filter=lambda sr: (
            sr.relationship.name.startswith("Priority")
            or sr.relationship.name == "Enrolled"
        ),
    )
    popular_classes.depend_on_row(
        StudentSubjectInterest, lambda ssi: {"prog": ssi.subject.parent_program}
    )
    popular_classes.depend_on_m2m(
        ClassSection,
        "meeting_times",
    ...
    def times_medical(self, prog):
        return list(
            ...
            .annotate(Min("time"))
            .order_by("time__min")
            .values_list("time__min", flat=True)
        )
    ...
    def times_enrolled(self, prog):
        ...
        return list(
            ...
            .values("user")
            .annotate(Min("start_date"))
            .order_by("start_date__min")
            .values_list("start_date__min", flat=True)
        )

    @staticmethod
    def chunk_times(
        times, start, end, delta=datetime.timedelta(0, 3600), cumulative=True
    ):
    ...
    def make_graph_data(timess, drop_beg=0, drop_end=0, cutoff=1):
        ...
        if not timess:
            ...
            start = min(
                [
                    times[drop_beg : (len(times) - drop_end)][0][1]
                    for desc, times, cumulative in timess
                ]
            )
            start = start.replace(hour=0, minute=0, second=0, microsecond=0)
                ...
                [
                    times[drop_beg : (len(times) - drop_end)][-1][1]
                    for desc, times, cumulative in timess
                ]
            )
            ...
            graph_data = [
                {
                    "description": desc,
                    "data": BigBoardModule.chunk_times(
                        times, start, end, cumulative=cumulative
                    ),
                }
                for desc, times, cumulative in timess
            ]
