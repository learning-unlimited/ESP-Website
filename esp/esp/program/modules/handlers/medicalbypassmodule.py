        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if Record.objects.filter(
                    user=user, program=self.program, event__name="med_bypass"
                ).exists():
                    status = "bypass exists"
                elif Record.objects.filter(
                    user=user, program=self.program, event__name="med"
                ).exists():
                    status = "reg bit exists"
                else:
                    rt = RecordType.objects.get(name="med_bypass")
                    Record.objects.create(user=user, program=self.program, event=rt)
                    status = "success"
            else:
                status = "invalid user"
