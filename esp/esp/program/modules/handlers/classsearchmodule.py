...
class ClassSearchModule(ProgramModuleObj):
    ...
    def query_builder(self):
        ...
        flag_datetime_inputs = [
            OptionalInput(
                name=t,
                inner=DatetimeInput(field_name="flags__%s_time" % t, english_name=""),
            )
        ...
        flag_status_input = SelectQInput(
            options=OrderedDict(
                [
                    ("any", {"title": "Any Status", "Q": Q()}),
                    (
                        "unresolved",
                        {"title": "Unresolved", "Q": Q(flags__resolved=False)},
                    ),
        ...
        flag_filter = SearchFilter(
            name="flag",
            title="the flag",
            inputs=[flag_select_input] + flag_datetime_inputs + [flag_status_input],
        )
        any_flag_filter = SearchFilter(
            name="any_flag",
            title="any flag",
            inputs=[any_flag_input] + flag_datetime_inputs + [flag_status_input],
        )

        resource_types = ResourceType.objects.filter(program=self.program)
        resource_value_input = OptionalInput(
            name="desired value",
            inner=TextInput(
                field_name="sections__resourcerequest__desired_value", english_name=""
            ),
        )
        ...
        resource_filter = SearchFilter(
            name="resource",
            title="the requested resource",
            inputs=[resource_select_input] + [resource_value_input],
        )
        any_resource_filter = SearchFilter(
            name="any_resource",
            title="any requested resource",
        ...
        category_filter = SearchFilter(
            ...
            inputs=[
                SelectInput(
                    field_name="category",
                    options={str(cat.id): cat.category for cat in categories},
                )
            ],
        )
        ...
        duration_filter = SearchFilter(
            ...
            inputs=[
                SelectInput(
                    field_name="duration",
                    options=OrderedDict(
                        [(str(duration[0]), duration[1]) for duration in durations]
                    ),
                )
            ],
        )
                ...
                SelectQInput(
                    options=OrderedDict(
                        [
                            (
                                str(grade),
                                {"title": str(grade), "Q": Q(grade_max__gte=grade)},
                            )
                            for grade in grades
                        ]
                    )
                ...
                SelectQInput(
                    options=OrderedDict(
                        [
                            (
                                str(grade),
                                {"title": str(grade), "Q": Q(grade_min__lte=grade)},
                            )
                            for grade in grades
                        ]
                    )
        ...
        sections_filter = SearchFilter(
            name="num_sections",
            title="between X and Y section(s)",
            inputs=[
                SelectQInput(
                    options=OrderedDict(
                        [
                            (
                                str(num),
                                {
                                    "title": str(num),
                                    "Q": Q(
                                        id__in=ClassSubject.objects.annotate(
                                            num_sections=Count("sections")
                                        )
                                        .filter(num_sections__gte=num)
                ...
                SelectQInput(
                    options=OrderedDict(
                        [
                            (
                                str(cap),
                                {"title": str(cap), "Q": Q(class_size_max__gte=cap)},
                            )
                ...
                SelectQInput(
                    options=OrderedDict(
                        [
                            (
                                str(cap),
                                {"title": str(cap), "Q": Q(class_size_max__lte=cap)},
                            )
                            for cap in capacities
                        ]
                    )
                        ...
                        options=OrderedDict(
                            [
                                (
                                    ...
                                    {
                                        "title": str(cap),
                                        "Q": Q(class_size_optimal__gte=cap),
                                    },
                                )
                                for cap in capacities
                            ]
                        ...
                        options=OrderedDict(
                            [
                                (
                                    ...
                                    {
                                        "title": str(cap),
                                        "Q": Q(class_size_optimal__lte=cap),
                                    },
                                )
                                for cap in capacities
                            ]
                            ...
                            [
                                (
                                    str(cap),
                                    {
                                        "title": str(cap),
                                        "Q": Q(
                                            optimal_class_size_range__range_max__gte=cap
                                        ),
                                    },
                                )
                                for cap in capacities
                            ...
                            [
                                (
                                    str(cap),
                                    {
                                        "title": str(cap),
                                        "Q": Q(
                                            optimal_class_size_range__range_min__lte=cap
                                        ),
                                    },
                                )
                                for cap in capacities
                            ...
                            [
                                (
                                    str(cap),
                                    {
                                        "title": str(cap),
                                        "Q": Q(
                                            allowable_class_size_ranges__range_max__gte=cap
                                        ),
                                    },
                                )
                                for cap in capacities
                            ...
                            [
                                (
                                    str(cap),
                                    {
                                        "title": str(cap),
                                        "Q": Q(
                                            allowable_class_size_ranges__range_min__lte=cap
                                        ),
                                    },
                                )
                                for cap in capacities
        ...
        status_filter = SearchFilter(
            ...
            inputs=[
                SelectInput(
                    field_name="status", options={str(k): v for k, v in STATUS_CHOICES}
                )
            ],
        )
        ...
        username_filter = SearchFilter(
            name="username",
            title="teacher username containing",
            inputs=[
                TextInput(field_name="teachers__username__icontains", english_name="")
            ],
        )
        ...
        message_for_directors_filter = SearchFilter(
            name="message_for_directors",
            title="a message to the directors",
        ...
        purchase_request_filter = SearchFilter(
            ...
            title="a purchase request",
            # Get classes with a purchase request
            inverted=True,
            inputs=[
                ConstantInput(
                    Q(purchase_requests__isnull=True) | Q(purchase_requests="")
                )
            ],
        )
        ...
        room_request_filter = SearchFilter(
            name="room_request",
            title="a room request",
            # Get classes with a room request
            inverted=True,
            inputs=[
                ConstantInput(Q(requested_room__isnull=True) | Q(requested_room=""))
            ],
        )
        ...
        hardness_filter = SearchFilter(
            ...
            title="the hardness rating",
            inputs=[
                SelectInput(
                    field_name="hardness_rating",
                    options=OrderedDict(
                        [(hardness[0], hardness[1]) for hardness in hardness_choices]
                    ),
                )
            ],
        )
        ...
        if capacity_inputs:
            capacity_filter = SearchFilter(
                name="capacity",
                title=capacity_title + " between X and Y",
        ...
        if Tag.getTag("class_style_choices"):
            ...
            style_filter = SearchFilter(
                name="class_style",
                title="the class style",
                inputs=[
                    SelectInput(
                        field_name="class_style",
    ...
    def create_autorule(self, request, tl, one, two, module, extra, prog):
        ...
        if not query_data or not flag_type_id:
            return HttpResponseRedirect(
                request.META.get("HTTP_REFERER", "./classsearch")
            )
        ...
        return HttpResponseRedirect(
            "./classsearch?query=" + urllib.parse.quote(query_data)
        )
    ...
    def simple_search_query(self, request, form_context):
        ...
        """
        choices = {
            "s_category": {str(cat.id) for cat in form_context["simple_categories"]},
            "s_status": {
                str(status) for status, label in form_context["simple_status_choices"]
            },
            "s_grade_min": {str(grade) for grade in form_context["simple_grades"]},
        }
        ...
        if value("s_grade_min") or value("s_grade_max"):
            subqueries.append(("grade", [value("s_grade_min"), value("s_grade_max")]))
        ...
        return {
            ...
            "negated": False,
            "values": [
                {"filter": name, "negated": False, "values": values}
    ...
    def classsearch(self, request, tl, one, two, module, extra, prog):
        ...
        if decoded is not None:
            ...
            context["regtypes"] = sorted(
                RegistrationType.objects.all(), key=lambda a: str(a)
            )
