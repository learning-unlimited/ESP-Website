...
_PROGRAM_URL_PATTERN = re.compile(
    r'(?:/learn|/teach|/volunteer)/([^/\s\'"<>]+/[^/\s\'"<>]+)', re.IGNORECASE
)
...
def _make_image_urls_absolute(body, request):
    ...
    def replace_src(m):
        ...
        return "src={}{}{}".format(
            m.group(1), request.build_absolute_uri(m.group(2)), m.group(1)
        )
...
class CommModule(ProgramModuleObj):
    ...
    def get_mailer_warnings(listcount, filterid, sendto_fn_name):
        ...
        if listcount_int >= 2000:
            mailer_warnings.append(
                f"Caution: You are about to send a massive mailer to {listcount_int} recipients."
            )
        ...
        if filter_obj and not CommModule._q_has_grade_filter(filter_obj.get_Q()):
            mailer_warnings.append(
                "Warning: You haven't selected a grade range filter."
            )
        ...
        if sendto_fn_name in guardian_or_emergency_sentto_values:
            mailer_warnings.append(
                "Note: This mailer includes parent and/or emergency contact emails."
            )
    ...
    def commprev(self, request, tl, one, two, module, extra, prog):
        ...
        sendto_fn_name = request.POST.get(
        ...
        if request.POST.get("from", "").strip():
            ...
            if not re.match(
                rf"(^.+@{re.escape(settings.SITE_INFO[1])}$)|(^.+<.+@{re.escape(settings.SITE_INFO[1])}>$)|(^.+@(\w+\.)*learningu\.org$)|(^.+<.+@(\w+\.)*learningu\.org>$)",
                fromemail,
            ):
                raise ESPError(
                    "Invalid 'From' email address. The 'From' email address must "
                    + "end in @"
            ...
            if not prs.exists():
                redirect = PlainRedirect.objects.create(
                    original="info",
                    destination=settings.DEFAULT_EMAIL_ADDRESSES["default"],
                )
        ...
        try:
            ...
            raise ESPError(
                "Corrupted POST data!  Please contact us at"
                + "websupport@learningu.org and tell us how you got this error,"
                + "and we'll look into it."
            )

        userlist = PersistentQueryFilter.getFilterFromID(filterid, ESPUser).getList(
            ESPUser
        )
        ...
        try:
            ...
            raise ESPError(
                "You seem to be trying to email 0 people!  "
        ...
        rendered_text = render_to_string(
            f"email/{template}_email.html", {"msgbody": body}
        )
        ...
        other_program_urls = _program_urls_in_text(
            (subject or "") + " " + (body or ""), current_program_url
        )
    ...
    def commfinal(self, request, tl, one, two, module, extra, prog):
        ...
        if other_program_urls and not request.POST.get(
            "confirm_send_with_other_program_links"
        ):
            rendered_text = render_to_string(
                f"email/{template}_email.html", {"msgbody": body}
            )
    ...
    def commpanel_old(self, request, tl, one, two, module, extra, prog):
        ...
        context["sendto_fn_name"] = request.POST.get(
            "sendto_fn_name", MessageRequest.SEND_TO_SELF_REAL
        )
        context["sendto_fn"] = MessageRequest.assert_is_valid_sendto_fn_or_ESPError(
            context["sendto_fn_name"]
        )

        context["default_from"] = (
            f"{Tag.getTag('full_group_name') or f'{settings.INSTITUTION_NAME} {settings.ORGANIZATION_SHORT_NAME}'} <info@{settings.SITE_INFO[1]}>"
        )
        context["from"] = context["default_from"]

        context["listcount"] = self.approx_num_of_recipients(
            filterObj, context["sendto_fn"]
        )
        context["filterid"] = filterObj.id
        context["mailer_warnings"] = self.get_mailer_warnings(
            context["listcount"], context["filterid"], context["sendto_fn_name"]
        )
        ...
        if not prs.exists():
            redirect = PlainRedirect.objects.create(
                original="info", destination=settings.DEFAULT_EMAIL_ADDRESSES["default"]
            )
    ...
    def commpanel(self, request, tl, one, two, module, extra, prog):
        ...
        if request.method == "POST":
            ...
            if ("base_list" in data and "recipient_type" in data) or (
                ...
                sendto_fn = MessageRequest.assert_is_valid_sendto_fn_or_ESPError(
                    sendto_fn_name
                )
                ...
                if data["use_checklist"] == "1":
                    (response, unused) = get_user_checklist(
                        ...
                        extra_context={"module": "Communications Portal"},
                    )
                    return response
                ...
                context["from"] = context["default_from"]
                return render_to_response(
                    self.baseDir() + "step2.html", request, context
                )
            ...
            ##  Prepare a message starting from an earlier request
                ...
                sendto_fn = MessageRequest.assert_is_valid_sendto_fn_or_ESPError(
                    msgreq.sendto_fn_name
                )
                context["listcount"] = self.approx_num_of_recipients(
                    msgreq.recipients, sendto_fn
                )
                context["mailer_warnings"] = self.get_mailer_warnings(
                    context["listcount"], context["filterid"], context["sendto_fn_name"]
                )
    ...
    def maincomm2(self, request, tl, one, two, module, extra, prog):
        ...
        return render_to_response(
            ...
            request,
            {
                ...
                "filterid": filterid,
                "sendto_fn_name": sendto_fn_name,
                "mailer_warnings": self.get_mailer_warnings(
                    listcount, filterid, sendto_fn_name
                ),
                "from": fromemail,
                "replyto": replytoemail,
                "subject": subject,
    ...
    def send_test_email(self, request, tl, one, two, module, extra, prog):
        ...
        rendered_text = render_to_string(
            "email/{}_email.html".format(template), {"msgbody": body}
        )
        ...
        try:
            ...
            return JsonResponse(
                {
                    "success": False,
                    "error": "Failed to send test email. Please check the server logs.",
                },
                status=500,
            )
