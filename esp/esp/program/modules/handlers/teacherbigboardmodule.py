...
class TeacherBigBoardModule(ProgramModuleObj):
    ...
    def teacherbigboard(self, request, tl, one, two, module, extra, prog):
        ...
        if prog.hasModule("TeacherModeratorModule"):
            numbers.extend(
                [
                    ("moderators registered", self.num_moderators(prog)),
                    ("moderator blocks offered", self.num_moderator_blocks(prog)),
                ]
            )
    ...
    def num_moderator_blocks(prog):
        return ModeratorRecord.objects.filter(program=prog).aggregate(Sum("num_slots"))[
            "num_slots__sum"
        ]
    ...
    def get_hours(prog, approved=False, scheduled=False, teachers=None):
        classes = (
            ...
            .prefetch_related(
                Prefetch(
                    "sections",
                    queryset=ClassSection.objects.order_by("id").prefetch_related(
                        "meeting_times"
                    ),
        ...
        hours = [
            [cls.timestamp, cls.class_size_max, cls.section_sum] for cls in classes
        ]
