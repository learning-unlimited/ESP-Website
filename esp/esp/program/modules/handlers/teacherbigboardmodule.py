...
class TeacherBigBoardModule(ProgramModuleObj):
    ...
    def teacherbigboard(self, request, tl, one, two, module, extra, prog):
        ...
        numbers = [
            ...
            ("teachers with a class registered", self.num_teachers_teaching(prog)),
            ("teachers with a class approved", self.num_teachers_teaching(prog, True)),
            ("classes registered", self.num_class_reg(prog)),
            ("classes registered (sequences)", self.num_class_style(prog, "Sequence")),
            ("classes registered (standalones)", self.num_class_style(prog, "Standalone")),
            ("classes approved", self.num_class_reg(prog, True)),
            ("class-hours registered", hours_stats[0]),
            ("class-hours approved", approved_hours_stats[0]),
            ("class-student-hours registered", hours_stats[1]),
            ("class-student-hours approved", approved_hours_stats[1]),
            ("teachers checked in today", self.num_checked_in_teachers(prog)),
        ]
        ...
        if ClassSubject.objects.filter(
            parent_program=prog, timestamp__isnull=True
        ).exists():
            timess_data = []
            class_hourss_data = []
            student_hourss_data = []
            ...
            timess = [
                (
                    ...
                    [(1, time) for time in self.reg_classes(prog)],
                    True,
                ),
                (
                    "number of registered classes (sequences)",
                    [(1, time) for time in self.style_classes(prog, "Sequence")],
                    True,
                ),
                (
                    "number of registered classes (standalones)",
                    [(1, time) for time in self.style_classes(prog, "Standalone")],
                    True,
                ),
                (
                    "number of approved classes",
                    [(1, time) for time in self.reg_classes(prog, True)],
                    True,
                ),
                (
                    "number of teachers registered",
                    [(1, time) for time in self.teach_times(prog)],
                    True,
                ),
                (
                    "number of teachers approved",
                    [(1, time) for time in self.teach_times(prog, True)],
                    True,
                ),
            ]
            ...
            class_hours_approved, student_hours_approved = self.get_hours(
                prog, approved=True
            )
            ...
            student_hourss = [
                ...
                (
                    "number of approved class-student-hours",
                    student_hours_approved,
                    True,
                ),
            ]
        ...
        return render_to_response(
            "program/modules/bigboardmodule/bigboard.html", request, context
        )
    ...
    def num_teachers_teaching(prog, approved=False, scheduled=False, teachers=None):
        ...
        return (
            ClassSubject.objects.filter(
                get_filter(
                    prog, approved=approved, scheduled=scheduled, teachers=teachers
                )
            )
            .exclude(category__category__iexact="Lunch")
            .exclude(teachers=None)
            .values_list("teachers", flat=True)
            .distinct()
            .count()
        )
    ...
    def num_active_users(self, prog, minutes=10):
        ...
        return (
            ClassSubject.objects.filter(parent_program=prog, timestamp__gt=recent)
            .exclude(category__category__iexact="Lunch")
            .exclude(teachers=None)
            .values_list("teachers")
            .distinct()
            .count()
        )
    ...
    @cache_function_for(105)
    def num_class_style(prog, style, approved=False):
        return (
            ClassSubject.objects.filter(
                get_filter(prog, approved) & Q(class_style=style)
            )
            .exclude(category__category__iexact="Lunch")
            .distinct()
            .count()
    ...
    num_class_style = staticmethod(num_class_style)

    @cache_function_for(105)
    def reg_classes(self, prog, approved=False, scheduled=False):
        class_times = (
            ClassSubject.objects.filter(
                get_filter(prog, approved=approved, scheduled=scheduled)
            )
            .exclude(category__category__iexact="Lunch")
            .distinct()
    ...
    @cache_function_for(105)
    def style_classes(prog, style, approved=False):
        class_times = (
            ClassSubject.objects.filter(
                get_filter(prog, approved) & Q(class_style=style)
            )
    ...
    style_classes = staticmethod(style_classes)
    ...
    def get_hours(prog, approved=False, scheduled=False, teachers=None):
        classes = (
            ...
            .annotate(num_sections=Count("sections"))
            .filter(num_sections__gt=0)
            .exclude(status__lt=0)  # no rejected classes
            .exclude(category__category__iexact="Lunch")
            .values_list("timestamp", "class_size_max", "class_size_optimal")
            .annotate(duration=Sum("sections__duration"))
        )
        # LU has different way of compiling statistics, kept MIT ESP version that already exists
    ...
    def static_hours(prog, approved=False, scheduled=False, teachers=None):
        hours = TeacherBigBoardModule.get_hours(
            prog, approved=approved, scheduled=scheduled, teachers=teachers
        )
        if hours[0]:
