...
class ClassFlagModule(ProgramModuleObj):
    ...
    def teachers(self, QObject=False):
        ...
        for flag_type in fts:
            q = Q(
                classsubject__flags__flag_type=flag_type.id,
    ...
    def teacherDesc(self):
        ...
        descs = {}
        for flag_type in fts:
            descs["flag_%s" % flag_type.id] = (
                """Teachers who have a class with the "%s" flag""" % flag_type.name
            )
        return descs
    ...
    def newflag(self, request, tl, one, two, module, extra, prog):
        ...
        if form.is_valid():
            ...
            if not prog.flag_types.filter(
                pk=form.cleaned_data["flag_type"].pk
            ).exists():
                return HttpResponseBadRequest("")
            flag = form.save()
            extra_payload = None
            if flag.flag_type.notify_teacher_by_email:
                try:
                    ...
                    logger.error(
                        "Failed to send teacher notification for flag %s on class %s",
                        flag.id,
                        flag.subject_id,
                        exc_info=True,
                    )
            ...
            return self._flag_json_response(
                request, flag, include_header=True, extra=extra_payload
            )
        else:
    ...
    def deleteflag(self, request, tl, one, two, module, extra, prog):
        ...
        if request.method != "POST" or "id" not in request.POST:
            return HttpResponseBadRequest("")
        flag = ClassFlag.objects.filter(
            id=request.POST["id"], subject__parent_program=prog
        ).first()
        if flag is None:
            return HttpResponseBadRequest("")
        flag.delete()
