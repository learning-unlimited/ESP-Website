...
from esp.users.models            import ESPUser, TeacherInfo
...
class TeacherClassRegModule(ProgramModuleObj):
    ...
    def get_resource_pairs(self):
        ...
        for res_type in self.program.getResourceTypes():
            ...
            for i in range(len(possible_values)):
                ...
                full_description = 'Teachers who requested "%s" for their %s' % (
                    val,
                    res_type.name,
                )
                query = Q(
    ...
    def teacherDesc(self):
        ...
        result = {
            ...
            "class_nearly_full": """Teachers teaching a nearly-full class (>%d%% of capacity)"""
            % (100 * capacity_factor),
    ...
    def cancelrequest(self, request, tl, one, two, module, extra, prog):
        if request.method == "POST" and "reason" in request.POST:
            ...
            email_title = "[%s] Class Cancellation Request for %s: %s" % (
    ...
    def coteachers_logic(cls, request, prog, template, ajax, is_admin=False):
        ...
        if "coteachers" not in request.POST:
            ...
            coteachers = [ESPUser.objects.get(id=userid) for userid in coteachers]
            add_list_members(
