...
class TeacherClassRegModule(ProgramModuleObj):
    ...
    def get_admin_search_entry(cls, program, tl, view_name, pmo):
        ...
        entries = {
            "makeaclass": (
                "Register Your Classes (Teacher Lookup)",
                "Other",
                ["teacher", "classes", "registration", "lookup", "add"],
            ),
            "copyclasses": (
                "Copy Your Classes",
                "Other",
                ["teacher", "classes", "registration", "copy"],
            ),
        }
    ...
    def prepare(self, context={}):
        ...
        context["modlist"] = (
            get_current_request().user.getModeratingSectionsFromProgram(self.program)
        )
        context["friendly_times_with_date"] = Tag.getBooleanTag(
            "friendly_times_with_date", self.program
        )
    ...
    def get_resource_pairs(self):
        ...
        for res_type in self.program.getResourceTypes():
            possible_values = res_type.resourcerequest_set.values_list(
                "desired_value", flat=True
            ).distinct()
            for i in range(len(possible_values)):
                val = possible_values[i]
                label = f"teacher_res_{res_type.id}_{i}"
                full_description = (
                    f'Teachers who requested "{val}" for their {res_type.name}'
                )
                query = Q(
                    classsubject__sections__resourcerequest__res_type=res_type,
                    classsubject__sections__resourcerequest__desired_value=val,
                    classsubject__sections__parent_class__parent_program=self.program,
                )
    ...
    def teachers(self, QObject=False):
        fields_to_defer = [
            x.name for x in ClassSubject._meta.fields if isinstance(x, models.TextField)
        ]
        ...
        Q_approved_teacher = (
            Q(classsubject__in=classes_qs.filter(status__gt=0, sections__status__gt=0))
            & Q_isteacher
        )
        Q_proposed_teacher = (
        ...
        if (
            len(self.program.dates()) == 0
        ):  # If current program doesn't have dates set yet, there are no past programs
            previous_programs = []
        else:
            previous_programs = [
                x
        ...
        Q_taught_before_temp = Q(
            classsubject__status=ClassStatus.ACCEPTED,
            classsubject__parent_program__in=previous_programs,
        )
        taught_before_users = (
            ESPUser.objects.filter(Q_taught_before_temp).values("id").distinct()
        )
        ...
        for item in self.get_resource_pairs():
            additional_qs[item[0]] = (
                Q_isteacher
                & (Q_rejected_teacher | Q_approved_teacher | Q_proposed_teacher)
                & item[2]
            )
        ...
        if QObject:
            ...
            result = {
                k: ESPUser.objects.filter(v).distinct() for k, v in qobjects.items()
            }
            for key in additional_qs:
    ...
    def deadline_met(self, extension=""):
        ...
        if len(extension) > 0:
            ...
            return self.any_reg_is_open() or tmpModule.deadline_met("/Classes/Edit")

    def class_reg_is_open(self):
        return self.deadline_met("/Classes/Create/Class")

    def open_class_reg_is_open(self):
        return self.crmi.open_class_registration and self.deadline_met(
            "/Classes/Create/OpenClass"
        )
    ...
    def section_attendance(self, request, tl, one, two, module, extra, prog):
        ...
        if len(sections) == 1:
            if not request.user.canEdit(
                sections[0].parent_class
            ) and not request.user.canMod(sections[0]):
                return render_to_response(
                    self.baseDir() + "cannoteditclass.html", request, {}
                )
            else:
                section = sections[0]
                context["section"], context["not_found"] = self.process_attendance(
                    section, request, prog
                )

        return render_to_response(
            self.baseDir() + "section_attendance.html", request, context
        )
    ...
    def process_attendance(section, request, prog):
        ...
        if request.POST and "submitted" in request.POST:
            ...
            misc_students = [
                _f
                for _f in re.split(r"[;,\s]\s*", request.POST.get("misc_students"))
                if _f
            ]
        ...
        for student in section.students():
            ...
            student.attended = (
                ...
                .filter(user=student, section=section, relationship=attended)
                .exists()
            )
        ...
        if "student" in request.POST and "secid" in request.POST:
            ...
            if not students.exists():
                ...
                if not sections.exists():
                    json_data["error"] = (
                        "Section with ID %s not found!" % request.POST["secid"]
                    )
                else:
                    section = sections[0]
                    if not (
                        request.user.canEdit(section.parent_class)
                        or request.user.canMod(section)
                    ):
                        json_data["error"] = (
                            "You do not have permission to modify attendance for this section."
                        )
                        return HttpResponse(
                            json.dumps(json_data), content_type="text/json"
                        )
                    json_data["secid"] = section.id
                    if request.POST.get("undo", "false").lower() == "true":
                        srs = StudentRegistration.valid_objects().filter(
                            user=student, section=section, relationship=attended
                        )
                        if srs.exists():
                            for sr in srs:
                                sr.expire()
                            json_data["message"] = (
                                "%s is no longer marked as attending." % student.name()
                            )
                        else:
                            json_data["message"] = (
                                "%s was not marked as attending." % student.name()
                            )
                        if request.POST.get("undo_checkin", "false").lower() == "true":
                            ...
                            rec = (
                                Record.objects.filter(
                                    user=student, program=prog, event=rt
                                )
                                .order_by("-id")
                                .first()
                            )
                            ...
                            if request.POST.get("enroll", "true").lower() == "true":
                                for rt in [enrolled, onsite]:
                                    ...
                                    if rt.name == "OnSite/AttendedClass":
                                        ...
                                        sr.save()
                        json_data["message"] = (
                            "%s is marked as attending." % student.name()
                        )
    ...
    def cancelrequest(self, request, tl, one, two, module, extra, prog):
        if request.method == "POST" and "reason" in request.POST:
            ...
            email_contents = render_to_string(
                "program/modules/teacherclassregmodule/cancelrequest.txt", email_context
            )
            ...
            email_to = [
                ESPUser.email_sendto_address(
                    cls.parent_program.director_email, "Directors"
                )
            ]
            send_mail(email_title, email_contents, email_from, email_to, False)
    ...
    def class_status(self, request, tl, one, two, module, extra, prog):
        ...
        return render_to_response(
            self.baseDir() + "class_status.html", request, context
        )
    ...
    def class_docs(self, request, tl, one, two, module, extra, prog):
        ...
        if request.method == "POST":
            if request.POST["command"] == "delete":
                ...
                if form.is_valid():
                    media = Media(
                        friendly_name=form.cleaned_data["title"], owner=target_class
                    )
                ...
                try:
                    ...
                    taught_ids = request.user.getTaughtClasses().values_list(
                        "id", flat=True
                    )
                        ...
                        if os.path.isfile(source_path):
                            with open(source_path, "rb") as f:
                                ...
                                django_file.content_type = (
                                    source_media.mime_type or "application/octet-stream"
                                )
                                django_file.size = source_media.size or 0
                                new_media = Media(
                                    friendly_name=source_media.friendly_name,
                                    owner=target_class,
                                )
                                ...
                                base_name = (
                                    parts[1] if len(parts) > 1 else original_name
                                )
                                desired_filename = "%s_%s" % (
                                    target_class.emailcode(),
                                    base_name,
                                )
                                ...
                                new_media.save()
                                copy_message = (
                                    'Successfully copied "%s" to this class.'
                                    % source_media.friendly_name
                                )
                        else:
    ...
    def coteachers_logic(cls, request, prog, template, ajax, is_admin=False):
        ...
        if op == "add":
            ...
            elif not is_admin and request.POST["teacher_selected"] == str(
                request.user.id
            ):
            ...
            availability = teacher.getAvailableTimes(
                prog, ignore_classes=True, ignore_moderation=True
            )
            ...
            if not availability:
                ...
                if (
                    not conflictinguser
                    and not unavailableuser
                    and not noavailuser
                    and not fullybookeduser
                    and cls.conflicts(teacher)
                ):
                    fullybookeduser = teacher
            ...
            if (
                ...
                if not lastProf.teacher_info:
                    ...
                    if anyInfo:
                        lastProf.teacher_info = TeacherInfo.addOrUpdate(
                            teacher,
                            lastProf,
                            {
            ...
            elif request.POST["moderator_selected"] in request.POST.get(
                "moderators", ""
            ).split(","):
                error = (
                    "Error - You already added this "
                    + prog.getModeratorTitle().lower()
                    + " to this section!"
                )
            ...
            if len(sections) != 1:
                error = (
                    "Error - Please use the form to add a "
                    + prog.getModeratorTitle().lower()
                    + "."
                )
            else:
    ...
    def coteachers(self, request, tl, one, two, module, extra, prog):
        ...
        return self.coteachers_logic(
            cls, request, prog, self.baseDir() + "coteachers.html", ajax
        )
    ...
    def editclass(self, request, tl, one, two, module, extra, prog):
        ...
        return self.makeaclass_logic(
            request, tl, one, two, module, extra, prog, cls, action
        )
    ...
    def makeaclass(self, request, tl, one, two, module, extra, prog, newclass=None):
        return self.makeaclass_logic(
            request, tl, one, two, module, extra, prog, newclass=None
        )
    ...
    def copyaclass(self, request, tl, one, two, module, extra, prog):
        ...
        if len(classes) != 1:
            raise ESPError(
                "Something weird happened, more than one class found matching this ID.",
                log=False,
            )
        cls = classes[0]
        ...
        return self.makeaclass_logic(
            request, tl, one, two, module, extra, prog, cls, action, populateonly=True
        )
    ...
    def copyclasses(self, request, tl, one, two, module, extra, prog):
        ...
        return render_to_response(
            self.baseDir() + "listcopyclasses.html", request, context
        )
    ...
    def makeaclass_logic(
        ...
        if request.method == "POST" and "class_reg_page" in request.POST:
            ...
            try:
                if action == "create":
                    ...
                    newclass = ccc.makeaclass(
                        request.user, request.POST, form_class=TeacherOpenClassRegForm
                    )
                elif action == "edit":
                ...
                do_question = bool(
                    ProgramModule.objects.filter(
                        handler="TeacherReviewApps", program=self.program
                    )
                )

                if do_question:
                    return HttpResponseRedirect(
                        newclass.parent_program.get_teach_url() + "app_questions"
                    )
                if request.POST.get("manage") == "manage":
                    if request.POST["manage_submit"] == "reload":
                        return HttpResponseRedirect(
                            request.get_full_path() + "?manage=manage"
                        )
                    elif request.POST["manage_submit"] == "manageclass":
                        return HttpResponseRedirect(
                            f"/manage/{self.program.getUrlBase()}/manageclass/{extra}"
                        )
                    elif request.POST["manage_submit"] == "dashboard":
                        return HttpResponseRedirect(
                            "/manage/%s/dashboard" % self.program.getUrlBase()
                        )
                    elif request.POST["manage_submit"] == "main":
                        return HttpResponseRedirect(
                            "/manage/%s/main" % self.program.getUrlBase()
                        )
                return self.goToCore(tl)
            ...
            if newclass is not None:
                ...
                current_duration = (
                    current_data["duration"] or newclass.sections.all()[0].duration
                )
                rounded_duration = 0
                ...
                if newclass.optimal_class_size_range:
                    current_data["optimal_class_size_range"] = (
                        newclass.optimal_class_size_range.id
                    )
        ...
        context["grade_range_popup"] = Tag.getBooleanTag(
            "grade_range_popup", self.program
        )
        ...
        if (
            (request.method == "POST" and request.POST.get("manage") == "manage")
            or (request.method == "GET" and request.GET.get("manage") == "manage")
            or (tl == "manage" and "class" in context)
        ) and request.user.isAdministrator():
            context["manage"] = True
    ...
    def teacherlookup(self, request, tl, one, two, module, extra, prog, newclass=None):
        ...
        return TeacherClassRegModule.teacherlookup_logic(
            request, tl, one, two, module, extra, prog, newclass
        )
