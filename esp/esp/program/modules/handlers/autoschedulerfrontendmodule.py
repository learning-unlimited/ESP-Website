...
class AutoschedulerFrontendModule(ProgramModuleObj):
    ...
    def autoscheduler(self, request, tl, one, two, module, extra, prog):
        ...
        try:
            ...
            context = {"program": prog, "err_msg": str(e)}
            return render_to_response(self.baseDir() + "error.html", request, context)
    ...
    @json_response()
    @needs_admin
    def autoscheduler_execute(self, request, tl, one, two, module, extra, prog):
        ...
        return {"response": [{"info": info, "autoscheduler_data": autoscheduler_data}]}
    ...
    def autoscheduler_save(self, request, tl, one, two, module, extra, prog):
        if "autoscheduler_data" not in request.POST:
            return {
                "response": [{"error_msg": "missing autoscheduler_data POST field"}]
            }
