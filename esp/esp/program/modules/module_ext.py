...
class StudentClassRegModuleInfo(models.Model):
    ...
    enforce_max = models.BooleanField(
        default=True,
        help_text="Check this box to prevent students from signing up for full classes.",
    )
    ...
    class_cap_multiplier = models.DecimalField(
        ...
        decimal_places=2,
        default="1.00",
        help_text="A multiplier for class capacities (set to 0.5 to cap all classes at half their stored capacity).",
    )
    class_cap_offset = models.IntegerField(
        default=0,
        help_text="Offset for class capacities (this number is added to the original capacity of every class).",
    )
    ...
    use_priority = models.BooleanField(
        default=False,
        help_text="Check this box to enable priority registration. Note, this is NOT for the two-phase student registration module. This will remove \
    ...
    priority_limit = models.IntegerField(
        default=3,
        help_text="The maximum number of choices a student can make per timeslot when priority registration is enabled. Also, the \
    ...
    use_grade_range_exceptions = models.BooleanField(
        default=False,
        help_text="Check this box to enable grade range exceptions for the two-phase class lottery.",
    )

    #   Set to true to allow classes to be added (via Ajax) using buttons on the catalog
    register_from_catalog = models.BooleanField(
        default=False,
        help_text="Check this box to allow students to add classes from the catalog page if they are logged in.",
    )

    #   Enrollment visibility
    visible_enrollments = models.BooleanField(
        default=True,
        help_text="Uncheck this box to prevent students from seeing enrollments on the catalog.",
    )
    #   Meeting times visibility
    visible_meeting_times = models.BooleanField(
        default=True,
        help_text="Uncheck this box to prevent students from seeing classes' meeting times on the catalog.",
    )
    ...
    view_button_text = models.CharField(
        ...
        help_text='Label for the "get receipt" button (for already confirmed students) at the bottom of student reg.',
    )
    ...
    temporarily_full_text = models.CharField(
        ...
        help_text='The text that replaces the "Add class" button when the class has reached its adjusted capacity',
    )
    ...
    cancel_button_dereg = models.BooleanField(
        default=False,
        help_text="Check this box to remove a student from all of their classes when they cancel their registration.",
    )
    ...
    progress_mode = models.IntegerField(
        default=1,
        help_text="Select which to use on student reg: 1=checkboxes, 2=progress bar, 0=neither.",
    )
    ...
    show_emailcodes = models.BooleanField(
        default=True,
        help_text="Uncheck this box to prevent email codes (i.e. E534, H243) from showing up on catalog and fillslot pages.",
    )
    ...
    force_show_required_modules = models.BooleanField(
        default=True,
        help_text='Check this box to require that users see and fill out "required" modules before they can see the main StudentReg page',
    )
    ...
    def reg_verbs(self):
        verb_list = [RegistrationType.get_cached(name="Enrolled", category="student")]

        if self.use_priority:
            for i in range(0, self.priority_limit):
                name = f"Priority/{i + 1}"
                verb_list.append(
                    RegistrationType.get_map(include=[name], category="student")[name]
                )

        #   Require that the /Applied bit is in the list, since students cannot enroll
        #   directly in classes with application questions.
        applied_verb = RegistrationType.get_map(
            include=["Applied"], category="student"
        )["Applied"]
...
class ClassRegModuleInfo(models.Model):
    ...
    allow_coteach = models.BooleanField(
        blank=True,
        default=True,
        help_text="Check this box to allow teachers to specify co-teachers.",
    )
    set_prereqs = models.BooleanField(
        blank=True,
        default=True,
        help_text="Check this box to allow teachers to enter prerequisites for each class that are displayed separately on the catalog.",
    )

    #   The maximum length of a class, in minutes.
    class_max_duration = models.IntegerField(
        blank=True, null=True, help_text="The maximum length of a class, in minutes."
    )

    #   Class size options: teachers will see [min:step:max] plus other_sizes
    class_min_cap = models.IntegerField(
        blank=True,
        null=True,
        help_text="The minimum number of students a teacher can choose as their class capacity.",
    )
    class_max_size = models.IntegerField(
        blank=True,
        null=True,
        help_text="The maximum number of students a teacher can choose as their class capacity.",
    )
    class_size_step = models.IntegerField(
        blank=True, null=True, help_text="The interval for class capacity choices."
    )
    class_other_sizes = models.CharField(
        blank=True,
        null=True,
        max_length=100,
    ...
    allowed_sections = models.CharField(
        max_length=100,
        blank=True,
        validators=[validate_comma_separated_integer_list],
    ...
    session_counts = models.CharField(
        max_length=100,
        blank=True,
        validators=[validate_comma_separated_integer_list],
    ...
    num_teacher_questions = models.PositiveIntegerField(
        default=1,
        blank=True,
        null=True,
        help_text="The maximum number of application questions that can be specified for each class.",
    )
    ...
    color_code = models.CharField(
        ...
        blank=True,
        null=True,
        help_text="The background color for class titles in the catalog and registration pages. If no color is chosen, the default light blue will be used.",
        validators=[
            RegexValidator(
                r"^#([a-zA-Z0-9]{3}){1,2}$",
                message='Value must be a valid 3-character or 6-character hex color starting with "#".',
            )
        ],
    )
    ...
    #   Room requests
    ask_for_room = models.BooleanField(
        blank=True,
        default=True,
    ...
    open_class_registration = models.BooleanField(
        blank=True,
        default=False,
    ...
    progress_mode = models.IntegerField(
        default=1,
        help_text="Select which to use on teacher reg: 1=checkboxes, 2=progress bar, 0=neither.",
    )
