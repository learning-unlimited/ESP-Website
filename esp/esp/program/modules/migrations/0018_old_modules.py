...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        if pm.exists():
            print 'Deleting: %s' % pm
            pm.delete()
        pm = ProgramModule.objects.filter(handler='ClassRoomModule')
        if pm.exists():
            print 'Deleting: %s' % pm
            pm.delete()
        
        #   Delete duplicate program module types (there should be no need for the
        #   same module to show up twice in the same section of the site)
        for handler_type in ProgramModule.objects.all().values_list('handler', flat=True).distinct():
            pm_qs = ProgramModule.objects.filter(handler=handler_type).order_by('-id')
            if pm_qs.exists():
                real_pm = pm_qs[0]
                pm = ProgramModule.objects.filter(handler=handler_type, module_type=real_pm.module_type).exclude(id=real_pm.id)
                #   Delete except for a particular exception that we have because
                #   we want two different links to appear on the onsite core page
                if pm.exists() and handler_type != u'OnSiteClassList':
                    print 'Deleting: %s' % pm
                    pm.delete()
            else:
                print 'Could not find any ProgramModules with handler: %s' % handler_type
    ...
    models = {
        ...
        'modules.creditcardsettings': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'invoice_prefix': ('django.db.models.fields.CharField', [], {'default': "'uchicago'", 'max_length': '80'}),
            'module': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['modules.ProgramModuleObj']"}),
