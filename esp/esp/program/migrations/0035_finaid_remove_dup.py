...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        cursor.execute('SELECT program_id, user_id, count(*) FROM program_financialaidrequest GROUP BY program_id, user_id HAVING count(*) > 1')
        results = cursor.fetchall()
        for tup in results:
            qs = FinancialAidRequest.objects.filter(program__id=tup[0], user__id=tup[1]).order_by('-id')
            for item in qs[1:]:
                item.delete()
