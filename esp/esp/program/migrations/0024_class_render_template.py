...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        re_str = r'{% render_class(|_core) ((([A-Za-z0-9\\._])*? )*?)request %}'
        re_replace =  r'{% render_class\1 \2%}'
        update_template_overrides(re_str, re_replace)
        
    def backwards(self, orm):
        "Write your backwards methods here."
        
        re_str = r'{% render_class(|_core) (([A-Za-z0-9\\._]*? )*?)%}'
        re_replace =  r'{% render_class\1 \2request %}'
        update_template_overrides(re_str, re_replace)

        #   Correct duplicates - can't figure out how to avoid this with regexp alone
        for t in TemplateOverride.objects.filter(content__contains='request request'):
            TemplateOverride.objects.filter(id=t.id).update(content=t.content.replace('request request', 'request'))
    ...
    models = {
        ...
        'program.registrationprofile': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'last_ts': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2011, 7, 29, 19, 23, 48, 799000)'}),
