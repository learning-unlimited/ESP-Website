...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Changing field 'RegistrationProfile.program'
        db.alter_column('program_registrationprofile', 'program_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['program.Program'], null=True))
        
        # Changing field 'TeacherBio.program'
        db.alter_column('program_teacherbio', 'program_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['program.Program'], null=True))

        # Changing field 'ClassSizeRange.program'
        db.alter_column('program_classsizerange', 'program_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['program.Program'], null=True))

        # Load ClassSizeRange defaults, which now are not associated with a program
        ClassSizeRange.objects.get_or_create(range_min=4, range_max=10, program=None)
        ClassSizeRange.objects.get_or_create(range_min=10, range_max=15, program=None)
        ClassSizeRange.objects.get_or_create(range_min=15, range_max=22, program=None)
        ClassSizeRange.objects.get_or_create(range_min=22, range_max=27, program=None)
        ClassSizeRange.objects.get_or_create(range_min=27, range_max=35, program=None)
        ClassSizeRange.objects.get_or_create(range_min=35, range_max=45, program=None)
        ClassSizeRange.objects.get_or_create(range_min=45, range_max=60, program=None)
        ClassSizeRange.objects.get_or_create(range_min=60, range_max=200, program=None)

    def backwards(self, orm):
        # Remove default ClassSizeRanges
        ClassSizeRange.objects.filter(program=None).delete()
        
        # User chose to not deal with backwards NULL issues for 'RegistrationProfile.program'
        try:
            db.alter_column('program_registrationprofile', 'program_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['program.Program'], null=False))
        except:
            raise RuntimeError("Cannot reverse this migration. 'RegistrationProfile.program' and its values cannot be restored.")
        
        # User chose to not deal with backwards NULL issues for 'TeacherBio.program'
        try:
            db.alter_column('program_teacherbio', 'program_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['program.Program'], null=False))
        except:
            raise RuntimeError("Cannot reverse this migration. 'TeacherBio.program' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'ClassSizeRange.program'
        try:
            db.alter_column('program_classsizerange', 'program_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['program.Program'], null=False))
        except:
            raise RuntimeError("Cannot reverse this migration. 'ClassSizeRange.program' and its values cannot be restored.")
    ...
    models = {
        ...
        'program.registrationprofile': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'last_ts': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2011, 5, 30, 11, 9, 10, 710000)'}),
