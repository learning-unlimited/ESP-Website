...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding field 'RegistrationProfile.most_recent_profile'
        db.add_column('program_registrationprofile', 'most_recent_profile', self.gf('django.db.models.fields.BooleanField')(default=False, blank=True), keep_default=False)

        from esp.program.models import RegistrationProfile
        from esp.users.models import ESPUser
        RegistrationProfile.objects.all().update(most_recent_profile = False)
        for user in ESPUser.objects.filter(registrationprofile__isnull=False).distinct().values_list('id', flat = True):
            ## Django will flatten this whole mess into one SQL expression
            RegistrationProfile.objects.filter(id__in=RegistrationProfile.objects.filter(user = user).order_by('-last_ts')[:1]).update(most_recent_profile = True)

    def backwards(self, orm):
        
        # Deleting field 'RegistrationProfile.most_recent_profile'
        db.delete_column('program_registrationprofile', 'most_recent_profile')
    ...
    models = {
        ...
        'program.registrationprofile': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'last_ts': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2010, 8, 13, 0, 14, 54, 47752)'}),
            ...
            'text_reminder': ('django.db.models.fields.NullBooleanField', [], {'null': 'True', 'blank': 'True'}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['auth.User']"})
        },
