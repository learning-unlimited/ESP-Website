...
def replace_director_emails(apps, schema_editor):
    ...
    for prog in Program.objects.all():
        if not re.match(
            r"(^.+@{0}$)|(^.+@(\w+\.)?learningu\.org$)".format(
                settings.SITE_INFO[1].replace(".", "\\.")
            ),
            prog.director_email,
        ):
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.EmailField(
                ...
                max_length=75,
                help_text="The director email address must end in @"
                + settings.SITE_INFO[1]
                + " (your website), @learningu.org, or a valid subdomain of learningu.org (i.e., @subdomain.learningu.org). The default is <b>info@"
                + settings.SITE_INFO[1]
