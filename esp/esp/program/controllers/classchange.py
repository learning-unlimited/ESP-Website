...
class ClassChangeController(object):
    ...
    def get_student_schedule(self, student_ind, for_real=False):
        ...
        schedule = f"<table border='1'>\n<tr>\n<th>Time{' and Room' if show_rooms else ''}</th>\n<th>Class and Teacher</th>\n<th>Code</th>\n</tr>\n"
        sections = list(
            self.students[student_ind]
            .getSections(self.program, ["Enrolled"])
            .distinct()
        )
        sections.sort(
            key=lambda section: section.meeting_times.order_by("start")[0].start
        )
        for cls in sections:
            schedule += (
                "<tr>\n<td>"
    ...
    def get_changed_student_email_text(self, student_ind, for_real=False):
        ...
        text += (
            f"{self.get_student_schedule(student_ind, for_real)}\n\n<br /><br />\n\n"
        )
    ...
    def get_unchanged_student_email_text(self, student_ind, for_real=False):
        ...
        if self.enroll_final[student_ind, :, :].any():
            ...
            text += (
                "We're sorry that we couldn't accommodate your class preferences this time, and we hope to see you at a future "
    ...
    def _init_Q_objects(self):
        self.Q_SR_STUDENTS = (
            Q(studentregistration__user__in=self.students) & self.Q_SR_PROG
        )
        self.Q_SR_WAIT = [
            Q(studentregistration__relationship__name=(f"Priority/{i}"))
            & self.Q_SR_STUDENTS
            for i in range(self.priority_limit + 1)
        ]
        self.Q_SR_WAIT[0] = (
            Q(
                studentregistration__relationship__name__regex=ClassChangeController.WAIT_REGEX_PATTERN
            )
            & self.Q_SR_STUDENTS
        )
        self.Q_SR_EN = (
            Q(studentregistration__relationship__name="Enrolled") & self.Q_SR_STUDENTS
        )

        self.Q_STUDENTS = Q(user__in=self.students) & self.Q_PROG
        self.Q_WAIT = [
            Q(relationship__name=(f"Priority/{i}")) & self.Q_STUDENTS
            for i in range(self.priority_limit + 1)
        ]
        self.Q_WAIT[0] = (
            Q(relationship__name__regex=ClassChangeController.WAIT_REGEX_PATTERN)
            & self.Q_STUDENTS
        )
    ...
    def __init__(self, program, **kwargs):
        ...
        if "deadline" in list(self.options.keys()):
            self.deadline = self.options["deadline"]

        self.Q_SR_NOW = nest_Q(
            StudentRegistration.is_valid_qobject(self.now), "studentregistration"
        )
        self.Q_SR_PROG = (
            Q(
                studentregistration__section__parent_class__parent_program=self.program,
                studentregistration__section__meeting_times__isnull=False,
            )
            & self.Q_SR_NOW
        )
        self.Q_SR_REQ = (
            Q(studentregistration__relationship__name="Request") & self.Q_SR_PROG
        )
        ...
        if "students_not_checked_in" in list(self.options.keys()) and isinstance(
            self.options["students_not_checked_in"], QuerySet
        ):
            self.students_not_checked_in = list(
                self.options["students_not_checked_in"]
                .values_list("id", flat=True)
                .distinct()
            )
        else:
            self.students_not_checked_in = list(
                self.students.exclude(id__in=self.program.students()["attended"])
                .values_list("id", flat=True)
                .distinct()
            )
        ...
        self.sections = (
            ...
            .filter(
                status__gt=0, parent_class__status__gt=0, meeting_times__isnull=False
            )
            .order_by("id")
    ...
    def print_stats(
        self,
        popular_count=10,
        print_section_notation=False,
        list_no_class_students=False,
    ):
        ...
        print(f"{len(self.program.students()['attended']):5d} attended")
        print(f"{len(self.students):5d} with requests")
        print(
            f"{len(set(self.students.values_list('id', flat=True)) & set(self.program.students()['attended'].values_list('id', flat=True))):5d} attended with requests"
        )
        ...
        print(
            f"{numpy.count_nonzero(self.request.any(axis=1)):5d} student-timeslots with requests"
        )
        print()
        ...
        for i in range(ts_freqs.size):
            if ts_freqs[i]:
                print(
                    f"{ts_freqs[i]:5d} students with {i:3d} timeslot(s) with requests"
                )
        print()
        ...
        for i in range(ts_freqs.size):
            if oe_freqs[i]:
                print(
                    f"{oe_freqs[i]:5d} students originally enrolled in {i:3d} section(s)"
                )
        ...
        for section_index in numpy.argsort(request_freq)[::-1][:popular_count]:
            ...
            print(
                "# = {:5d} ({:3d} ->{:3d} /{:3d}) [{:3d} -{:3d} +{:3d} =>{:3d}]: {}".format(
        ...
        for i in range(changed_classes_freq.size):
            if changed_classes_freq[i]:
                print(
                    f"{changed_classes_freq[i]:5d} students with {i:3d} added sections"
                )

        print()
        print("Students originally with no classes")
        print("-----------------------------------")
        orig_no_class_student_indices = numpy.nonzero(
            ~self.enroll_orig.any(axis=(1, 2))
        )
        ...
        print("----------------------------------")
        (final_no_class_student_indices,) = numpy.nonzero(
            ~self.enroll_final.any(axis=(1, 2))
        )
        ...
        if list_no_class_students:
            for student_index in final_no_class_student_indices:
                ...
                for section_index in numpy.nonzero(
                    self.request[student_index, :, :].any(axis=1)
                )[0]:
                    section = self.sections[section_index]
                    print(
                        "# = {:5d} ({:3d} ->{:3d} /{:3d}): {}".format(
    ...
    def sanity_check(self):
        ...
        for student_index, timeslot_index in numpy.transpose(numpy.nonzero(bad_drops)):
            (orig_section_indices,) = numpy.nonzero(
                self.enroll_orig[student_index, :, timeslot_index]
            )
            ...
            print(
                f"{self.students[student_index]:>20} had {orig_section} @ {self.timeslots[timeslot_index]}"
            )
        ...
        for student_index, section_index, timeslot_index in numpy.transpose(
            ...
            print(
                f"{self.students[student_index]:>20} got {self.sections[section_index]} @ {self.timeslots[timeslot_index]}"
            )
        ...
        if num_bad_drops or num_unrequested_changes:
            print(
                "^^^",
                num_bad_drops + num_unrequested_changes,
    ...
    def initialize(self):
        ...
        self.enroll_orig = numpy.zeros(
            (self.num_students, self.num_sections, self.num_timeslots), dtype=bool
        )
        ...
        self.student_not_checked_in[
            self.student_indices[self.students_not_checked_in]
        ] = True
        ...
        self.lunch_timeslots = numpy.zeros(
            (len(lunch_by_day), ts_count), dtype=numpy.int32
        )
        for i in range(len(lunch_by_day)):
        ...
        enroll_regs = (
            StudentRegistration.objects.filter(self.Q_EN)
        ...
        try:
            self.enroll_orig[
                self.student_indices[era[:, 0]],
                self.section_indices[era[:, 1]],
                self.timeslot_indices[era[:, 2]],
            ] = True
        except IndexError:
            pass

        self.student_not_checked_in[
            numpy.transpose(numpy.nonzero(~(self.enroll_orig.any(axis=(1, 2)))))
        ] = True

        #   Populate request matrix
        request_regs = (
            StudentRegistration.objects.filter(self.Q_REQ)
            .values_list("user__id", "section__id", "section__meeting_times__id")
            .distinct()
        )
        ...
        try:
            self.request[
                self.student_indices[rra[:, 0]],
                self.section_indices[rra[:, 1]],
                self.timeslot_indices[rra[:, 2]],
            ] = True
        except IndexError:
            pass
        ...
        waitlist_regs = [
            StudentRegistration.objects.filter(self.Q_WAIT[i])
            .values_list("user__id", "section__id", "section__meeting_times__id")
            .distinct()
            for i in range(self.priority_limit + 1)
        ]
        ...
        for i in range(1, self.priority_limit + 1):
            try:
                self.waitlist[i][
                    self.student_indices[wra[i][:, 0]],
                    self.section_indices[wra[i][:, 1]],
                    self.timeslot_indices[wra[i][:, 2]],
                ] = True
                self.waitlist[0][
                    self.student_indices[wra[i][:, 0]],
                    self.section_indices[wra[i][:, 1]],
        ...
        for i in range(self.num_sections):
            ...
            for [
                ...
                self.section_conflict[
                    numpy.transpose(numpy.nonzero(self.section_schedules[:, ts_ind])), i
                ] = True
                self.section_conflict[
                    i, numpy.transpose(numpy.nonzero(self.section_schedules[:, ts_ind]))
                ] = True
        ...
        for sec in self.sections:
            ...
            sec_enroll_orig = self.enroll_orig[
                :, sec_ind, self.section_schedules[sec_ind, :]
            ].any(axis=1)
            any_overlapping_requests = self.request[
                :, :, self.section_schedules[sec_ind, :]
            ].any(axis=(1, 2))
            # Optimistically add number enrolled but want to switch out to capacity
            self.section_capacities[sec_ind] += numpy.count_nonzero(
                sec_enroll_orig * any_overlapping_requests
            )
            ...
            self.enroll_final[
                numpy.transpose(
                    numpy.nonzero(sec_enroll_orig * ~(any_overlapping_requests))
                ),
                sec_ind,
                self.section_schedules[sec_ind, :],
            ] = True
            ...
            self.section_scores[sec_ind] += numpy.count_nonzero(
                self.request[:, sec_ind, :].any(axis=1)
            )  # number who want to switch in
    ...
    def fill_section(self, si, priority=False):
        ...
        for i in range(self.lunch_timeslots.shape[0]):
            if (
                len(self.lunch_timeslots[i]) != 0
                and numpy.sum(
                    lunch_overlap[self.timeslot_indices[self.lunch_timeslots[i]]]
                )
                >= (self.lunch_timeslots.shape[1])
            ):
                if self.options["stats_display"]:
                    logger.info(
        ...
        for [
            ...
            if numpy.sum(self.lunch_timeslots == self.timeslot_ids[ts_ind]) > 0:
                lunch_day = numpy.nonzero(
                    self.lunch_timeslots == self.timeslot_ids[ts_ind]
                )[0][0]
        ...
        if self.options["stats_display"]:
            logger.info(
                "   About to try to add %d candidates to %d spaces",
        ...
        if candidate_students.shape[0] <= max(num_spaces, 0):
            ...
            selected_students = numpy.random.choice(
                candidate_students, num_spaces, replace=False
            )
            section_filled = True
    ...
    def push_back_students(self):
        ...
        while more_pushing:
            ...
            self.sorted_section_indices.sort(
                key=lambda sec_ind: -self.section_capacities[sec_ind]
            )
            for sec_ind in self.sorted_section_indices:
                ...
                any_enroll_orig = self.enroll_orig[
                    :, sec_ind, self.section_schedules[sec_ind, :]
                ].any(axis=1)
                ...
                # so far
                no_enroll_final = ~(
                    self.enroll_final[:, :, self.section_schedules[sec_ind, :]].any(
                        axis=(1, 2)
                    )
                )
                ...
                students_to_push = numpy.transpose(
                    numpy.nonzero(any_enroll_orig * no_enroll_final)
                )
                ...
                for student_ind in students_to_push:
                    ...
                    # classes.
                    self.enroll_final[
                        student_ind, sec_ind, self.section_schedules[sec_ind, :]
                    ] = True
                    if self.section_capacities[sec_ind] > 0:
                        ...
                        if not sec_ind in list(students_to_kick.keys()):
                            ...
                            # computed so far.
                            students_to_kick[sec_ind] = numpy.transpose(
                                numpy.nonzero(
                                    (self.enroll_final * self.request)[
                                        :, sec_ind, self.section_schedules[sec_ind, :]
                                    ].any(axis=1)
                                )
                            )
                            ...
                            for [student] in students_to_kick[sec_ind]:
                                ...
                                for [old_section] in old_sections:
                                    pq.put(
                                        (
                                            self.section_scores[old_section],
                                            random.random(),
                                            student,
                                        ),
                                        False,
                        ...
                        try:
                            self.enroll_final[
                                students_to_kick[sec_ind].get(False)[2],
                                sec_ind,
                                self.section_schedules[sec_ind, :],
                            ] = False
                        except queue.Empty:
    ...
    def compute_assignments(self):
        ...
        self.sorted_section_indices = list(range(self.num_sections))
        self.sorted_section_indices.sort(
            key=lambda sec_ind: self.section_scores[sec_ind]
        )
        for i in list(range(1, self.priority_limit + 1)) + [
            False,
        ]:
            if self.options["stats_display"]:
                logger.info(
                    "\n== Assigning priority%s students",
                    str(i) if self.priority_limit > 1 else "",
                )
    ...
    def save_assignments(self):
        ...
        assignments = numpy.transpose(
            numpy.nonzero((self.enroll_final * ~(self.enroll_orig)).any(axis=2))
        )
        removals = numpy.transpose(
            numpy.nonzero((self.enroll_orig * ~(self.enroll_final)).any(axis=2))
        )
        relationship, created = RegistrationType.objects.get_or_create(name="Enrolled")

        for student_ind, section_ind in removals:
            self.sections[section_ind].unpreregister_student(
                self.students[student_ind], prereg_verbs=["Enrolled"]
            )
            self.changed[student_ind] = True
        for student_ind, section_ind in assignments:
            self.sections[section_ind].preregister_student(
                self.students[student_ind],
                overridefull=False,
    ...
    def unsave_assignments(self):
        StudentRegistration.objects.filter(
            end_date__gte=self.now, end_date__lte=datetime(9000, 1, 1)
    ...
    def send_student_email(self, student_ind, changed=True, for_real=False, f=None):
        ...
        if f:
            f.write(
                (text_fn(student_ind, for_real=False) + sent_to)
                .replace("\u2019", "'")
                .replace("\u201c", '"')
                .replace("\u201d", '"')
            )
        if for_real:
            send_mail(
                self.subject,
                text_fn(student_ind, for_real=True),
                self.from_email,
                recipient_list,
                bcc=self.bcc,
    ...
    def send_emails(self, for_real=False):
        ...
        self.from_email = ESPUser.email_sendto_address(
            self.program.director_email, self.program.niceName()
        )
