...
class ScheduleLoadAndSaveTest(ProgramFrameworkTest):
    ...
    def setUpProgram(self, settings, extra_settings):
        ...
        end_time = start_time + datetime.timedelta(minutes=settings["timeslot_length"])
        ...
        duration = (
            (settings["timeslot_length"] * extra_settings["extra_class_duration"])
            + (settings["timeslot_gap"] * (extra_settings["extra_class_duration"] - 1))
        ) / 60.0
        new_class, created = ClassSubject.objects.get_or_create(
            title="Extra Class",
            category=self.categories[extra_settings["extra_class_category"]],
        ...
        self.teachers[extra_settings["teacher_admin_idx"]].makeRole("Administrator")
    ...
    def setUpSchedule(self, settings, extra_settings):
        ...
        for i in range(settings["num_timeslots"]):
            start_time = settings["start_time"] + datetime.timedelta(
                minutes=(i * (settings["timeslot_length"] + settings["timeslot_gap"]))
            )
            end_time = start_time + datetime.timedelta(
                minutes=settings["timeslot_length"]
            )
            timeslots.append(
                data_model.AS_Timeslot(start_time, end_time, timeslot_id, None)
            )
            timeslot_id += 1
        start_time = extra_settings["extra_timeslot_start"]
        end_time = start_time + datetime.timedelta(minutes=settings["timeslot_length"])
        ...
        for i in range(settings["num_rooms"]):
            classrooms.append(
                data_model.AS_Classroom(f"Room {str(i)}", capacity, timeslots[:-1])
            )
        restype_id = ResourceType.objects.get(
            name=extra_settings["extra_resource_type_name"]
        ).id
        ...
        room_timeslots = [
            timeslots[i] for i in extra_settings["extra_room_availability"]
        ]
        classrooms.append(
            data_model.AS_Classroom(
                "Extra Room",
                extra_settings["extra_room_capacity"],
                room_timeslots,
        ...
        for i in range(settings["num_teachers"]):
            ...
            teacher_availability = [ts for j, ts in enumerate(timeslots) if j != i]
            is_admin = i == extra_settings["teacher_admin_idx"]
            teachers.append(
                data_model.AS_Teacher(teacher_availability, teacher_id, is_admin)
            )
        ...
        for t in teachers:
            for i in range(settings["classes_per_teacher"]):
                ...
                for j in range(settings["sections_per_class"]):
                    ...
                    section_id += 1
                subject_id += 1
        category_id = extra_settings["extra_class_category"] + self.initial_category_id
        grade_min = extra_settings["extra_class_grade_min"]
        grade_max = extra_settings["extra_class_grade_max"]
    ...
    def assert_roomslot_equality(self, roomslot1, roomslot2):
        ...
        if roomslot1.assigned_section is None:
            ...
            self.assertEqual(
                roomslot1.assigned_section.id, roomslot2.assigned_section.id
            )
    ...
    def assert_section_equality(self, section1, section2):
        ...
        self.assertEqual(
            set(t.id for t in section1.teachers), set(t.id for t in section2.teachers)
        )
        ...
        self.assertEqual(
            len(section1.assigned_roomslots), len(section2.assigned_roomslots)
        )
        for rs1, rs2 in zip(section1.assigned_roomslots, section2.assigned_roomslots):
            self.assert_roomslot_equality(rs1, rs2)

        self.assertSetEqual(
            set(section1.resource_requests.keys()),
    ...
    def assert_teacher_equality(self, teacher1, teacher2):
        ...
        self.assertSetEqual(
            set(teacher1.taught_sections.keys()), set(teacher2.taught_sections.keys())
        )
    ...
    def assert_classroom_equality(self, room1, room2):
        ...
        for rs1, rs2 in zip(room1.availability, room2.availability):
            self.assert_roomslot_equality(rs1, rs2)
        self.assertSetEqual(
            set(room1.furnishings.keys()), set(room2.furnishings.keys())
        )
        for furnishing_name in room1.furnishings:
            self.assert_restype_equality(
                room1.furnishings[furnishing_name], room2.furnishings[furnishing_name]
            )
    ...
    def assert_schedule_equality(self, schedule1, schedule2):
        ...
        self.assertSetEqual(
            set(schedule1.class_sections.keys()), set(schedule2.class_sections.keys())
        )
        ...
        self.assertSetEqual(
            set(schedule1.teachers.keys()), set(schedule2.teachers.keys())
        )
        ...
        self.assertSetEqual(
            set(schedule1.classrooms.keys()), set(schedule2.classrooms.keys())
        )
        for room_name in schedule1.classrooms:
            self.assert_classroom_equality(
                schedule1.classrooms[room_name], schedule2.classrooms[room_name]
            )
    ...
    def remove_class_simple_model(self, schedule=None):
        ...
        del section.teachers[0].availability_dict[(timeslot.start, timeslot.end)]
    ...
    def test_schedule_save(self):
        ...
        self.assertEqual(
            len(section_obj.get_meeting_times()), 0, "Section already had meeting times"
        )
        try:
        ...
        self.assertEqual(
            len(section_obj.get_meeting_times()),
            1,
            "Section should have been scheduled for 1 timeslot",
        )
        self.assertEqual(
            section_obj.get_meeting_times()[0].id,
    ...
    def test_schedule_double_save(self):
        ...
        try:
            ...
            self.fail(
                f"Schedule second save crashed with error: \n{traceback.format_exc()}"
            )
    ...
    def test_load_lunch(self):
        ...
        subj.accept()
        lunch_timeslots = [self.timeslots[1], self.timeslots[2], self.extra_timeslot]
        ...
        self.assertEqual(len(loaded_lunch), 2, "Should have loaded lunch on 2 days")
        start = lunch_timeslots[0].start
        start_day = (start.year, start.month, start.day)
        self.assertIn(start_day, loaded_lunch, "Couldn't find first day of lunch")
        day1 = loaded_lunch[start_day]
        self.assertEqual(len(day1), 2, "Should have 2 lunch timeslots on day 1")
        ...
        self.assertIn(start_day2, loaded_lunch, "Cloudln't find second day of lunch")
        day2 = loaded_lunch[start_day2]
        self.assertEqual(len(day2), 1, "Should have 1 lunch timeslot on day 2")
        self.assertEqual(
            (day2[0].start, day2[0].end),
            lunch_times[2],
    ...
    def test_schedule_save_swap_and_rollback(self):
        ...
        sections = [
            self.schedule.class_sections[i]
            for i in range(self.initial_section_id, self.initial_section_id + 2)
        ]
        ...
        for s in section_objs:
            self.assertEqual(
                len(s.get_meeting_times()), 0, "Section already had meeting times"
            )
        try:
            db_interface.save(self.schedule)
        except SchedulingError:
        ...
        db_interface.check_can_schedule_sections = lambda x, y: raise_after_one()
        try:
            for s in sections:
                s.clear_roomslots()
            for s, r in zip(sections, roomslots):
            ...
            db_interface.check_can_schedule_sections = db_interface_can_schedule
