...
class ConsistencyChecker:
    ...
    def check_availability_dict_consistency(self, schedule):
        ...
        for room in schedule.classrooms.values():
            ...
            if set(room.availability) != set(room.availability_dict.values()):
                raise ConsistencyError(
    ...
    def check_lunch_consistency(self, schedule):
        ...
        for day in schedule.lunch_timeslots:
            for timeslot1, timeslot2 in zip(
                schedule.lunch_timeslots[day], schedule.lunch_timeslots[day][1:]
            ):
                if timeslot1 == timeslot2:
                    raise ConsistencyError("Duplicate lunch timeslots detected.")
                if timeslot1 > timeslot2:
            ...
            for timeslot in schedule.lunch_timeslots[day]:
                ...
                # roomslots.
                if (
                    timeslot not in schedule.timeslots
                    or timeslot
                    is not schedule.timeslots[schedule.timeslots.index(timeslot)]
                ):
    ...
    def check_roomslots_consistency(self, schedule):
        ...
        for room in schedule.classrooms.values():
            for roomslot in room.availability:
                if roomslot.room != room:
                    raise ConsistencyError("Room and roomslot were inconsistent")
        ...
        for section in schedule.class_sections.values():
            for roomslot in section.assigned_roomslots:
                if roomslot.assigned_section != section:
                    raise ConsistencyError("Section and roomslot were inconsistent")
        ...
        for timeslot in schedule.timeslots:
            for roomslot in timeslot.associated_roomslots:
                if roomslot.timeslot is not timeslot:
                    raise ConsistencyError("Timeslot and roomslot were inconsistent")
        ...
        for roomslot in existing_roomslots:
            ...
            if (
                roomslot.timeslot not in schedule.timeslots
                or roomslot.timeslot
                is not schedule.timeslots[schedule.timeslots.index(roomslot.timeslot)]
            ):
            ...
            if roomslot.assigned_section is not None:
                if roomslot.assigned_section.id not in schedule.class_sections:
                    raise ConsistencyError("Event had an unregistered section")
                if roomslot not in roomslot.assigned_section.assigned_roomslots:
                    raise ConsistencyError(
    ...
    def roomslot_sorting_helper(self, roomslots):
        ...
        return all(
            rs1.timeslot < rs2.timeslot for rs1, rs2 in zip(roomslots, roomslots[1:])
        )
    ...
    def check_sorting_consistency(self, schedule):
        ...
        for section in schedule.class_sections.values():
            if not self.roomslot_sorting_helper(section.assigned_roomslots):
                raise ConsistencyError("Section assigned roomslots weren't sorted.")
        for classroom in schedule.classrooms.values():
            if not self.roomslot_sorting_helper(classroom.availability):
                raise ConsistencyError("Classroom availability wasn't sorted.")
        for teacher in schedule.teachers.values():
            if not all(
                t1 < t2
                for t1, t2 in zip(teacher.availability, teacher.availability[1:])
            ):
                raise ConsistencyError("Teacher availability wasn't sorted.")
    ...
    def check_timeslot_consistency(self, schedule):
        ...
        for timeslot1, timeslot2 in zip(schedule.timeslots, schedule.timeslots[1:]):
            if timeslot1 == timeslot2:
    ...
    def check_timeslot_duration_consistency(self, schedule):
        ...
        for timeslot in schedule.timeslots:
            actual_duration = util.hours_difference(timeslot.start, timeslot.end)
    ...
    def check_timeslot_span_consistency(self, schedule):
        ...
        for timeslot in schedule.timeslots:
            ...
            if (start.year, start.month, start.day) != (end.year, end.month, end.day):
                raise ConsistencyError(f"Timeslot ({start}, {end}) spans multiple days")
    ...
    def check_timeslots_list_and_dict_consistency(self, schedule):
        ...
        for timeslot in schedule.timeslots:
            if (timeslot.start, timeslot.end) not in schedule.timeslot_dict:
                raise ConsistencyError("Timeslot in list was missing from dict")
            visited[(timeslot.start, timeslot.end)] = True
        for times in schedule.timeslot_dict:
            if not visited[times]:
                raise ConsistencyError("Timeslot in dict was missing from list")
            timeslot = schedule.timeslot_dict[times]
            if (timeslot.start, timeslot.end) != times:
                raise ConsistencyError("Timeslot didn't match key in dict")
