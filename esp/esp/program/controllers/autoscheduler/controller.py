...
class AutoschedulerController(object):
    def __init__(self, prog, **options):
        ...
        search_options = {
            k.split("_", 1)[1]: v for k, v in options.items() if k.startswith("search_")
        }
        constraint_names = [k for k, v in constraint_options.items() if v]
        resource_criteria = load_all_resource_criteria(prog)
        valid_res_types = ResourceType.objects.filter(program=prog).values_list(
            "name", flat=True
        )
        ...
        schedule = db_interface.load_schedule_from_db(prog, **search_options)
    ...
    def scorer_options(prog):
        ...
        scorer_options = {
            k: (v, config.SCORER_DESCRIPTIONS[k]) for k, v in scorers.items()
        }
    ...
    def resource_options(prog):
        ...
        resource_options = {
            k: (wt, spec) for k, (spec, wt) in resource_criteria.items()
        }
    ...
    def search_options(prog, section=None):
        ...
        if section is not None and section.isdigit():
            try:
                ...
                pass
        return {
            "section_emailcode": (emailcode, "Emailcode of the section to optimize."),
            "depth": (1, "Depth to search. 1, 2, maybe 3 are okay, 4 is too slow."),
            "timeout": (10.0, "Timeout in seconds for the search."),
            "require_approved": (True, "Only schedule approved classes."),
            "exclude_lunch": (True, "Don't schedule lunch classes."),
            "exclude_walkins": (True, "Don't schedule walk-ins."),
            "exclude_scheduled": (False, "Don't touch already-scheduled classes."),
            "exclude_locked": (
                True,
                "Don't touch classes locked on the AJAX scheduler",
            ),
        }
    ...
    def get_scheduling_info(self):
        rows = []
        for action in self.simplify_history(self.optimizer.manipulator.history):
            row = []
            if action["action"] == "schedule":
                row.append(
                    [
                        f"Scheduled {self.section_identifier(action['section'])}:",
                        self.section_info(action["section"]),
                    ]
                )
                row.append(
                    [
                        f"In {self.roomslot_identifier(action['start_roomslot'])}:",
                ...
                row.append(
                    [
                        f"Moved {self.section_identifier(action['section'])}:",
                        self.section_info(action["section"]),
                    ]
                )
                row.append(
                    [
                        f"From {self.roomslot_identifier(action['prev_start_roomslot'])}:",
                        self.roomslot_info(action["prev_start_roomslot"]),
                    ]
                )
                row.append(
                    [
                        f"To {self.roomslot_identifier(action['start_roomslot'])}:",
                        self.roomslot_info(action["start_roomslot"]),
                    ]
                )
                ...
                row.append(
                    [
                        f"Unscheduled {self.section_identifier(action['section'])}:",
                        self.section_info(action["section"]),
                    ]
                )
                row.append(
                ...
                row.append(
                    [
                        f"Swapped {self.section_identifier(sec1)}:",
                        self.section_info(sec1),
                    ]
                )
                row.append(
                    [
                        f"Now in {self.roomslot_identifier(rs1)}:",
                        self.roomslot_info(rs1),
                    ]
                )
                row.append(
                    [f"and {self.section_identifier(sec2)}:", self.section_info(sec2)]
                )
                row.append(
                    [
                        f"Now in {self.roomslot_identifier(rs2)}:",
        ...
        if len(rows) > 0:
            ...
            diffs = []
            for (scorer, weight, old_score), (scorer2, weight2, new_score) in zip(
                self.initial_scores, final_scores
            ):
            ...
            total_change = (final_total_score - self.initial_total_score) * len(
            ...
            rows.append(
                [["Major score changes (weight, change, contribution):", new_row]]
            )
        return rows
    ...
    def section_info(self, section):
        info = []
        teachers_list = [
            ESPUser.objects.get(id=teacher.id).name() for teacher in section.teachers
        ]
        ...
        info.append(
            f"<b>Class Flags: </b>{', '.join(cls.flags.filter(resolved=False).values_list('flag_type__name', flat=True))}"
        )
        return info
    ...
    def import_assignments(self, data):
        ...
        for section, scheduling_hash in scheduling_hashes.items():
            initial_state = self.schedule.class_sections[int(section)].initial_state
            logger.info(initial_state)
            if initial_state != scheduling_hash:
                emailcode = ClassSection.objects.get(id=int(section)).emailcode()
                raise SchedulingError(f"Section {emailcode} was moved.")
...
def load_all_resource_criteria(prog, use_comments=False):
    ...
    resource_criteria = db_interface.load_resource_scoring(
        prog, overrides, specs_only=True, ignore_comments=(not use_comments)
    )
    if use_comments:
        for k in resource_criteria:
            ...
            if comment in resource_criteria:
                resource_criteria[k] = (
                    resource_criteria[comment][0],
                    resource_criteria[k][1],
                )
    return {k: v for k, v in resource_criteria.items() if "_comment" not in k}
