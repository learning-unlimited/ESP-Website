...
def create_test_schedule_1():
    timeslots = [
        data_model.AS_Timeslot(
            datetime(2017, 2, 2, 10 + i, 5), datetime(2017, 2, 2, 10 + i, 55), i + 1
        )
        for i in range(6)
    ]
    ...
    classrooms = {
        ...
        "10-250": data_model.AS_Classroom(
            "10-250", 30, timeslots[3:], furnishings={restype.name: restype}
        ),
    }
    teachers = {
...
def create_test_schedule_3():
    ...
    timeslots_extra = [
        data_model.AS_Timeslot(
            datetime(2017, 2, 3, 10 + i, 5), datetime(2017, 2, 3, 10 + i, 55), i + 7
        )
        for i in range(6)
    ]
