...
def get_custom_fields():
    ...
    if form_list_str:
        ...
        for item in form_cls_list:
            mod = __import__(
                "esp.program.modules.forms.teacherreg_custom", (), (), [item]
            )
...
class ClassCreationController(object):
    ...
    def makeaclass(self, user, reg_data, form_class=TeacherClassRegForm):
        ...
        self.force_availability(
            user
        )  ## So the default DB state reflects the default form state of "all times work"
    ...
    def editclass(self, current_user, reg_data, clsid, form_class=TeacherClassRegForm):
        ...
        try:
            ...
            raise ESPError(
                f"The class you're trying to edit (ID {repr(clsid)}) does not exist!",
                log=False,
            )

        extra_time = (
            reg_form._get_total_time_requested()
    ...
    def get_forms(self, reg_data, form_class=TeacherClassRegForm):
        ...
        if not reg_form.is_valid() or (
            resource_formset and not resource_formset.is_valid()
        ):
    ...
    def set_class_data(self, cls, reg_form):
        ...
        if (
            "optimal_class_size_range" in reg_form.cleaned_data
            and reg_form.cleaned_data["optimal_class_size_range"]
        ):
            cls.optimal_class_size_range = ClassSizeRange.objects.get(
                id=reg_form.cleaned_data["optimal_class_size_range"]
            )

        if (
            "allowable_class_size_ranges" in reg_form.cleaned_data
            and reg_form.cleaned_data["allowable_class_size_ranges"]
        ):
            cls.allowable_class_size_ranges = ClassSizeRange.objects.filter(
                id__in=reg_form.cleaned_data["allowable_class_size_ranges"]
            )
    ...
    def send_availability_email(self, teacher, note=None):
        ...
        email_contents = render_to_string(
            "program/modules/availabilitymodule/update_email.txt", email_context
        )
    ...
    def teacher_has_time(self, user, hours):
        return (
            user.getTaughtTime(self.program, include_scheduled=True)
    ...
    def add_teacher_to_program_mailinglist(self, user):
        add_list_member(
            f"{self.program.program_type}_{self.program.program_instance}-teachers",
            user,
        )
    ...
    def generate_director_mail_context(self, cls):
        ...
        mail_ctxt["category"] = ClassCategories.objects.get(
        ...
        if new_data.get("optimal_class_size_range_id") is not None:
            opt_range = ClassSizeRange.objects.get(
                id=new_data["optimal_class_size_range_id"]
            )
            mail_ctxt["optimal_class_size_range"] = (
                str(opt_range.range_min) + "-" + str(opt_range.range_max)
            )
        else:
            mail_ctxt["optimal_class_size_range"] = ""
        try:
            mail_ctxt["allowable_class_size_ranges"] = (
                cls.allowable_class_size_ranges.all()
            )
        ...
        for teacher in cls.get_teachers():
            ...
            if last_profile.teacher_info is not None:
                ...
                teacher_ctxt["from_here"] = (
                    "[Teacher hasn't filled out teacher profile!]"
                )
            ...
            teacher_ctxt["taught_programs"] = ", ".join(
                [
                    prog.niceName()
                    for prog in teacher.getTaughtPrograms()
                    .order_by("pk")
                    .exclude(id=self.program.id)
                ]
            )
    ...
    def send_class_mail_to_directors(self, cls, create=True):
        ...
        if recipients:
            send_mail(
                subject,
                render_to_string(
                    "program/modules/teacherclassregmodule/classreg_email", mail_ctxt
                ),
                (
                    ESPUser.email_sendto_address(
                        self.program.director_email,
                        f"{self.program.program_type} Class Registration",
                    )
                ),
                recipients,
                False,
        ...
        if self.program.director_email:
            ...
            send_mail(
                ...
                [self.program.getDirectorCCEmail()],
                False,
            )
