...
class CatalogSortTest(ProgramFrameworkTest):
    ...
    def test_sort_by_earliest_start_ascending(self):
        ...
        cls_a = self._make_class(
            "Class A",
            [
                (datetime(2026, 1, 1, 10, 0), ClassStatus.ACCEPTED),
            ],
        )
        cls_b = self._make_class(
            "Class B",
            [
        ...
        catalog = ClassSubject.objects.catalog(
            self.program,
            order_args_override=["earliest_start", "id"],
        )
    ...
    def test_cancelled_section_excluded_from_sort(self):
        ...
        self.assertEqual(catalog[0].id, cls_a.id, "Class A (10am) should be first")
        self.assertEqual(catalog[1].id, cls_b.id, "Class B (11am) should be second")
        self.assertEqual(
            catalog[2].id,
            cls_c.id,
    ...
    def test_multi_section_no_duplicates(self):
        ...
        self.assertEqual(
            len(ids), 2, "Multi-section class should not produce duplicates"
        )
