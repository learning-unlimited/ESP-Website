...
class ProgramCreationForm(BetterModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.program_module_question_ids = OrderedDict([('Will you charge for items such as shirts or lunch?', [x.id for x in ProgramModule.objects.filter(admin_title__in=['Student Optional Fees', 'Accounting', 'Financial Aid Application', 'Easily Approve Financial Aid Requests'])]),
                                                        ('If you will charge for admission or other costs, will you accept payment by credit card?', [x.id for x in ProgramModule.objects.filter(admin_title__in=['Credit Card Payment Module (Cybersource)', 'Credit Card View Module'])]),
                                                        ('Do you want a pre-program quiz for teachers? (Directors usually have this initially disabled, then enable it after scheduling weekend.)', [x.id for x in ProgramModule.objects.filter(admin_title='Teacher Logistics Quiz')]),
                                                        ...
                                                        ('Do you want to show teachers the classes that other teachers have registered (including test classes, etc.) during teacher registration?', [x.id for x in ProgramModule.objects.filter(admin_title='Teacher Class Previewing')]),
                                                        ...
                                                        (mark_safe('Will you use lottery admission (as opposed to first come, first served) for <b>classes</b>? (Two-Phase Student Registration module)'), [x.id for x in ProgramModule.objects.filter(admin_title__in=['Two-Phase Student Registration', 'Lottery Frontend'])]),
                                                        (mark_safe('Will you use lottery registration (as opposed to first come, first served) to the <b>program</b>? (Phase Zero module)'), [x.id for x in ProgramModule.objects.filter(admin_title__in=['Student Registration Phase Zero', 'Manage Student Registration Phase Zero'])]),
