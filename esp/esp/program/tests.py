...
class ProgramFrameworkTest(TestCase):
    ...
    def setUp(self, *args, **kwargs):
        ...
        prog_form_values = {
                ...
                'teacher_reg_start': (timezone.now() - timedelta(days=365)),
                'teacher_reg_end':   (timezone.now() + timedelta(days=365)),
                'student_reg_start': (timezone.now() - timedelta(days=365)),
                'student_reg_end':   (timezone.now() + timedelta(days=365)),
    ...
    def create_past_program(self):
        ...
        prog_form_values = {
                ...
                'class_categories': [x.id for x in self.categories],
                'admins': [x.id for x in self.admins],
                'teacher_reg_start': timezone.make_aware(datetime(1901, 1, 1, 0, 0, 0)),
                'teacher_reg_end':   timezone.make_aware(datetime(2000, 1, 1, 0, 0, 0)),
                'student_reg_start': timezone.make_aware(datetime(1901, 1, 1, 0, 0, 0)),
                'student_reg_end':   timezone.make_aware(datetime(2000, 1, 1, 0, 0, 0)),
                'base_cost':         '666',
                'finaid_cost':       '37',
            }
        pcf = ProgramCreationForm(prog_form_values)
