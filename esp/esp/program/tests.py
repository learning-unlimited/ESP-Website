...
class ProgramFrameworkTest(TestCase):
    ...
    def setUp(self, *args, **kwargs):
        ...
        for i in range(settings['num_onsites']):
            new_onsite, created = ESPUser.objects.get_or_create(username='onsite%04d' % i)
            new_onsite.set_password('password')
            new_onsite.save()
            role_bit, created = UserBit.objects.get_or_create(user=new_onsite, verb=GetNode('V/Registration/OnSite'), qsc=GetNode('Q/Programs/%s/%s' % (settings['program_type'], settings['program_instance_name'])), recursive=False)
            self.onsites.append(ESPUser(new_onsite))
