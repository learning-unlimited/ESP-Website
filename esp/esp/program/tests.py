...
class ViewUserInfoTest(TestCase):
    ...
    def testUserIDSearchOneResult(self):
        ...
        self.assertStringContains(response.url, "/manage/userview?username=adminuser124353")
    ...
    def testUserSearchFn(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertStringContains(response.url, "/manage/userview?username=notanadminuser124353")

        # Try some fuzzy searches
        # First name only, unique
        response = c.get("/manage/usersearch", { "userstr": self.unique_name })
        self.assertEqual(response.status_code, 302)
        self.assertStringContains(response.url, "/manage/userview?username=testuser123543")
