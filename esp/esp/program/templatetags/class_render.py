...
render_class_core.cached_function.depend_on_row(
    ClassSection, lambda sec: {"cls": sec.parent_class}
)
render_class_core.cached_function.depend_on_cache(
    ClassSection.num_students,
    lambda self=wildcard, **kwargs: {"cls": self.parent_class},
)
render_class_core.cached_function.depend_on_m2m(
    ClassSection, "meeting_times", lambda sec, ts: {"cls": sec.parent_class}
)
render_class_core.cached_function.depend_on_row(
    StudentAppQuestion, lambda ques: {"cls": ques.subject}
)
render_class_core.cached_function.depend_on_m2m(
    ClassSubject, "teachers", lambda cls, user: {"cls": cls}
)
render_class_core.cached_function.depend_on_row(
    QSDMedia,
    lambda media: {"cls": media.owner},
    lambda media: isinstance(media.owner, ClassSubject),
)
...
render_class.cached_function.depend_on_cache(
    render_class_core.cached_function, lambda cls=wildcard, **kwargs: {"cls": cls}
)
...
render_class.cached_function.depend_on_row(
    "program.StudentRegistration", lambda reg: {"user": reg.user}
)
...
def render_class_webapp(
    cls, prog, user=None, filter=False, timeslot=None, checked_in=False
):
    """Render the entire class for the webapp, including user-specific parts.

    Calls render_class_core for non-user-specific parts.
    """
    context = _render_class_helper(
        cls, user, filter, timeslot, webapp=True, prereg_suffix="onsiteaddclass"
    )
    context["checked_in"] = checked_in
    return context


render_class_webapp.cached_function.depend_on_cache(
    render_class_core.cached_function, lambda cls=wildcard, **kwargs: {"cls": cls}
)
...
# affect whether you can add a class in this one.  So we depend on all SRs for
# this user.  This only applies to tags that can depend on a user.
render_class_webapp.cached_function.depend_on_row(
    "program.StudentRegistration", lambda reg: {"user": reg.user}
)
...
def render_class_direct(cls):
    ...
    return render_to_string(
        "inclusion/program/class_catalog.html", _render_class_helper(cls)
    )


render_class_direct.depend_on_cache(
    render_class_core.cached_function, lambda cls=wildcard, **kwargs: {"cls": cls}
)


def _render_class_helper(
    cls, user=None, filter=False, timeslot=None, webapp=False, prereg_suffix="addclass"
):
...
render_class_preview.cached_function.depend_on_row(
    ClassSubject, lambda cls: {"cls": cls}
)
render_class_preview.cached_function.depend_on_m2m(
    ClassSubject, "teachers", lambda cls, user: {"cls": cls}
)
...
render_class_row.cached_function.depend_on_row(
    ClassSection, lambda sec: {"cls": sec.parent_class}
)
render_class_row.cached_function.depend_on_cache(
    ClassSection.num_students,
    lambda self=wildcard, **kwargs: {"cls": self.parent_class},
)
render_class_row.cached_function.depend_on_m2m(
    ClassSubject, "teachers", lambda cls, user: {"cls": cls}
)
render_class_row.cached_function.depend_on_m2m(
    ClassSection, "meeting_times", lambda sec, ts: {"cls": sec.parent_class}
