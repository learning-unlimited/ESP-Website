...
urlpatterns += [
    ...
    re_path(
        r"^admin/ajax_qsd_version_preview/?$", esp.qsd.views.ajax_qsd_version_preview
    ),
    ...
    re_path(
        ...
        RedirectView.as_view(url="/%(subsection)s/index.html", permanent=True),
    ),
]
...
urlpatterns += [
    ...
    re_path(
        r"^set_csrf_token", main.set_csrf_token, name="set_csrf_token"
    ),  # tiny view used to set csrf token
]
...
urlpatterns += [
    ...
    re_path(r"^theme/?$", RedirectView.as_view(url="/themes/", permanent=False)),
    re_path(r"^themes", include(esp.themes.urls)),
    re_path(
        r"^myesp$", RedirectView.as_view(url="/myesp/accountmanage/", permanent=False)
    ),
...
urlpatterns += [
    ...
    re_path(
        r"^(contact|contact\.html)$", main.ContactUsView.as_view(), name="Contact Us"
    ),
]
...
urlpatterns += [
    ...
    re_path(
        r"^contact/contact/(?P<section>[^/]+)/?$", main.contact, name="contact_section"
    ),
    # Program stuff
    re_path(
        r"^(onsite|manage|teach|learn|volunteer|json)/([-A-Za-z0-9_ ]+)/([-A-Za-z0-9_ ]+)/([-A-Za-z0-9_ ]+)/([-A-Za-z0-9_ ]+)/?$",
        main.program,
    ),
    re_path(
        r"^(onsite|manage|teach|learn|volunteer|json)/([-A-Za-z0-9_ ]+)/([-A-Za-z0-9_ ]+)/([-A-Za-z0-9_ ]+)/?$",
        main.program,
    ),
    # all the archives
    ...
    re_path(
        r"^archives/([-A-Za-z0-9_ ]+)/([-A-Za-z0-9_ ]+)/([-A-Za-z0-9_ ]+)/?$",
        main.archives,
    ),
...
urlpatterns += [
    re_path(
        r"^(?P<subsection>onsite|manage|teach|learn|volunteer)/(?P<program>[-A-Za-z0-9_ ]+)/?$",
        RedirectView.as_view(
            url="/%(subsection)s/%(program)s/index.html", permanent=True
        ),
    )
]
...
urlpatterns += [
    re_path(
        r"^manage/templateoverride/default_content/?$",
        esp.utils.views.get_default_template_content,
    ),
    re_path(
        r"^manage/templateoverride/(?P<template_id>[0-9]+)",
        esp.utils.views.diff_templateoverride,
        name="diff_templateoverride",
    ),
]
