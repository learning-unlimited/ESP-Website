...
class Migration(DataMigration):
    """ Identical to the migration 0004_set_owner, but sets the content type
        explicitly to avoid a failure in assigning the owner due a mismatch
        between the ORM frozen model and the original model.    """
    ...
    def forwards(self, orm):
        ...
        for media in orm['qsdmedia.Media'].objects.all():
            if orm['program.Program'].objects.filter(anchor=media.anchor).count() == 1:
                target_prog = orm['program.Program'].objects.filter(anchor=media.anchor)[0]
                ...
                media.save()
                count_program += 1
            if orm['program.ClassSubject'].objects.filter(anchor=media.anchor).count() == 1:
                target_subj = orm['program.ClassSubject'].objects.filter(anchor=media.anchor)[0]
    ...
    models = {
        ...
        'program.registrationprofile': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'last_ts': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 1, 27, 0, 0)'}),
    ...
    complete_apps = ['program', 'qsdmedia']
    symmetrical = True
