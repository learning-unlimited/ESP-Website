...
class MyESPStopTestingValidAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        self.admin = _make_user("stopadmin", role="Administrator", is_superuser=True)
    ...
    def test_valid_admin_redirects_to_manage(self):
        ...
        request = self._make_request_with_testing_session(self.admin.pk, "test/prog")
        # auth_login/auth_logout are local imports inside myesp_stop_testing;
        # patch at the source module.
        with patch("django.contrib.auth.login"), patch("django.contrib.auth.logout"):
...
class ProfileEditorGetTest(TestCase):
    ...
    def test_get_populates_first_name(self):
        ...
        teacher = _make_user(
            "profile_fn_teacher", role="Teacher", first_name="Alice", last_name="Smith"
        )
        request = self._get_request(teacher)
        response = profile_editor(request, role="teacher")
        response.render()
    ...
    def test_get_administrator_role_returns_200(self):
        ...
        admin = _make_user("profile_admin_get", role="Administrator", is_superuser=True)
