...
class Command(BaseCommand):
    help = "Migrate file extensions to lowercase for case-insensitive file handling"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Starting file extension migration to lowercase...")
        logger.info("File extension migration started (dry_run=%s)", dry_run)
        ...
        for root, dirs, files in os.walk(media_root):
            for filename in files:
                ...
                if ext and ext != ext.lower():
                    ...
                    if verbose:
                        self.stdout.write(f"  Found: {rel_path} -> {new_filename}")
                        logger.debug(
        ...
        try:
            with transaction.atomic():
                    ...
                    try:
                        ...
                        if old_path.lower() == new_path.lower():
                            ...
                            if os.path.exists(old_path):
                                ...
                                old = file_info["old_path"]
                                new = file_info["new_filename"]
                                logger.info("File renamed: %s -> %s", old, new)
                ...
                if renamed_files:
                    ...
                    msg = f"\nUpdating database references for {count} files..."
                    self.stdout.write(msg)
                    self._update_database_references(renamed_files, verbose)

                count = len(renamed_files)
                msg = self.style.SUCCESS(
                    f"\nSuccessfully migrated {count} files to lowercase extensions."
                )
                self.stdout.write(msg)
                logger.info(
            ...
            logger.error(msg, exc_info=True)
            warn_msg = (
                "Due to transaction atomicity, file renames may have been rolled back."
            )
    ...
    def _update_database_references(self, renamed_files, verbose):
        ...
        try:
                            ...
                            try:
                                ...
                                for old_path, new_path in file_mapping.items():
                                    if old_path in old_value_str:
                                        new_value_str = old_value_str.replace(
                                            old_path, new_path
                                        )
                                        setattr(obj, field.name, new_value_str)
            ...
            error_msg = (
                "Error iterating through models during database update: {}".format(
                    str(e)
                )
            )
            errors.append(error_msg)
        ...
        if errors:
            error_summary = (
                f"Completed with {len(errors)} error(s) during database updates."
            )
            self.stdout.write(self.style.WARNING(error_summary))
            logger.warning(error_summary)

        self.stdout.write(f"Updated {updates_count} database reference(s).")
        logger.info("Updated %d database reference(s)", updates_count)
