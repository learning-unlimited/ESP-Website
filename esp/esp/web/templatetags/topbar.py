...
def get_primary_nav(context):
    ...
    if retVal and cache_key:
        return {"page_setup": retVal, "user": curuser}
    ...
    if len(path_list) == 2 and path.lower() in [x[2] for x in sections.values()]:
        ...
        page_setup["stylesheet"] = [
            x for x in basic_navlinks if sections[x][0] == path_list[0]
        ][0] + "2"
        for section in basic_navlinks:
            page_setup["navlinks"].append(
                {
                    "id": section,
        ...
        return {"page_setup": page_setup, "user": curuser}
            ...
            page_setup["navlinks"].append(
                {
                    "id": section,
                    "alt": sections[section][1],
                    "highlight": path_list[0] == sections[section][0]
                    and sections[section][4],
                    "href": sections[section][2],
...
sections = {
    "discoveresp": ("about", "Discover ESP", "/about/index.html", [], True),
    "takeaclass": (
        "learn",
        "Take a Class!",
        "/learn/index.html",
        ["getinvolved", "volunteertoteach"],
        True,
    ),
    "volunteertoteach": (
        "teach",
    ...
    "archivesresources": (
        ...
        "ESP Archives",
        "/archives/index.html",
        ["takeaclass", "getinvolved", "volunteertoteach"],
        True,
    ),
    "myesp": (
        "myesp",
