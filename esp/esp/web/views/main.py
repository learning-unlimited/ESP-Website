...
def program(request, tl, one, two, module, extra=None):
    ...
    if two == "current":
        try:
            ...
            progs = [
                (program, program.dates()[-1])
                for program in programs
                if program.program_type == one and len(program.dates()) > 0
            ]
    ...
    newResponse = ProgramModuleObj.findModule(
...
def public_email(request, email_id):
    ...
    if email_req:
        return render_to_response(
            "public_email.html", request, {"email_req": email_req}
        )
    else:
...
def archives(request, selection, category=None, options=None):
    ...
    if request.POST and "newparam" in request.POST:
        ...
        for key in request.POST:
            if (
                key.startswith("sortparam")
                and request.POST[key] != request.POST["newparam"]
            ):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not anonymous:
                ...
                usernames = ESPUser.objects.filter(email__iexact=email).values_list(
                    "username", flat=True
                )

                if usernames and not form.cleaned_data["decline_password_recovery"]:
                    m = "password|account|log( ?)in"
                    if re.search(m, form.cleaned_data["message"].lower()) or re.search(
                        m, form.cleaned_data["subject"].lower()
                    ):
                ...
                if len(form.cleaned_data["name"].strip()) > 0:
                    email = ESPUser.email_sendto_address(
                ...
                email = settings.CONTACTFORM_EMAIL_ADDRESSES[
                    form.cleaned_data["topic"].lower()
                ]

            to_email.append(
                settings.CONTACTFORM_EMAIL_ADDRESSES[form.cleaned_data["topic"].lower()]
            )
            ...
            if ok_to_send:
                ...
                send_mail(
                    SUBJECT_PREPEND + " " + form.cleaned_data["subject"],
                    msgtext,
                    email,
                    to_email,
                    fail_silently=True,
...
def registration_redirect(request):
    ...
    if user.isTeacher():
        ...
        for perm in student_perms:
            progs_set |= set(
                Permission.program_by_perm(user, perm).filter(
                    grade_min__lte=user_grade, grade_max__gte=user_grade
                )
            )
        progs = list(progs_set)
    ...
    if len(progs) == 1 and Tag.getBooleanTag("automatic_registration_redirect"):
        ...
        return HttpResponseRedirect(
            f"/{userrole['base']}/{progs[0].getUrlBase()}/{userrole['reg']}"
        )
    else:
        if len(progs) > 0:
