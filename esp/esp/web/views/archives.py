...
def extract_criteria(postvars):
    ...
    for key in postvars.keys():
        if len(postvars[key]) > 0 and key.find("filter_") != -1 and len(key) > 7:
            criteria.append(ArchiveFilter(category=key[7:], options=postvars[key]))
...
def archive_classes(request, category, options, sortorder=None):
    ...
    category_list = list(
        ArchiveClass.objects.all().values_list("category", flat=True).distinct()
    )
    program_list = list(
        ArchiveClass.objects.all().values_list("program", flat=True).distinct()
    )
    ...
    filter_keys = {
        "category": [{"name": c, "value": c, "selected": False} for c in category_list],
        "year": [
            {"name": str(y), "value": str(y), "selected": False}
            for y in range(1998, datetime.now().year + 1)
        ],
        "title": [
            {"name": "Starts with " + letter, "value": letter, "selected": False}
            for letter in map(chr, list(range(65, 91)))
        ],
    ...
    if "filter_teacher" in request.POST:
        filter_keys["teacher"][0]["default_value"] = request.POST["filter_teacher"]
    if "filter_description" in request.POST:
        filter_keys["description"][0]["default_value"] = request.POST[
            "filter_description"
        ]
    ...
    context["sortparams"] = [
        {"name": k, "options": filter_keys[k]} for k in filter_keys.keys()
    ]
    ...
    if sortorder[0] == "title":
        headings = [
            title_heading(item.__dict__["title"])
            for item in results[res_range["start"] : res_range["end"]]
        ]
    else:
        headings = [
            item.__dict__[sortorder[0]]
            for item in results[res_range["start"] : res_range["end"]]
        ]
