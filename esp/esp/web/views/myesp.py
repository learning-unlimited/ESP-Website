...
def myesp_passwd(request):
    ...
    return render_to_response(
        "users/passwd.html", request, {"Problem": False, "form": form, "Success": False}
    )
...
def edit_profile(request):
    ...
    if curUser.isTeacher():
        ...
        return profile_editor(
            request, None, True, user_types[0].name.lower() if user_types else ""
        )
...
def profile_editor(request, prog_input=None, responseuponCompletion=True, role=""):
    ...
    additional_types = [
        ["", {"label": "Not specified", "profile_form": "UserContactForm"}],
        [
            "Administrator",
            {"label": "Administrator", "profile_form": "UserContactForm"},
        ],
    ]
    ...
    if role_lower in user_type_labels_lower:
        ...
        target_type = additional_types[additional_type_labels_lower.index(role_lower)][
            1
        ]
    else:
    ...
    mod = __import__(
        "esp.users.forms.user_profile", (), (), target_type["profile_form"]
    )
    ...
    if request.method == "POST" and "profile_page" in request.POST:
        ...
        if form.is_valid():
            ...
            regProf.contact_user = ContactInfo.addOrUpdate(
                curUser, regProf, new_data, regProf.contact_user, ""
            )
            ...
            if role == "student":
                regProf.student_info = StudentInfo.addOrUpdate(
                    curUser, regProf, new_data
                )
                regProf.contact_guardian = ContactInfo.addOrUpdate(
                    curUser, regProf, new_data, regProf.contact_guardian, "guard_"
                )
                regProf.contact_emergency = ContactInfo.addOrUpdate(
                    curUser, regProf, new_data, regProf.contact_emergency, "emerg_"
                )
            elif role == "teacher":
                regProf.teacher_info = TeacherInfo.addOrUpdate(
                    curUser, regProf, new_data
                )
            elif role == "guardian":
                regProf.guardian_info = GuardianInfo.addOrUpdate(
                    curUser, regProf, new_data
                )
            elif role == "educator":
                regProf.educator_info = EducatorInfo.addOrUpdate(
                    curUser, regProf, new_data
                )
            regProf.save()
            ...
            try:
                replacement_data["k12school"] = (
                    form.fields["k12school"].clean(form.data["k12school"]).id
                )
            except Exception:
                pass
            ...
            if not Tag.getBooleanTag("allow_change_grade_level"):
                if regProf.student_info:
                    ...
                    if (
                        regProf.student_info.graduation_year
                        and "graduation_year" in form.fields
                    ):
                        form.data["graduation_year"] = (
                            regProf.student_info.graduation_year
                        )
        ...
        if "k12school" in new_data and isinstance(new_data["k12school"], str):
