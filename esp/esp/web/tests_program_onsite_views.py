...
class ProgramViewMissingTest(ProgramFrameworkTest):
    ...
    def test_valid_program_no_module_raises_404(self):
        ...
        request = _get(
            self.factory,
            "/learn/%s/%s/nonexistent_module/"
            % (prog.program_type, prog.program_instance),
            self.students[0],
        )
        with self.assertRaises(Http404):
            program(
    ...
    def test_current_resolves_to_program_instance(self):
        ...
        prog = self.program
        request = _get(
            self.factory, "/learn/%s/current/" % prog.program_type, self.students[0]
        )
        # Http404 must be raised for the missing module, not for missing program.
        with self.assertRaises(Http404) as cm:
            program(
                request, "learn", prog.program_type, "current", "nonexistent_module"
            )
    ...
    def test_program_type_mismatch_raises_404(self):
        ...
        request = _get(
            self.factory,
            "/learn/WrongType/%s/" % prog.program_instance,
            self.students[0],
        )
        with self.assertRaises(Http404):
