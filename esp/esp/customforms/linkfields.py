...
generic_fields = {
    "textField": {
        "typeMap": forms.CharField,
        "attrs": {
            "widget": forms.TextInput,
        },
        "widget_attrs": {"size": "30", "class": ""},
    },
    "longTextField": {
        "typeMap": forms.CharField,
        "attrs": {
    ...
    "longAns": {
        ...
        "widget_attrs": {"rows": "8", "cols": "50", "class": ""},
    },
    "reallyLongAns": {
        "typeMap": forms.CharField,
        "attrs": {
        ...
        "widget_attrs": {"rows": "14", "cols": "70", "class": ""},
    },
    "radio": {
        "typeMap": forms.ChoiceField,
        "attrs": {
    ...
    "multiselect": {
        "typeMap": forms.MultipleChoiceField,
        "attrs": {
    ...
    "numeric": {
        "typeMap": forms.IntegerField,
        "attrs": {
            "widget": forms.TextInput,
        },
        "widget_attrs": {"class": "digits "},
    },
    "date": {
        "typeMap": forms.DateField,
        "attrs": {
            "widget": forms.DateInput,
        },
        "widget_attrs": {"class": "ddate ", "format": "%m-%d-%Y"},
    },
    "time": {
        "typeMap": forms.TimeField,
        "attrs": {
            "widget": forms.TimeInput,
        },
        "widget_attrs": {"class": "time "},
    },
    "file": {
        "typeMap": forms.FileField,
        "attrs": {
            "widget": CustomFileWidget,
        },
        "widget_attrs": {"class": "file"},
    },
    "phone": {
        "typeMap": PhoneNumberField,
        "attrs": {
            "widget": PhoneNumberInternationalFallbackWidget,
        },
        "widget_attrs": {"class": "USPhone "},
    },
    "email": {
        "typeMap": forms.EmailField,
        "attrs": {
            "max_length": 30,
            "widget": forms.TextInput,
        },
    ...
    "state": {
        "typeMap": USStateField,
        "attrs": {"widget": USStateSelect},
        "widget_attrs": {"class": ""},
    },
    "gender": {
        "typeMap": forms.ChoiceField,
    ...
    "pronoun": {
        ...
        "widget_attrs": {"size": "50", "class": "pronoun "},
    },
    "radio_yesno": {
        "typeMap": forms.ChoiceField,
        "attrs": {"widget": forms.RadioSelect, "choices": (("T", "Yes"), ("F", "No"))},
        "widget_attrs": {"class": ""},
    },
    "boolean": {
        "typeMap": forms.BooleanField,
        "attrs": {"widget": forms.CheckboxInput},
        "widget_attrs": {"class": ""},
    },
    "null_boolean": {
        "typeMap": forms.TypedChoiceField,
        "attrs": {
            "widget": forms.NullBooleanSelect,
            "choices": (("1", "Unknown"), ("2", "Yes"), ("3", "No")),
            "coerce": coerce_null_boolean,
            "empty_value": None,
        },
        "widget_attrs": {"class": ""},
    },
...
class CustomFormsCache:
        ...
        for model in apps.get_models():
            if CustomFormsLinkModel in model.__bases__:
                if not hasattr(model, "link_fields_list"):
                    ...
                    self.link_fields[model.form_link_name] = {
                        "model": model,
                        "fields": {},
                    }
                    # Now getting the fields
                    all_form_fields = fields_for_model(
                        model, widgets=getattr(model, "link_fields_widgets", None)
                    )
                    ...
                    for field, display_name in sublist:
                        ...
                        if field in all_form_fields:
                            ...
                            field_instance = self.getCustomFieldInstance(
                                field, f"{model.form_link_name}_{field}"
                            )
                            generic_field_type = "custom"

                        model_field = field
                        if (
                            hasattr(model, "link_compound_fields")
                            and field_name in model.link_compound_fields
                        ):
                            model_field = model.link_compound_fields[field_name]

                        self.link_fields[model.form_link_name]["fields"].update(
                            {
                                field_name: {
                                    "model_field": field,
    ...
    def getGenericType(self, field_instance):
        ...
        backup_type = "custom"
        for k, v in generic_fields.items():
            if widget is v["attrs"]["widget"] or (
                widget.__class__ is v["attrs"]["widget"]
            ):
                return k
            try:
    ...
    def isCompoundLinkField(self, model, field):
        if (
            hasattr(model, "link_compound_fields")
            and field in model.link_compound_fields
        ):
            return True
        else:
            return False
