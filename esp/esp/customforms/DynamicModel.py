...
class DynamicModelHandler:
    ...
    def _getModelField(self, field_type):
        ...
        """
        if cf_cache.isLinkField(field_type):
            model = cf_cache.modelForLinkField(field_type)
            return self._getLinkModelField(model)
        elif self._field_types[field_type]['typeMap']:
