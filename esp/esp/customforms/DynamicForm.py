...
class CustomFormHandler:
    ...
    def _getAttrs(self, attrs):
        ...
        for attr in attrs:
            if attr["attr_type"] == "options":
                ...
                limits = attr["value"].split(",")
                if limits[0]:
                    other_attrs["min_length"] = int(limits[0])
                if limits[1]:
                    other_attrs["max_length"] = int(limits[1])
    ...
    def _getFields(self):
        ...
        for section in self.page:
            curr_fieldset = []
            curr_fieldset.extend(
                [
                    section[0]["section__title"],
                    {
                        "fields": [],
                        "classes": [
                            "section",
            ...
            for field in section:
                ...
                field_attrs = {
                    ...
                    "help_text": mark_safe(field["help_text"]),
                ...
                for attr_name in field["attributes"]:
                    other_attrs.append(
                        {
                            "attr_type": attr_name,
                            "value": field["attributes"][attr_name],
                        }
                    )
                    ...
                    if (
                        attr_name == "correct_answer"
                        and len(field["attributes"]["correct_answer"].strip()) > 0
                    ):
                        if field["field_type"] in ["dropdown", "radio"]:
                            value_choices = field["attributes"]["options"].split("|")
                            target_value = value_choices[
                                int(field["attributes"]["correct_answer"])
                            ]
                        elif field["field_type"] in ["checkboxes", "multiselect"]:
                            value_choices = field["attributes"]["options"].split("|")
                            target_value = [
                                value_choices[int(index)]
                                for index in field["attributes"][
                                    "correct_answer"
                                ].split("|")
                            ]
                        elif field["field_type"] == "null_boolean":
                            target_value = {"Unknown": None, "Yes": True, "No": False}[
                                field["attributes"]["correct_answer"]
                            ]
                        elif field["field_type"] == "numeric":
                            target_value = float(field["attributes"]["correct_answer"])
                        elif field["field_type"] == "date":
                            target_value = datetime.strptime(
                                field["attributes"]["correct_answer"], "%Y-%m-%d"
                            ).date()
                        elif field["field_type"] == "time":
                            target_value = datetime.strptime(
                                field["attributes"]["correct_answer"], "%H:%M"
                            ).time()
                        else:
                ...
                if cf_cache.isLinkField(field["field_type"]):
                    ...
                    if link_model.__name__ not in model_fields_cache:
                        ...
                        model_fields_cache[link_model.__name__].update(
                            fields_for_model(
                                link_model,
                                widgets=getattr(
                                    link_model, "link_fields_widgets", None
                                ),
                            )
                        )

                    model_field = cf_cache.getLinkFieldData(field["field_type"])[
                        "model_field"
                    ]

                    field_is_custom = False
                    if model_field in model_fields_cache[link_model.__name__]:
                        form_field = model_fields_cache[link_model.__name__][
                            model_field
                        ]
                    else:
                        #   See if there's a custom field
                        if model_field in custom_fields:
                            form_field = cf_cache.getCustomFieldInstance(
                                model_field, field_name
                            )
                            field_is_custom = True
                        else:
                            raise Exception(
                                f"Could not find linked field: {model_field}"
                            )
                    ...
                    if not field_is_custom:
                        ...
                        if (
                            generic_type in self._field_types
                            and "widget_attrs" in self._field_types[generic_type]
                            and "class"
                            in self._field_types[generic_type]["widget_attrs"]
                        ):
                            form_field.widget.attrs["class"] += self._field_types[
                                generic_type
                            ]["widget_attrs"]["class"]
                ...
                if "widget_attrs" in self._field_types[field["field_type"]]:
                    widget_attrs.update(
                        self._field_types[field["field_type"]]["widget_attrs"]
                    )
                ...
                if field["field_type"] == "courses":
                    if (
                        self.form.link_type == "program"
                        or self.form.link_type == "Program"
                    ):
                        field_attrs["queryset"] = Program.objects.get(
                            pk=self.form.link_id
                        ).classsubject_set.all()

                # Initializing widget
                if field_attrs["widget"] is not None:
                    try:
                        field_attrs["widget"] = field_attrs["widget"](
                            attrs=widget_attrs
                        )
                    except KeyError:
                        pass
...
class ComboForm(SessionWizardView):
    ...
    def done(self, form_list, **kwargs):
            ...
            for k, v in form.cleaned_data.items():
                ...
                if cf_cache.isLinkField(ftype):
                    ...
                    if model.__name__ not in link_models_cache:
                        ...
                        pre_instance = self.form_handler.getInstanceForLinkField(
                            k, model
                        )
                        if pre_instance is not None:
                            link_models_cache[model.__name__]["instance"] = pre_instance
                        else:
                            link_models_cache[model.__name__]["instance"] = getattr(
                                model, "cf_link_instance"
                            )(self.curr_request)
                    ftype_parts = ftype.split("_")
                    if len(ftype_parts) > 1 and cf_cache.isCompoundLinkField(
                        model, "_".join(ftype_parts[1:])
                    ):
                        #   Try to match a model field to the last part of the key we have.
                        partial_field_name = (
                            str(field_id).join(k.split(str(field_id))[1:]).lstrip("_")
                        )
                        target_fields = cf_cache.getCompoundLinkFields(
                            model, "_".join(ftype_parts[1:])
                        )
        ...
        for key in initial_keys:
            ...
            if (
                key.split("_")[0] == "question"
                and generic_fields[fields[int(key.split("_")[1])]]["typeMap"]
                == DummyField
            ):
                del data[key]
        ...
        return HttpResponseRedirect(
            kwargs.get("redirect_url", f"/customforms/success/{self.form.id}/")
        )
...
class FormHandler:
    ...
    def _getFormMetadata(self, form):
        ...
        fields = (
            ...
            .order_by("section__page__seq", "section__seq", "seq")
            .values(
                "id",
                "field_type",
                "label",
        ...
        for field in fields:
            ...
            if field["id"] not in field_dict:
                ...
                field_dict[field["id"]]["attributes"] = {
                    field["attribute__attr_type"]: field["attribute__value"]
                }
            else:
                field_dict[field["id"]]["attributes"].update(
                    {field["attribute__attr_type"]: field["attribute__value"]}
                )
        return master_struct

    _getFormMetadata.get_or_create_token(("form",))
    _getFormMetadata.depend_on_row(
        "customforms.Field", lambda field: {"form": field.form}
    )
    _getFormMetadata.depend_on_row(
        "customforms.Attribute", lambda attr: {"form": attr.field.form}
    )
    _getFormMetadata.depend_on_row(
        "customforms.Section", lambda section: {"form": section.page.form}
    )
            ...
            for section in page:
                for field in section:
                    if field["id"] == field_id:
                        ...
                        # -jmoldow, 2013-01-24
                        if (
                            field["attribute__value"] == "-1"
                            or not field["attribute__value"]
                        ):
                            return None
                        else:
                ...
                for k, v in initial.items():
                    if v["model"].__name__ not in link_models_cache:
                        ...
                        if pre_instance is not None:
                            ...
                            link_models_cache[v["model"].__name__] = getattr(
                                v["model"], "cf_link_instance"
                            )(self.request)
                        if link_models_cache[v["model"].__name__] is not None:
                            link_models_cache[v["model"].__name__] = link_models_cache[
                                v["model"].__name__
                            ].__dict__
                    if link_models_cache[v["model"].__name__] is not None:
                        if not isinstance(v["model_field"], list):
                            ...
                            initial_data[handler.seq].update(
                                {
                                    k: link_models_cache[v["model"].__name__][
                                        v["model_field"]
                                    ]
                                }
                            )
                            ...
                            initial_data[handler.seq].update(
                                {
                                    k: [
                                        link_models_cache[v["model"].__name__][val]
                                        for val in v["model_field"]
                                    ]
                                }
                            )
        return initial_data
    ...
    def getResponseData(self, form):
        ...
        if form.link_type != "-1":
            ...
            response_data["questions"].append(
                [f"link_{only_fkey_model.__name__}_id", form.link_type, "fk"]
            )
        else:
        ...
        for field in fields:
            ...
            if cf_cache.isLinkField(ftype):
                ...
                add_fields[qname] = [
                    model,
                    cf_cache.getLinkFieldData(ftype)["model_field"],
                ]
        ...
        if only_fkey_model is not None:
            fkey_ids = [
                response["link_%s_id" % only_fkey_model.__name__]
                for response in responses
                if response.get("link_%s_id" % only_fkey_model.__name__)
            ]
        ...
        for qname, data in add_fields.items():
            ...
            if model_name not in add_field_instances:
                link_ids = [
                    response.get("link_%s_id" % model_name)
                    for response in responses
                    if response.get("link_%s_id" % model_name)
                ]
        ...
        for response in responses:
            ...
            for qname, data in add_fields.items():
                ...
                if model_name not in link_instances_cache:
                    ...
                    link_instances_cache[model_name] = add_field_instances.get(
                        model_name, {}
                    ).get(link_id)
                ...
                if cf_cache.isCompoundLinkField(model, data[1]):
                    if link_instances_cache[model_name] is None:
                        ...
                        response[qname] = [
                            link_instances_cache[model_name].__dict__[x]
                            for x in cf_cache.getCompoundLinkFields(model, data[1])
                        ]
                else:
                    if link_instances_cache[model_name] is None:
                        response[qname] = ""
                    else:
                        response[qname] = link_instances_cache[model_name].__dict__[
                            data[1]
                        ]
    ...
    def rebuildData(self):
        ...
        if self.form.link_type == "Program" and self.form.link_id != -1:
            ...
            if tags.count() == 1:
                ...
                raise ESPError(
                    f"Custom form #{self.form.id} is linked to multiple registration modules for {prog.name}"
                )
            else:
                tl = ""
                module = ""
