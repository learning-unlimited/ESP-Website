...
class CustomFormsTest(TestCase):
    ...
    def tearDown(self):
        try:
            ...
            # Handle case where transaction is already failed or connection is closed
            # This can happen if a test fails before completion
            pass
    ...
    def testOwnershipPermissions(self):
        ...
        field = Field.objects.get(form=form, label='Q1')
        question_name = 'question_%d' % field.id
        dyn_model = DynamicModelHandler(form=form).createDynModel()
        dyn_model.objects.create(**{question_name: __file__})
        ...
        response = self.client.get("/customforms/bulkdownloadfiles/",
                                   {'form_id': form.id, 'question_name': question_name})
        self.assertEqual(response.status_code, 200)
