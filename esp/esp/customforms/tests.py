...
class CustomFormsTest(TestCase):
    ...
    def testForms(self):
        ...
        for target_field in form_data["pages"][0]["sections"][0]["fields"]:
            ...
            self.assertEqual(
                target_field["data"]["field_type"], stored_field.field_type
            )
        ...
        responses_initial[f"question_{field_id_map['Your phone no.']}"] = (
            "(201) 426-5797"
        )
        ...
        for key in responses_corrected:
            self.assertEqual(
                self.map_form_value(responses_corrected[key]),
                getattr(form_response, key),
            )
        ...
        for field_spec in indiv_response:
            if field_spec in [
                ...
                continue
            self.assertEqual(
                self.map_form_value(responses_corrected[field_spec]),
                indiv_response[field_spec],
            )
        self.assertTrue("questions" in response_data)
        self.assertEqual(
            len(response_data["questions"]), len(responses_initial) + 4
        )  #   provided fields plus user_id, user_display, user_email, username
...
class ViewResponseTest(TestCase):
    ...
    def test_unauthenticated_redirected(self):
        ...
        self.assertRedirects(
            response,
            "/accounts/login/?next=/customforms/responses/%d/" % self.admin_form.id,
        )
...
class SchemaLockTimeoutTest(TestCase):
    ...
    def _operational_error(self, sqlstate):
        """
        Returns a genuine OperationalError carrying the given SQLSTATE
        """
        try:
...
class FailedFormSaveRollbackTest(TestCase):
    ...
    def test_form_metadata_is_not_committed_when_the_table_cannot_be_made(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertIn("could not get a lock", json.loads(response.content)["message"])
