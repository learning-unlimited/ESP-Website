...
class ThemesTest(TestCase):
    ...
    def testSelector(self):
        ...
        self.assertTrue(
            len(
                re.findall(
                    r'<a href="/themes.*?Configure site appearance.*?</a>',
        ...
        images_backup_dir = os.path.join(
            settings.MEDIA_ROOT, "images", "theme_backup_for_tests"
        )
        ...
        try:
            ...
            for theme_name in tc.get_theme_names():
                ...
                response = self.client.post(
                    "/themes/select/", {"action": "select", "theme": theme_name}
                )
                self.assertEqual(response.status_code, 200)

                #   Supply more settings if the theme asks for them.
                if '<form id="theme_setup_form"' in str(
                    response.content, encoding="UTF-8"
                ):
                    field_matches = re.findall(
                        r'<(input id="\S+"|textarea).*?name="(\S+)".*?>',
                        str(response.content, encoding="UTF-8"),
                        flags=re.DOTALL,
                    )
    ...
    def testEditor(self):
        ...
        for theme_name in theme_names:
            theme_data_dir = os.path.join(
                settings.PROJECT_ROOT, "esp", "themes", "theme_data", theme_name
            )
            ...
            if os.path.exists(variables_filename):
                ...
                for varname, value in variables:
                    ...
                    self.assertTrue(
                        len(
                            re.findall(
                                rf'<input[^>]*\bname="{re.escape(varname)}"',
                                str(response.content, encoding="UTF-8"),
                                flags=re.I,
                            )
                        )
        ...
        def verify_linkcolor(color_str, absent_color_str=None):
            ...
            self.assertGreaterEqual(
                len(re.findall(regexp, css, flags=re.I)),
                1,
            ...
            if absent_color_str is not None:
                ...
                self.assertEqual(
                    len(re.findall(old_regexp, css, flags=re.I)),
                    0,
        ...
        config_dict = {
            "save": True,
            "linkColor": color_str1,
            "saveThemeName": "save_test",
        }
        response = self.client.post("/themes/customize/", config_dict, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def testEditorBootswatchApplyAndSaveLoad(self):
        ...
        try:
            ...
            config_dict = {
                "apply": True,
                "bootswatch_theme": bw_a,
                "linkColor": custom_color,
            }
            response = self.client.post("/themes/customize/", config_dict, follow=True)
            self.assertEqual(response.status_code, 200)
    ...
    def testRecompileThemeCreatesMissingCustomization(self):
        ...
        customization_file = os.path.join(
            themes_settings.themes_dir, f"{fake_customization_name}.less"
        )
...
class ScssPipelineTest(TestCase):
    ...
    def test_compile_css_produces_bs5_markers(self):
        ...
        self.assertIn(
            ".navbar-toggler", css, "Missing .navbar-toggler — BS5 not compiling"
        )
        self.assertIn(".card", css, "Missing .card — BS5 not compiling")
        self.assertIn(".bi", css, "Missing .bi — Bootstrap Icons not included")
        self.assertNotIn(
            ".glyphicon", css, ".glyphicon present — BS3 leaked into BS5 output"
        )
    ...
    def test_get_variable_defaults_roundtrip(self):
        ...
        for theme_name in self.tc.get_theme_names():
            ...
            self.assertGreater(
                len(defaults),
                0,
                f"{theme_name}: get_variable_defaults() returned empty dict",
            )
    ...
    def test_get_bootswatch_themes_returns_sorted_list(self):
        ...
        self.assertEqual(
            themes_list, sorted(themes_list), "Bootswatch theme list must be sorted"
        )
    ...
    def test_bootswatch_themes_have_required_scss_files(self):
        ...
        bootswatch_dist = os.path.normpath(
            os.path.join(
    ...
    def test_compile_css_with_scss_bootswatch_produces_valid_output(self):
        ...
        self.tc.compile_css(
            scss_themes[0], {}, self.css_filename, bootswatch_theme=themes_list[0]
        )
        with open(self.css_filename) as f:
            css = f.read()
        self.assertGreater(len(css), 10000)
        self.assertIn(
            "--bs-", css, "Compiled CSS should contain Bootstrap 5 custom properties"
        )
    ...
    def test_get_variable_defaults_raises_for_theme_without_scss(self):
        ...
        with mock.patch.object(
            controllers_mod, "_ALL_THEMES", frozenset({"fake_less_only"})
        ):
...
class EspOverriddenBehaviorTest(TestCase):
    ...
    def test_effective_defaults_include_bootswatch_derivation(self):
        ...
        bw_vars = self.tc.get_bootswatch_esp_vars(
            self.bw_theme, esp_theme=self.theme_name
        )
    ...
    def test_switching_bootswatch_theme_does_not_carry_over_stale_overrides(self):
        ...
        other_bw_themes = [
            t for t in self.tc.get_bootswatch_themes() if t != self.bw_theme
        ]
...
class FindScssVariablesSecurityTest(TestCase):
    ...
    def test_get_scss_names_unknown_theme_returns_global_files_only(self):
        ...
        self.assertTrue(
            all("theme_data" not in f for f in names),
    ...
    def test_find_theme_variables_uses_scss_pipeline(self):
        ...
        with mock.patch.object(
            self.tc, "find_scss_variables", return_value={"x": "1"}
        ) as mock_scss:
...
class CompileScssErrorPathTest(TestCase):
    ...
    def test_compile_scss_raises_on_nonzero_returncode(self):
        ...
        with mock.patch(
            "esp.themes.controllers.subprocess.Popen", return_value=mock_proc
        ):
...
class EnsureThemeMediaTest(TestCase):
    ...
    def test_placeholder_logo_is_replaced(self):
        ...
        with mock.patch(
            "esp.themes.controllers.PLACEHOLDER_LOGO_MD5",
