...
class SectionListTest(ProgramFrameworkTest):
    ...
    def test_process_routes_no_mailman(self):
        ...
        with (
            patch.object(handler, "process_nomailman") as mock_nm,
            patch.object(handler, "process_mailman") as mock_m,
        ):
            handler.process(None, self.cls.id, self.section_num, "class")
            mock_nm.assert_called_once_with(
                None, self.cls.id, self.section_num, "class"
            )
    ...
    def test_invalid_class_id(self):
        ...
        self.assertFalse(
            handler.send, "send should remain False for a non-existent class ID"
        )
    ...
    def test_invalid_section_num(self):
        ...
        self.assertFalse(
            handler.send, "send should remain False for an invalid section number"
        )
    ...
    @override_settings(
        USE_MAILMAN=True,
        DEFAULT_EMAIL_ADDRESSES={"mailman_moderator": "mod@example.com"},
    )
    def test_mailman_invalid_class_id(self):
        handler = _make_handler()
        handler.process_mailman(None, -9999, 1, "class")
        self.assertFalse(handler.send, "send should remain False for invalid class ID")
    ...
    def test_mailman_invalid_section_num(self):
        ...
        self.assertFalse(
            handler.send, "send should remain False for invalid section number"
        )
    ...
    def test_mailman_students_list(
        self,
        mock_site,
        mock_send_mail,
        mock_set_pwd,
        ...
        mock_load.assert_called_once_with(
            expected_list_name, "lists/class_mailman.config"
        )
    ...
    @patch("esp.dbmail.receivers.sectionlist.apply_list_settings")
    @patch("esp.dbmail.receivers.sectionlist.Site")
    def test_mailman_teachers_list(
        self,
        mock_site,
        mock_apply,
        mock_add_members,
        ...
        self.assertTrue(
            any("default_member_moderation" in settings for settings in apply_calls)
        )
        self.assertTrue(
            any("generic_nonmember_action" in settings for settings in apply_calls)
        )
