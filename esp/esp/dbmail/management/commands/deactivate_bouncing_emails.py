...
class Command(BaseCommand):
    help = 'Fetches hard bounces from SendGrid and disables emailing'

    def handle(self, *args, **options):
        api_key = getattr(settings, 'SENDGRID_API_KEY', None)
        if not api_key or not SendGridAPIClient:
            logger.info(
                "SENDGRID_API_KEY not configured or sendgrid package "
                "not installed. Skipping bounce polling."
            )
            return
        ...
        last_poll_str = Tag.getTag(
            'last_sendgrid_bounce_poll_time', default=str(default_start)
        )
        ...
        for bounce in bounces:
            ...
            is_hard_bounce = (
                status.startswith('5') or
                '5.' in status or
                'does not exist' in reason or
            ...
            if is_hard_bounce:
                ...
                if bounce_timestamp:
                    record.last_bounced_at = datetime.fromtimestamp(
                        bounce_timestamp
                    )
                else:
