...
class ClassListTest(ProgramFrameworkTest):
    ...
    def test_invalid_class_id(self):
        ...
        self.assertFalse(
            handler.send, "send should remain False for an invalid class identifier"
        )
    ...
    def test_mailman_teachers_list(
        ...
        self.assertTrue(
            any("acceptable_aliases" in settings for settings in apply_calls)
        )
    ...
    @patch("esp.dbmail.receivers.classlist.send_mail")
    @patch("esp.dbmail.receivers.classlist.Site")
    def test_mailman_no_archive_address(
        self,
        mock_site,
        mock_send_mail,
        mock_set_pwd,
        ...
        archive_calls = [
            call for call in mock_add_member.call_args_list if "archive" in str(call)
        ]
