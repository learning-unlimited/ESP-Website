...
class SendToSelfTest(CacheFlushTestCase):
    ...
    def test_no_profile_needed(self):
        ...
        self.assertFalse(RegistrationProfile.objects.filter(user=user).exists())
...
class SendtoFnDispatchTest(CacheFlushTestCase):
    ...
    def test_is_sendto_fn_name_choice(self):
        ...
        self.assertFalse(MessageRequest.is_sendto_fn_name_choice("send_to_nobody"))
    ...
    def test_non_callable_implementation_raises_improperly_configured(self):
        ...
        with (
            patch.object(MessageRequest, "SENDTO_FN_CHOICES", choices),
            patch.object(
                sendto_fns, "send_to_not_callable", "not a function", create=True
            ),
        ):
            with self.assertRaises(ImproperlyConfigured) as ctx:
    ...
    def test_esperror_carries_the_explanation(self):
        ...
        self.assertIn(settings.DEFAULT_EMAIL_ADDRESSES["support"], message)
