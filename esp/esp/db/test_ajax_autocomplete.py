    ...
    def test_ajax_str_contains_id_suffix(self):
        ...
        for entry in data["result"]:
            self.assertTrue(
                ...
                "ajax_str '%s' should end with ' (%s)'"
                % (entry["ajax_str"], entry["id"]),
            )
...
class AutocompleteESPUserSearchTest(TestCase):
    ...
    def _search(self, query):
        ...
        self.assertEqual(
            response.status_code, 200, "Autocomplete search should return HTTP 200 OK"
        )
        return response
