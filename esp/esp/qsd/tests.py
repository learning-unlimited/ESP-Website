...
from esp.qsd.seltests import (
    TestQsdCachePurging,
)  # Run Selenium tests with regular tests
...
class QSDCorrectnessTest(TestCase):
    ...
    def testUnauthorizedAccess(self):
        ...
        for user in unauthorized_users:
            ...
            self.assertEqual(
                str(messages[0]), "You don't have permission to edit this page."
            )
            ...
            self.assertEqual(len(messages), 1)
            self.assertEqual(
                str(messages[0]), "Sorry, you do not have permission to edit this page."
            )
...
class QSDVersionHistoryTest(TestCase):
    ...
    def setUp(self):
        self.admin, created = ESPUser.objects.get_or_create(
            username="qsd_history_admin"
        )
        self.admin.set_password("password")
        self.admin.save()
        self.admin.makeRole("Administrator")

        self.student, created = ESPUser.objects.get_or_create(
            username="qsd_history_student"
        )
    ...
    def test_history_requires_permission(self):
        ...
        response = self.client.get(
            "/admin/ajax_qsd_history", {"url": "learn/historytest"}
        )
    ...
    def test_history_empty_for_nonexistent(self):
        ...
        self.client.login(username="qsd_history_admin", password="password")
        response = self.client.get(
            "/admin/ajax_qsd_history", {"url": "learn/doesnotexist"}
        )
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_version_preview(self):
        ...
        response = self.client.get(
            "/admin/ajax_qsd_version_preview", {"version_id": old_version_id}
        )
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_version_preview_requires_permission(self):
        ...
        self.client.login(username="qsd_history_student", password="password")
        response = self.client.get(
            "/admin/ajax_qsd_version_preview", {"version_id": version_id}
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_restore_version(self):
        ...
        response = self.client.post(
            "/admin/ajax_qsd_restore", {"version_id": old_version_id}
        )
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_restore_requires_permission(self):
        ...
        self.client.login(username="qsd_history_student", password="password")
        response = self.client.post(
            "/admin/ajax_qsd_restore", {"version_id": version_id}
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_invalid_version_id_returns_400(self):
        ...
        self.client.login(username="qsd_history_admin", password="password")
        response = self.client.get(
            "/admin/ajax_qsd_version_preview", {"version_id": "abc"}
        )
...
class QSDImageUploadTest(TestCase):
    ...
    def test_upload_rejects_invalid_extension(self):
        ...
        self.client.login(username="upload_admin", password="password")
        f = self._make_image_file(
            name="malicious.exe", content_type="application/x-executable"
        )
        response = self.client.post(self.UPLOAD_URL, {"files[0]": f})
        self.assertEqual(response.status_code, 400)
        import json
    ...
    def test_multi_file_bad_second_no_orphan(self):
        ...
        bad = self._make_image_file(
            name="evil.exe", content_type="application/x-executable"
        )
        ...
        self.assertEqual(
            new_files,
            set(),
            f"Orphaned files found after failed batch upload: {new_files}",
        )
...
class QSDAdminAuthorTest(TestCase):
    def setUp(self):
        ...
        self.nav_category = NavBarCategory.objects.create(name="default")
    ...
    def test_admin_updates_author_on_edit(self):
        ...
        url = reverse("admin:qsd_quasistaticdata_change", args=[self.qsd.pk])
