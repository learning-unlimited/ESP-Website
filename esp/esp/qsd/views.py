    ...
    try:
        ...
        if match.func is not qsd:
            ...
            try:
                ...
                logger.debug(
                    "Internal render failed for QSD default: %s",
                    base_url,
                    exc_info=True,
                )
            default = cache.get(cache_key)
        ...
        logger.warning(
            "Unexpected error resolving QSD default: %s", base_url, exc_info=True
        )
        ...
        if have_edit:
            if action in (
                ...
                if default:
                    ...
                    qsd_rec.content = default.get(
                        "content", "Please insert your text here"
                    )
                else:
...
def ajax_qsd_preview(request):
    ...
    if not Permission.user_can_edit_qsd(request.user, url):
        return HttpResponse(
            content="Sorry, you do not have permission to preview this page.",
            status=403,
        )
...
def ajax_qsd_history(request):
    ...
    versions = (
        ...
        .order_by("-revision__date_created")[:50]
    )
    ...
    if user_ids:
        user_map = dict(
            ESPUser.objects.filter(pk__in=user_ids).values_list("pk", "username")
        )
...
def ajax_qsd_version_preview(request):
    ...
    ct = ContentType.objects.get_for_model(QuasiStaticData)
    try:
        version = Version.objects.select_related("revision").get(
            pk=version_pk, content_type=ct
        )
...
def ajax_qsd_restore(request):
    ...
    if "nav_category" in field_dict:
        ...
        if (
            nav_cat_id is not None
            and NavBarCategory.objects.filter(pk=nav_cat_id).exists()
        ):
