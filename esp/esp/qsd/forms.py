...
class QSDMoveForm(forms.Form):
    ...
    destination = forms.CharField(
        help_text="The portion of the URL that comes before the '.html'."
    )

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["nav_category"].choices = [
            (n.id, n.name) for n in NavBarCategory.objects.all()
        ]
    ...
    def save_data(self):
        ...
        other_qsds = QuasiStaticData.objects.filter(
            url=main_qsd.url, name=main_qsd.name
        )
        for qsd in other_qsds:
            #   Move them over
            qsd.url = self.cleaned_data["destination"]
            qsd.nav_category = NavBarCategory.objects.get(
                id=self.cleaned_data["nav_category"]
            )
            qsd.save()
...
class QSDBulkMoveForm(forms.Form):
    ...
    def save_data(self):
        qsd_list = QuasiStaticData.objects.filter(
            id__in=[int(x) for x in self.cleaned_data["id_list"].split(",")]
        )
