...
def set_my_defaults(apps, schema_editor):
    ...
    if recs.exists():
        for rec in recs:
            ...
            if (
                rec.user.groups.filter(name="Teacher").exists()
                and rec.program.program_modules.filter(
...
def reverse_func(apps, schema_editor):
    Record = apps.get_model("users", "Record")
    Record.objects.filter(event__contains="extra_form_done").update(
        event="extra_form_done"
    )
