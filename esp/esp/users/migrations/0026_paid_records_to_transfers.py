...
def set_my_defaults(apps, schema_editor):
    ...
    recs = Record.objects.filter(
        event="paid", program__isnull=False, user__isnull=False
    )
    for rec in recs:
        ...
        if amount_due:
            ...
            payments_lit = LineItemType.objects.filter(
                program__id=rec.program.id, for_payments=True
            ).order_by("-id")[0]
    ...
    for tf in tfs:
        ...
        if IndividualAccountingController(prog, user).has_paid(in_full=True):
            Record.objects.get_or_create(
                user=tf.user, program=tf.line_item.program, event="paid"
            )
    tfs.delete()
