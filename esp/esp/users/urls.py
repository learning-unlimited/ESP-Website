...
urlpatterns = [
    re_path(
        r"^register/?$",
        views.user_registration_phase1,
        name="esp.users.views.user_registration_phase1",
    ),
    re_path(
        r"^register/information/?$",
        views.user_registration_phase2,
        name="esp.users.views.user_registration_phase2",
    ),
    re_path(
        r"^register/check-email/?$",
        views.registration_live_email_check,
        name="check_email",
    ),
    re_path(
        r"^register/check-password/?$",
        views.registration_live_password_check,
        name="check_password",
    ),
    re_path(
        r"^register/check-username/?$",
        views.registration_live_username_check,
        name="check_username",
    ),
    re_path(
        r"^activate/?$",
        views.registration.activate_account_legacy,
        name="activate_account_legacy",
    ),
    re_path(
        r"^activate/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z\-]+)/?$",
        views.registration.activate_account,
        name="activate_account",
    ),
    re_path(
        r"^passwdrecover/(success)?/?$",
        views.initial_passwd_request,
        name="passwd_recover_success",
    ),
    re_path(r"^passwdrecover/?$", views.initial_passwd_request, name="passwd_recover"),
    re_path(
        r"^resetpassword/(?P<uidb64>[-\w]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]+)/$",
    ...
    re_path(
        r"^grade_change_request/?$",
        GradeChangeRequestView.as_view(),
    ...
    re_path(
        r"^unsubscribe/(?P<username>[^/]+)/(?P<token>[\w.:\-_=]+)/$",
        views.unsubscribe,
        name="unsubscribe",
    ),
    re_path(
        r"^unsubscribe_oneclick/(?P<username>[^/]+)/(?P<token>[\w.:\-_=]+)/$",
...
urlpatterns += [
    ...
    re_path(
        r"^accountmanage/?$", myesp.myesp_accountmanage, name="myesp_accountmanage"
    ),
