...
This function is overloaded to handle either one or two phase reg"""
    ...
    if form.is_valid():
        ...
        #   Record that activation is outstanding.  This has to happen before
        #   the token is generated, because the token hashes this state.
        if require_activation:
...
#   Matches the "_<key>" suffix the legacy scheme appended to the password
#   hash.  Anchored at the end, and no hasher in use here can emit an
#   underscore after the first "$" (salts are [a-zA-Z0-9], digests are base64
#   or hex), so this cannot damage an untouched hash.
LEGACY_KEY_SUFFIX = re.compile(r'_\d+$')
...
def _finish_activation(user):
    """Activate a pending account and clear the state that kept it pending."""
    #   Accounts created under the legacy scheme still carry "_<key>" on their
    #   password hash, which makes the hash unusable.  Those users reach this
    #   view if they ask the resend form for a fresh link, so strip it here.
    user.password = LEGACY_KEY_SUFFIX.sub('', user.password)
    user.is_active = True
    user.save()
...
def activate_account(request, uidb64, token):
    """Activate an account from an HMAC-signed link."""
    try:
    ...
    #   Fails for a link that was already used, for one that has passed
    #   PASSWORD_RESET_TIMEOUT, and for an account that was disabled on
    #   purpose rather than left pending.
...
def activate_account_legacy(request):
    """Activate an account from a pre-HMAC "?username=&key=" link.

    Kept so that activation emails already sitting in inboxes keep working.
    Safe to delete once PASSWORD_RESET_TIMEOUT has elapsed since the deploy
    that stopped issuing these links.
    """
    ...
    u.is_active = True
    u.save()
    PendingActivation.objects.filter(user=u).delete()
