...
def get_user_list(request, listDict2, extra=""):
    ...
    if "select_mailman" in request.POST:
        ...
        filterObj = PersistentQueryFilter.getFilterFromQ(
            Q(id__in=[x.id for x in all_list_members]),
            ESPUser,
            "Custom Mailman filter: " + ", ".join(lists),
        )

        if request.POST["submitform"] == "I want to search within this list":
            getUser, found = search_for_user(
                request,
            ...
            if found:
                if isinstance(getUser, User):
                    newfilterObj = PersistentQueryFilter.getFilterFromQ(
                        Q(id=getUser.id), ESPUser, f"User {getUser.username}"
                    )
                else:
                    newfilterObj = PersistentQueryFilter.getFilterFromQ(
                        filterObj.get_Q() & getUser, ESPUser, "Custom user filter"
                    )
            ...
            getUsers, found = get_user_checklist(
                request,
                ESPUser.objects.filter(filterObj.get_Q()).distinct(),
                filterObj.id,
            )
    ...
    if request.POST.get("submit_checklist") == "true":
        ...
        filterObj = PersistentQueryFilter.getFilterFromID(
            request.POST["extra"], ESPUser
        )
        getUsers, found = get_user_checklist(
            request, ESPUser.objects.filter(filterObj.get_Q()).distinct(), filterObj.id
        )
        if found:
    ...
    if request.POST.get("submit_user_list") == "true":
        ...
        opmapping = {
            ...
            "not": (
                lambda x: ~x
            ),  # aseering 7/12/2008 -- Should work with Django SVN HEAD
        ...
        for key in keys:
            if (
                request.POST.get("operator_" + key)
                and request.POST["operator_" + key] != "ignore"
            ):  # and it's not ignore (it should be 'and' or 'or')
                # We are adding to the list of 'and'd' lists and 'or'd' lists.
                separated[request.POST["operator_" + key]].append(
                    opmapping[request.POST["not_" + key]](listDict[key]["list"])
                )
        ...
        for i in range(len(separated["or"])):
            for j in range(len(separated["and"])):
                separated["or"][i] = opmapping["and"](
                    separated["or"][i], separated["and"][j]
                )
        ...
        filterObj = PersistentQueryFilter.getFilterFromQ(
            curList, ESPUser, request.POST["finalsent"]
        )
    ...
    if "advanced" in request.GET or not settings.USE_MAILMAN:
        ...
        for key, value in listDict.items():
            ...
            )  # prepare a nice list thing.

        arrLists.sort(reverse=True)

        return (
            render_to_response("users/create_list.html", request, {"lists": arrLists}),
            False,
        ...
        return (
            render_to_response(
                "users/select_mailman_list.html",
                request,
                {"public_lists": public_lists, "nonpublic_lists": nonpublic_lists},
            ),
            False,
        )  # No, we didn't find it yet...
...
def get_user_checklist(request, userList, extra="", nextpage=None, extra_context=None):
    ...
    if (
        "submit_checklist" in request.POST
        and request.POST["submit_checklist"] == "true"
    ):
        UsersQ = Q(id=-1)
    ...
    return (
        render_to_response("users/userchecklist.html", request, context),
        False,
    )  # make the checklist


def search_for_user(
    request, user_type="Any", extra="", returnList=False, add_to_context=None
):
    ...
    if len(users) == 1:
        ...
        if (
            request.GET.get("listokay") == "true"
            or request.GET.get("submitform") == "Use Filtered List"
        ):
