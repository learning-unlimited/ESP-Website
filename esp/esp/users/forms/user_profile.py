...
class DropdownOtherWidget(forms.MultiWidget):
    ...
    def __init__(self, choices, use_textarea=False, attrs=None):
        text_attrs = dict(attrs or {})
        text_attrs["style"] = text_attrs.get("style", "") + "margin-left: 10px;"
        widgets = (
            forms.Select(attrs=attrs, choices=choices),
            forms.Textarea(attrs=text_attrs)
            if use_textarea
            else forms.TextInput(attrs=text_attrs),
        )
...
class UserContactForm(FormUnrestrictedOtherUser, FormWithTagInitialValues):
    ...
    first_name = StrippedCharField(
        length=25, max_length=ESPUser._meta.get_field("first_name").max_length
    )
    last_name = StrippedCharField(
        length=30, max_length=ESPUser._meta.get_field("last_name").max_length
    )
    e_mail = forms.EmailField()
    phone_day = PhoneNumberField(required=False)
    phone_cell = PhoneNumberField(required=False)
    receive_txt_message = forms.TypedChoiceField(
    ...
    address_state = forms.ChoiceField(
        required=True,
        choices=list(zip(_states, _states)),
    ...
    address_country = forms.ChoiceField(
        ...
        + sorted(list(country_names.items()), key=lambda x: x[1]),
        widget=forms.Select(attrs={"class": "input-medium hidden"}),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            not Tag.getBooleanTag("text_messages_to_students")
            or not self.user.isStudent()
        ):
    ...
    def clean(self):
        super().clean()
        if self.user.isTeacher() or (
            self.user.isStudent() and Tag.getBooleanTag("require_student_phonenum")
        ):
            if "phone_day" in self.fields or "phone_cell" in self.fields:
                if (
                    self.cleaned_data.get("phone_day", "") == ""
                    and self.cleaned_data.get("phone_cell", "") == ""
                ):
                    raise forms.ValidationError(
                        "Please provide either a day phone or cell phone number in your personal contact information."
                    )
        if (
            self.cleaned_data.get("receive_txt_message", None)
            and self.cleaned_data.get("phone_cell", "") == ""
        ):
            raise forms.ValidationError(
                "Please specify your cellphone number if you ask to receive text messages."
            )
...
class EmergContactForm(FormUnrestrictedOtherUser):
    ...
    emerg_address_zip = StrippedCharField(
        length=5, max_length=5, widget=forms.TextInput(attrs={"class": "input-small"})
    )
    emerg_address_country = forms.ChoiceField(
        required=False,
        choices=[("", "(select a country)")]
        + sorted(list(country_names.items()), key=lambda x: x[1]),
        widget=forms.Select(attrs={"class": "input-medium hidden"}),
    ...
    def clean(self):
        ...
        if "emerg_phone_day" in self.fields or "emerg_phone_cell" in self.fields:
            if (
                self.cleaned_data.get("emerg_phone_day", "") == ""
                and self.cleaned_data.get("emerg_phone_cell", "") == ""
            ):
                raise forms.ValidationError(
...
class GuardContactForm(FormUnrestrictedOtherUser):
    ...
    def clean(self):
        ...
        if "guard_phone_day" in self.fields or "guard_phone_cell" in self.fields:
            if (
                self.cleaned_data.get("guard_phone_day", "") == ""
                and self.cleaned_data.get("guard_phone_cell", "") == ""
            ):
                raise forms.ValidationError(
                    "Please provide either a day phone or cell phone for your parent/guardian."
                )
        return self.cleaned_data
...
class StudentInfoForm(FormUnrestrictedOtherUser):
    ...
    k12school = AjaxForeignKeyNewformField(
        key_type=K12School,
    ...
    heard_about = DropdownOtherField(
        ...
        widget=DropdownOtherWidget(
            choices=list(zip(HEARD_ABOUT_ESP_CHOICES, HEARD_ABOUT_ESP_CHOICES))
        ),
    )  # forms.CharField(required=False)
    ...
    transportation = DropdownOtherField(
        required=False,
        widget=DropdownOtherWidget(
            choices=list(zip(HOW_TO_GET_TO_PROGRAM, HOW_TO_GET_TO_PROGRAM))
        ),
    )
    ...
    def __init__(self, user=None, *args, **kwargs):
        ...
        self.fields["shirt_size"].choices = [("", "")] + [
            (x.strip(), x.strip()) for x in Tag.getTag("student_shirt_sizes").split(",")
        ]
        self.fields["shirt_type"].choices = [("", "")] + [
            (x.strip(), x.strip()) for x in Tag.getTag("shirt_types").split(",")
        ]
        self.fields["food_preference"].choices = [("", "")] + [
            (x.strip(), x.strip()) for x in Tag.getTag("food_choices").split(",")
        ]
        ...
        if (
            not show_studentrep_application
        ...
        if user and user.registrationprofile_set.count() > 0:
            ...
            if (
                grade_tup not in self.fields["graduation_year"].choices
                and user_grade > 0
            ):
        ...
        if Tag.getBooleanTag("student_medical_needs"):
            self.fields["medical_needs"].widget = forms.Textarea(
...
class TeacherInfoForm(FormWithRequiredCss):
    ...
    reimbursement_choices = [
        (False, "I will pick up my reimbursement."),
    ...
    affiliation = DropdownOtherField(
        required=False,
        widget=DropdownOtherWidget(choices=AFFILIATION_CHOICES),
        label=f"What is your affiliation with {settings.INSTITUTION_NAME}?",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["shirt_size"].choices = [("", "")] + [
            (x.strip(), x.strip()) for x in Tag.getTag("teacher_shirt_sizes").split(",")
        ]
        self.fields["shirt_type"].choices = [("", "")] + [
            (x.strip(), x.strip()) for x in Tag.getTag("shirt_types").split(",")
        ]
    ...
    def clean(self):
        ...
        if "affiliation" in self.fields:
            ...
            affiliation, school = affiliation_field.widget.decompress(
                cleaned_data.get("affiliation")
            )
            if affiliation == "":
                msg = (
                    f"Please select your affiliation with {settings.INSTITUTION_NAME}."
                )
                ...
                cleaned_data["affiliation"] = affiliation_field.compress(
                    [affiliation, ""]
                )  # ignore the box
...
class StudentProfileForm(
    UserContactForm, EmergContactForm, GuardContactForm, StudentInfoForm
):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name in [
            x.strip().lower()
            for x in Tag.getTag("student_profile_hide_fields").split(",")
            if x.strip()
        ]:
...
class TeacherProfileForm(UserContactForm, TeacherInfoForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name in [
            x.strip().lower()
            for x in Tag.getTag("teacher_profile_hide_fields").split(",")
            if x.strip()
        ]:
            if field_name not in self.fields:
                logger.warning(
...
class GuardianProfileForm(UserContactForm, GuardianInfoForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name in [
            x.strip().lower()
            for x in Tag.getTag("guardian_profile_hide_fields").split(",")
            if x.strip()
        ]:
            if field_name not in self.fields:
                logger.warning(
            ...
            if (
                field_name
                not in _undeletable_fields_all + _undeletable_fields_guardians
            ):
                del self.fields[field_name]
...
class EducatorProfileForm(UserContactForm, EducatorInfoForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name in [
            x.strip().lower()
            for x in Tag.getTag("educator_profile_hide_fields").split(",")
            if x.strip()
        ]:
            if field_name not in self.fields:
                logger.warning(
            ...
            if (
                field_name
                not in _undeletable_fields_all + _undeletable_fields_educators
            ):
                del self.fields[field_name]
...
class VolunteerProfileForm(UserContactForm):
    def __init__(self, *args, **kwargs):
        ...
        for field_name in [
            x.strip().lower()
            for x in Tag.getTag("volunteer_profile_hide_fields").split(",")
            if x.strip()
        ]:
            if field_name not in self.fields:
                logger.warning(
            ...
            if (
                field_name
                not in _undeletable_fields_all + _undeletable_fields_volunteers
            ):
                del self.fields[field_name]
