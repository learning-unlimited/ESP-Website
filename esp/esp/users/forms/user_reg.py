    ...
    def clean(self, value):
        ...
        if len(email_parts) != 2:
            raise forms.ValidationError(
                'Email addresses must be of the form "name@host"'
            )
        ...
        try:
            ...
            try:
                ...
                if (
                    len(DNS.Request(qtype="a").req(email_host).answers) == 0
                    and len(DNS.Request(qtype="mx").req(email_host).answers) == 0
                ):
                    raise forms.ValidationError(
                        f'"{email_host}" is not a valid email host'
                    )
            except (
                DNS.DNSError,
                FileNotFoundError,
                socket.gaierror,
            ):  # (no resolv.conf, no nameservers, transient socket.gaierror lookup failure)
                pass
...
class EmailUserRegForm(forms.Form):
    ...
    def clean_confirm_email(self):
        if not (
            ("confirm_email" in self.cleaned_data) and ("email" in self.cleaned_data)
        ) or (self.cleaned_data["confirm_email"] != self.cleaned_data["email"]):
            raise forms.ValidationError(
    ...
    def __init__(self, *args, **kwargs):
        ...
        role_choices = [
            (item[0], item[1]["label"]) for item in ESPUser.getAllUserTypes()
        ]
...
class UserRegForm(forms.Form):
    ...
    def clean_confirm_password(self):
        ...
        ) or (self.cleaned_data["confirm_password"] != self.cleaned_data["password"]):
            raise forms.ValidationError(
...
class AwaitingActivationEmailForm(forms.Form):
    ...
    def clean_username(self):
        data = self.cleaned_data["username"]
        if (
            not ESPUser.objects.filter(username__iexact=data)
            .exclude(password="emailuser")
