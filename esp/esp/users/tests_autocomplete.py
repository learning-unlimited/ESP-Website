...
class AutocompleteTest(TestCase):
    def setUp(self):
        ...
        self.s1 = ESPUser.objects.create(
            username="s1", first_name="Alice", last_name="Alpha"
        )
        self.s1.groups.add(self.student_group)

        self.s2 = ESPUser.objects.create(
            username="s2", first_name="Bob", last_name="Bravo"
        )
        self.s2.groups.add(self.student_group)

        self.s3 = ESPUser.objects.create(
            username="s3", first_name="Charlie", last_name="Alpha"
        )
        ...
        RegistrationProfile.objects.create(
            user=self.s1, program=self.program, student_info=si1
        )
        RegistrationProfile.objects.create(
            user=self.s2, program=self.program, student_info=si2
        )
