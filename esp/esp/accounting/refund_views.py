...
def process_refund(request):
    ...
    for req in refund_requests:
        ...
        try:
            ...
            has_errors = True
            logger.warning(
                "Stripe refund InvalidRequestError for charge %s: %s",
                transfer.transaction_id,
                str(e),
            )
        except stripe.error.StripeError as e:
            ...
            has_errors = True
            logger.warning(
                "Stripe refund error for charge %s: %s", transfer.transaction_id, str(e)
            )
