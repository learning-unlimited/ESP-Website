...
from esp.utils.memcached_multikey import (
    CacheClass,
    DEFAULT_VALUE_CHUNK_SIZE,
...
class DependenciesTestCase(unittest.TestCase):
    ...
    def testDeps(self):
        ...
        self.tryImport(
            "PIL"
        )  # Needed for Django Image fields, which we use for (among other things) teacher bio's
        self.tryImport(
            "PIL._imaging"
        )  # Internal PIL module; PIL will import without it, but it won't have a lot of the functionality that we need
        self.tryImport(
            "pymemcache"
        )  # We use pymemcache as the Python<->memcached interface library.
        self.tryImport(
            "DNS"
        )  # Used for validating email address hostnames.  Imports as DNS, but the package and egg are named "pydns".
        self.tryImport("json")  # Used for some of our AJAX magic
        self.tryImport(
            "psycopg2"
        )  # Used for talking with PostgreSQL.  Someday, we'll support psycopg2, but not today...
        self.tryImport(
            "openpyxl"
        )  # Used in our giant statistics spreadsheet-generating code
        ...
        if (
            "locmem" not in default_backend.lower()
            and "dummy" not in default_backend.lower()
        ):
            ...
            elif hasattr(cache, "_wrapped_cache") and hasattr(
                cache._wrapped_cache, "_cache"
            ):
                self.assertTrue(isinstance(cache._wrapped_cache._cache, HashClient))

        self.tryExecutable(
            "latex"
        )  # Used for a whole pile of program printables, as well as inline LaTeX
        self.tryExecutable("dvips")  # Used to convert LaTeX output (.dvi) to .ps files
        self.tryExecutable(
            "convert"
        )  # Used in the generation of inline LaTeX chunks.  Also for some cases of generating .png images from LaTeX output; the student-schedule generator currently does dvips then convert to get .png's so that barcodes work
        self.tryExecutable(
            "dvipng"
        )  # Used to convert LaTeX output (.dvi) to .png files
        self.tryExecutable(
            "ps2pdf"
        )  # Used to convert LaTeX output (.dvi) to .pdf files (must go to .ps first because we use some LaTeX packages that depend on Postscript)
        self.tryExecutable(
            "inkscape"
        )  # Used to render LaTeX output (once converted to .pdf) to .svg image files
...
class MemcachedMultiKeyValueTestCase(unittest.TestCase):
    ...
    def test_large_value_round_trip_and_delete(self):
        ...
        self.assertTrue(
            self.store[cache_key].startswith(
    ...
    def test_sentinel_metadata_is_never_returned_to_the_caller(self):
        ...
        for bogus in (
            MULTIKEY_SENTINEL + separator + "garbage",
    ...
    def test_chunks_are_never_written_without_an_expiry(self):
        ...
        for chunk_key in chunk_keys:
            self.assertEqual(
                self.cache._wrapped_cache.timeouts[chunk_key],
...
class TemplateOverrideTest(DjangoTestCase):
    ...
    def test_diff_single_line_change(self):
        ...
        total_buggy = (
            diff_buggy.count('class="diff_chg"')
        ...
        diff_html = HtmlDiff().make_table(
            original_lines, override_lines, "original", "override"
        )
        ...
        self.assertLess(
            ...
            "Got {} diff markers for {} lines.".format(total_diff_lines, len(lines)),
        )
        ...
        self.assertLessEqual(
            total_diff_lines,
            total_buggy,
            "Fixed diff should not produce more markers than the buggy diff.",
        )
    ...
    def test_overrides(self):
        ...
        with reversion.create_revision():
            ...
            to.save()
        self.assertTrue(
            self.get_response_for_template("BLAARG.TEMPLATEOVERRIDE") == "Hello"
        )
        ...
        with reversion.create_revision():
            ...
            to.save()
        self.assertTrue(
            self.get_response_for_template("BLAARG.TEMPLATEOVERRIDE") == "Goodbye"
        )
    ...
    def test_diff_single_line_change_view(self):
        ...
        self.assertGreater(len(original_lines), 3, "Template must have several lines")
...
class QueryBuilderTest(DjangoTestCase):
    ...
    def test_query_builder(self):
        ...
        PrintRequest.objects.create(
            printer=happy_printer, user=nice_user, time_executed=now
        )
        PrintRequest.objects.create(
            printer=happy_printer, user=nice_user, time_executed=now
        )
        ...
        PrintRequest.objects.create(printer=sad_printer, user=mean_user)
        PrintRequest.objects.create(
            printer=slow_printer, user=nice_user, time_executed=now
        )
        ...
        has_ever_printed_filter = query_builder.SearchFilter(
            "has ever printed",
            "has ever printed",
            [query_builder.ConstantInput(Q(printrequest__time_executed__isnull=False))],
        )
        always_works_filter = query_builder.SearchFilter(
            "always works",
            "always works",
            [query_builder.ConstantInput(Q(printrequest__time_executed__isnull=True))],
            inverted=True,
        )
        ...
        self.assertEqual(
            ...
            print_query_builder.as_queryset(
                {"filter": "has ever printed", "negated": True, "values": [None]}
            ).get(),
            sad_printer,
        )
        self.assertEqual(
    ...
    def test_search_filter(self):
        ...
        self.assertEqual(
            search_filter_1.spec(),
            {
                "name": "instance",
                "title": "the instance",
                "inputs": [select_input.spec(), trivial_input.spec()],
            },
        )
        self.assertEqual(
            str(search_filter_1.as_q(["1", None])), str(Q(a_db_field="1") & Q(a="b"))
        )
    ...
    def test_select_input(self):
        ...
        select_input = query_builder.SelectInput("a_db_field", options)
        self.assertEqual(
            select_input.spec(),
            {
                "reactClass": "SelectInput",
                "options": [
                    {"name": i, "title": f"option {i}"}
    ...
    def test_optional_input(self):
        ...
        self.assertEqual(
            optional_input.spec(),
            {"reactClass": "OptionalInput", "name": "+", "inner": select_input.spec()},
        )
        self.assertEqual(str(optional_input.as_q(None)), str(Q()))
        self.assertEqual(
            str(optional_input.as_q({"inner": "5"})), str(Q(a_db_field="5"))
        )

    def test_datetime_input(self):
        datetime_input = query_builder.DatetimeInput("a_db_field")
        self.assertEqual(
            datetime_input.spec(), {"reactClass": "DatetimeInput", "name": "a db field"}
        )
        self.assertEqual(
            str(
                datetime_input.as_q(
        ...
        self.assertEqual(
            str(
                datetime_input.as_q({"comparison": "", "datetime": "11/01/2015 23:59"})
            ),
            str(Q(a_db_field=datetime.datetime(2015, 11, 1, 23, 59))),
        )
        with self.assertRaises(ValueError):
            datetime_input.as_q({"comparison": "", "datetime": "11/41/2015 23:59"})

    def test_text_input(self):
        text_input = query_builder.TextInput("a_db_field")
        self.assertEqual(
            text_input.spec(), {"reactClass": "TextInput", "name": "a db field"}
        )
        self.assertEqual(
            str(text_input.as_q("foo bar baz")), str(Q(a_db_field="foo bar baz"))
        )
...
class StripBase64ImagesTest(DjangoTestCase):
    ...
    def test_css_background_data_uri_stripped(self):
        html = (
            '<div style="background-image: url(data:image/png;base64,AAA)">text</div>'
        )
        result, count = self.strip(html)
        self.assertEqual(count, 1)
...
class ExpirableModelBoundaryTest(DjangoTestCase):
    ...
    def test_boundary_consistency(self):
        ...
        perm = Permission.objects.create(
            permission_type="Administer", start_date=start_time, end_date=end_time
        )
        ...
        self.assertTrue(
            perm.is_valid(when=end_time), "is_valid() should be True at exact end_date"
        )
        self.assertIn(
            perm,
...
class DiffTemplateOverrideViewTest(DjangoTestCase):
    def setUp(self):
        ...
        self.admin.makeAdmin()
        self.non_admin = ESPUser.objects.create_user(
            username="user_diff", password="pass"
        )
