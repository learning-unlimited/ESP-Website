...
class FormstackAppSettingsAdmin(admin.ModelAdmin):
    #   'program' is readonly, so a duplicate would have no program to attach
    #   to (and only one settings object per program is allowed anyway).
    save_as = False
...
class FormstackStudentProgramAppAdmin(admin.ModelAdmin):
    #   Mirrors a Formstack submission; 'user' and 'program' are readonly, so
    #   a duplicate could not be saved.
    save_as = False
...
class FormstackStudentClassAppAdmin(admin.ModelAdmin):
    #   Every field identifying the app is readonly or absent from the form,
    #   so a duplicate could not be saved.
    save_as = False
