...
class FormstackAppSettings(models.Model):
    ...
    coreclass_fields = models.CharField(
        max_length=80,
        blank=True,
    ...
    autopopulated_fields = models.TextField(
        blank=True,
        help_text="""\
...
field id.""",
    ...
    app_is_open = models.BooleanField(
        default=False, verbose_name="Application is currently open"
    )
    ...
    def create_username_field(self):
        ...
        api_response = self.formstack().create_field(
            self.form_id,
            {
                "field_type": "text",
                "label": "ESP Username",
...
class StudentProgramApp(models.Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.__class__ == StudentProgramApp:
            model = apps.get_model(
                "application", self.app_type + self.__class__.__name__
            )
    ...
    def admitted_to_class(self):
        ...
        """
        classapps = self.studentclassapp_set.filter(
    ...
    def waitlisted_to_class(self):
        ...
        result = []
        classapps = self.studentclassapp_set.filter(
            admission_status=StudentClassApp.WAITLIST
        )
...
class StudentClassApp(models.Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.__class__ == StudentClassApp:
            model = apps.get_model(
                "application", self.app.app_type + self.__class__.__name__
            )
            if model is not None:
                self.__class__ = model
...
class FormstackStudentProgramAppManager(models.Manager):
    def create_from_submission(self, submission, settings):
        ...
        data_dict = {int(entry["field"]): entry["value"] for entry in submission.data()}
        ...
        for preference, subject in choices.items():
            if subject is not None:
                try:
                    classapp = FormstackStudentClassApp.objects.get(
                        app=app, student_preference=preference
                    )
                except FormstackStudentClassApp.DoesNotExist:
                    classapp = FormstackStudentClassApp(
                        app=app, student_preference=preference
                    )
...
def create_app(sender, form_id, submission_id, fields, **kwargs):
    ...
    data = [{"field": field, "value": value} for field, value in fields.items()]
...
class FormstackStudentProgramApp(StudentProgramApp):
    ...
    def submission(self):
        return FormstackSubmission(
            self.submission_id, self.program_settings().formstack()
        )
    ...
    def get_responses(self, program=None):
        ...
        for response in data:
            result.append((id_to_label[response["field"]], response["value"]))
        return result
