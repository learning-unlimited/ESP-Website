...
//  This whitelist replaces the old practice of eval()ing a "script" value out
//  of the response: a response can only run code that the page has already
//  loaded and registered, and only with the arguments it sends as JSON data.
...
var apply_fragment_changes = function(data)
{
    ...
    //  Run the page callbacks that the response asked for, by name.  Only names
    //  the page passed to register_handler() are runnable; anything else is
    //  reported and ignored instead of being executed.
    if (data['callbacks'])
    {
    ...
    if ('script' in data)
    {
        //  Executing JavaScript supplied by the server is no longer supported.
        //  Views that need to run code after a fragment update should send
        //  "forms", "links" or "callbacks" data instead.
        if (window.console)
        {
...
//  Replace any existing registration with the same ID.  Fragments re-register
//  their forms and links every time the server sends new markup for them, so
//  without this the registration lists grow without bound.
