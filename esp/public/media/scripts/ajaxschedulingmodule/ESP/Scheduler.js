...
function Scheduler(
  ...
  this.init = function () {
    ...
    if (has_moderator_module === "True") {
      this.moderatorDirectory = new ModeratorDirectory(
        moderatorEl,
        data.moderators,
      );
    } else {
    ...
    this.messagePanel = new MessagePanel(
      ...
      "<i>Welcome to the Ajax Scheduler!</i><br><br>" +
        ...
        "<strong>Escape</strong>: unselect the selected section" +
        (has_moderator_module === "True" ? "/moderator" : "") +
        "<br>" +
        ...
        (has_moderator_module === "True"
          ? "<strong>F4</strong>: open the 'Moderators' tab<br>"
          : "") +
        "<strong>/</strong>: Search for a class",
    );
    this.sectionCommentDialog = new SectionCommentDialog(
      sectionDialogEl,
      this.sections,
    );
    this.sectionInfoPanel = new SectionInfoPanel(
      sectionInfoEl,
      this.sections,
      this.messagePanel,
    ...
    this.changelogFetcher = new ChangelogFetcher(this.matrix, new ApiClient());
    ...
    $j("body").on(
      ...
      function (evt) {
        // console.log(evt);
        if (evt.key === "Delete") {
          // delete is pressed: unschedule the selected section
          this.sections.unscheduleSection(this.sections.selectedSection);
        } else if (evt.key === "Escape") {
          // escape is pressed: unselect the currently selected section and/or moderator
          this.sections.unselectSection();
          if (has_moderator_module === "True")
            this.moderatorDirectory.unselectModerator();
        } else if (evt.key === "F1") {
          // F1 is pressed: open the first tab (class directory)
          evt.preventDefault();
          $j("#side-panel").tabs({ active: 0 });
        } else if (evt.key === "F2") {
          // F2 is pressed: open the second tab (room filters)
          evt.preventDefault();
          $j("#side-panel").tabs({ active: 1 });
        } else if (evt.key === "F3") {
          // F3 is pressed: open the third tab (scheduling checks)
          evt.preventDefault();
          $j("#side-panel").tabs({ active: 2 });
        } else if (evt.key === "F4") {
          // F4 is pressed: open the fourth tab (moderator directory)
          evt.preventDefault();
    ...
    $j("body").on("keyup", function (evt) {
      if (evt.key === "/") {
        // '/' is pressed: open the class directory and focus on the search box
        $j("#side-panel").tabs({ active: 0 });
    ...
    $j("body").on(
      ...
      function (evt, ui) {
        ...
        this.moderatorDirectory.selectModerator(
          this.moderatorDirectory.moderators[modID],
        );
      }.bind(this),
    );
    ...
    $j("body").on(
      ...
      function (evt, ui) {
        ...
        if ((evt.ctrlKey || evt.metaKey) && this.sections.selectedSection) {
          ...
          this.sections.swapSections(
            this.sections.selectedSection,
            cell.section,
          );
        } else if (
          (evt.ctrlKey || evt.metaKey) &&
          has_moderator_module === "True" &&
          this.moderatorDirectory.selectedModerator
        ) {
          if (
            this.moderatorDirectory.selectedModerator.sections.includes(
              cell.section.id,
            )
          ) {
    ...
    $j("body").on("click", "#print_button", function (evt, ui) {
      printJS({
        ...
        type: "html",
        css: [
          "/media/default_styles/scheduling.css",
          "/media/scripts/ajaxschedulingmodule/lib/fixed_table_rc.css",
          `https://ajax.aspnetcdn.com/ajax/jquery.ui/${jqueryui_version}/themes/base/jquery-ui.css`,
        ],
        targetStyles: ["*"],
        maxWidth: 5000,
